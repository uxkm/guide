...
`,`
  `)}
{% endset %}
{{ navbar(brand='${e}', items=items${n?`, ${n}`:``}) }}`,re=Object.fromEntries(r.map(e=>[e,n(e)]))})))()}function ae(e){let t=e=>e.map((e,t)=>x(e,t===0?`active=true`:``)).join(`
`);if(e===`basic`)return`${b}

${S(t([`대시보드`,`분석`,`사용자`,`설정`]),`bordered=true, ariaLabel='주요 메뉴'`)}`;if(e===`horizontal`)return`${b}

${S(t([`개요`,`컴포넌트`,`토큰`,`접근성`]),`mode='horizontal', bordered=true, ariaLabel='섹션 메뉴'`)}`;if(e===`icon`)return`${b}

${S([x(`대시보드`,`active=true, icon='grid'`),x(`사용자`,`icon='user'`),x(`문서`,`icon='book'`),x(`설정`,`icon='settings'`)].join(`
`),`bordered=true, ariaLabel='앱 메뉴'`)}`;if(e===`submenu`){let e=[`Button`,`Input`,`Card`].map(e=>x(e)).join(`
`),t=[`폼 레이아웃`,`데이터 테이블`].map(e=>x(e)).join(`
`);return`${b}

${S([x(`홈`,`active=true, icon='home'`),pe(`컴포넌트`,`menu-sub-components`,e,`expanded=true, icon='grid'`),pe(`패턴`,`menu-sub-patterns`,t,`icon='book'`)].join(`
`),`bordered=true, ariaLabel='컴포넌트 메뉴'`)}`}if(e===`group`)return`${b}
...
{% call menu(bordered=true, ariaLabel='설정 메뉴') %}
  ...
  {{ menuDivider() }}
  {{ menuItem(label='로그아웃') }}
{% endcall %}`;if(e===`disabled`)return`${b}

${S([x(`조회`,`active=true`),x(`편집`),x(`삭제`,`disabled=true`),x(`관리자 설정`,`disabled=true`)].join(`
`),`bordered=true, ariaLabel='권한 메뉴'`)}`;if(e===`badge`)return`${b}
{% from "components/data-display/Badge/badge.njk" import badge %}
...
{% endcall %}`;if(e===`horizontalSubmenu`){let e=[`Button`,`Input`,`Select`].map(e=>x(e)).join(`
`);return`${b}

{% call menu(mode='horizontal', bordered=true, ariaLabel='상단 메뉴') %}
  {{ menuItem(label='홈', active=true) }}
  ${pe(`컴포넌트`,`menu-sub-h-components`,e).replaceAll(`
`,`
  `)}
  {{ menuItem(label='토큰') }}
  {{ menuItem(label='접근성') }}
{% endcall %}`}if(e===`layout`){let e=[`시작하기`,`컴포넌트`].map(e=>x(e)).join(`
`);return`${b}
...
<div class="menu_demo-row">
  <div class="menu_demo-sidebar">
    {% call menu(bordered=true, compact=true, ariaLabel='앱 사이드바') %}
      ...
      ${pe(`문서`,`menu-sub-sidebar`,e,`expanded=true, icon='book'`).replaceAll(`
`,`
      `)}
...
${S(t([`항목 A`,`항목 B`,`항목 C`]),`bordered=true, compact=true, ariaLabel='컴팩트 메뉴'`)}

${S(t([`항목 A`,`항목 B`,`항목 C`]),`bordered=true, dark=true, ariaLabel='다크 서피스 메뉴'`)}`}function oe(e){let t=`Menu, MenuItem, MenuDivider, MenuGroup, MenuSubmenu`,n=[`icon`,`submenu`,`badge`,`layout`].includes(e),r=e===`badge`,i=[`import { ${t} } from '@uxkm/react/menu';`,n&&`import Icon from '@uxkm/react/icon';`,r&&`import Badge from '@uxkm/react/badge';`].filter(Boolean).join(`
`),a=[`import { ${t} } from '@uxkm/vue/menu';`,n&&`import Icon from '@uxkm/vue/icon';`,r&&`import Badge from '@uxkm/vue/badge';`].filter(Boolean).join(`
`),o=`${i}\n\nexport function Example() { return <>${de[e]}</>; }`,s=`<script setup>\n${a}\n<\/script>\n<template>\n${fe[e]}\n</template>`;return[{id:`html`,label:`HTML`,fileName:`Menu.html · ${e}`,code:ue[e]},{id:`gulp`,label:`Gulp`,fileName:`menu.njk · ${e}`,code:ae(e)},{id:`vue`,label:`Vue`,fileName:`@uxkm/vue/menu · ${e}`,code:s},{id:`nuxt`,label:`Nuxt`,fileName:`@uxkm/vue/menu · ${e}`,code:s},{id:`react`,label:`React`,fileName:`@uxkm/react/menu · ${e}`,code:o},{id:`next`,label:`Next`,fileName:`@uxkm/react/menu · ${e}`,code:o}]}var se,ce,p,le,m,h,ue,g,_,de,v,y,fe,b,x,S,pe,me;function he(){return(he=e((()=>{se=[`basic`,`horizontal`,`icon`,`submenu`,`group`,`disabled`,`badge`,`horizontalSubmenu`,`layout`,`variant`],ce=e=>`<svg class="icon menu_icon" data-component="Icon" data-icon="${e}" aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="${e===`grid`?`M4 4h6v6H4zM14 4h6v6h-6zM4 14h6v6H4zM14 14h6v6h-6z`:e===`user`?`M16 8a4 4 0 1 1-8 0 4 4 0 0 1 8 0ZM4 21a8 8 0 0 1 16 0`:e===`settings`?`M12 9a3 3 0 1 1 0 6 3 3 0 0 1 0-6Z`:e===`home`?`m3 11 9-8 9 8M5 10v11h14V10`:e===`bell`?`M18 8a6 6 0 0 0-12 0c0 7-3 9-3 9h18s-3-2-3-9M14 21h-4`:e===`mail`?`M3 5h18v14H3zM3 7l9 6 9-6`:`M4 3h12l4 4v14H4zM16 3v5h5`}"></path></svg>`,p=(e,t={})=>`<li class="menu_item${t.disabled?` is-disabled`:``}"><button type="button" class="menu_link${t.active?` is-active`:``}${t.disabled?` is-disabled`:``}"${t.active?` aria-current="page"`:``}${t.disabled?` aria-disabled="true"`:``}>${t.icon?ce(t.icon):``}<span class="menu_label">${e}</span>${t.extra?`<span class="menu_extra">${t.extra}</span>`:``}</button></li>`,le=(e,t,n,r=!1,i)=>`<li class="menu_item menu_item-submenu" aria-expanded="${r}"><button type="button" class="menu_link" data-menu-submenu-toggle aria-expanded="${r}" aria-controls="${t}">${i?ce(i):``}<span class="menu_label">${e}</span><svg class="icon menu_arrow" data-component="Icon" data-icon="chevron-down" aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="m6 9 6 6 6-6"></path></svg></button><ul id="${t}" class="menu_submenu">${n}</ul></li>`,m=(e,t,n=`menu_vertical menu_bordered`)=>`<nav class="menu ${n}" data-component="Menu" data-menu-selectable aria-label="${t}"><ul class="menu_list">${e}</ul></nav>`,h=(e,t=0)=>e.map((e,n)=>p(e,{active:n===t})).join(``),ue={basic:m(h([`대시보드`,`분석`,`사용자`,`설정`]),`주요 메뉴`),horizontal:m(h([`개요`,`컴포넌트`,`토큰`,`접근성`]),`섹션 메뉴`,`menu_horizontal menu_bordered`),icon:m([[`대시보드`,`grid`],[`사용자`,`user`],[`문서`,`book`],[`설정`,`settings`]].map(([e,t],n)=>p(e,{icon:t,active:n===0})).join(``),`앱 메뉴`),submenu:m(p(`홈`,{active:!0,icon:`home`})+le(`컴포넌트`,`menu-sub-components`,h([`Button`,`Input`,`Card`],-1),!0,`grid`)+le(`패턴`,`menu-sub-patterns`,h([`폼 레이아웃`,`데이터 테이블`],-1),!1,`book`),`컴포넌트 메뉴`),group:m(`<li class="menu_group" role="presentation"><span class="menu_group-title">계정</span></li>`+p(`프로필`,{active:!0})+p(`보안`)+`<li class="menu_divider" role="separator"></li><li class="menu_group" role="presentation"><span class="menu_group-title">앱</span></li>`+p(`알림`)+p(`언어`)+`<li class="menu_divider" role="separator"></li>`+p(`로그아웃`),`설정 메뉴`),disabled:m(p(`조회`,{active:!0})+p(`편집`)+p(`삭제`,{disabled:!0})+p(`관리자 설정`,{disabled:!0}),`권한 메뉴`),badge:m(p(`받은편지함`,{active:!0,icon:`mail`,extra:`<span class="badge badge_count color_primary" data-component="Badge">12</span>`})+p(`알림`,{icon:`bell`,extra:`<span class="badge_dot-only color_danger" data-component="Badge" role="status" aria-label="새 알림"></span>`})+p(`메시지`,{icon:`mail`,extra:`<span class="badge color_default" data-component="Badge">Beta</span>`}),`알림 메뉴`),horizontalSubmenu:m(p(`홈`,{active:!0})+le(`컴포넌트`,`menu-sub-h-components`,h([`Button`,`Input`,`Select`],-1))+p(`토큰`)+p(`접근성`),`상단 메뉴`,`menu_horizontal menu_bordered`),layout:`<div class="menu_demo-row"><div class="menu_demo-sidebar">${m(p(`개요`,{active:!0,icon:`grid`})+le(`문서`,`menu-sub-sidebar`,h([`시작하기`,`컴포넌트`],-1),!0,`book`)+p(`설정`,{icon:`settings`}),`앱 사이드바`,`menu_vertical menu_bordered menu_compact`)}</div><div class="menu_demo-content"><p>선택한 메뉴에 해당하는 콘텐츠 영역입니다. 사이드바 메뉴와 함께 레이아웃을 구성할 때 사용합니다.</p></div></div>`,variant:m(h([`항목 A`,`항목 B`,`항목 C`]),`컴팩트 메뉴`,`menu_vertical menu_bordered menu_compact`)+`
`+m(h([`항목 A`,`항목 B`,`항목 C`]),`다크 서피스 메뉴`,`menu_vertical menu_bordered menu_dark`)},g=(e,t=``)=>`<MenuItem label="${e}"${t?` ${t}`:``} />`,_=(e,t=`bordered`)=>`<Menu ${t}>${e}</Menu>`,de={basic:_(g(`대시보드`,`active`)+g(`분석`)+g(`사용자`)+g(`설정`),`bordered ariaLabel="주요 메뉴"`),horizontal:_(g(`개요`,`active`)+g(`컴포넌트`)+g(`토큰`)+g(`접근성`),`mode="horizontal" bordered ariaLabel="섹션 메뉴"`),icon:_(g(`대시보드`,`active icon={<Icon name="grid" className="menu_icon" />}`)+g(`사용자`,`icon={<Icon name="user" className="menu_icon" />}`)+g(`문서`,`icon={<Icon name="book" className="menu_icon" />}`)+g(`설정`,`icon={<Icon name="settings" className="menu_icon" />}`),`bordered ariaLabel="앱 메뉴"`),submenu:_(g(`홈`,`active icon={<Icon name="home" className="menu_icon" />}`)+`<MenuSubmenu label="컴포넌트" expanded submenuId="menu-sub-components" icon={<Icon name="grid" className="menu_icon" />}>`+g(`Button`)+g(`Input`)+g(`Card`)+`</MenuSubmenu><MenuSubmenu label="패턴" submenuId="menu-sub-patterns" icon={<Icon name="book" className="menu_icon" />}>`+g(`폼 레이아웃`)+g(`데이터 테이블`)+`</MenuSubmenu>`,`bordered ariaLabel="컴포넌트 메뉴"`),group:_(`<MenuGroup title="계정" />`+g(`프로필`,`active`)+g(`보안`)+`<MenuDivider /><MenuGroup title="앱" />`+g(`알림`)+g(`언어`)+`<MenuDivider />`+g(`로그아웃`),`bordered ariaLabel="설정 메뉴"`),disabled:_(g(`조회`,`active`)+g(`편집`)+g(`삭제`,`disabled`)+g(`관리자 설정`,`disabled`),`bordered ariaLabel="권한 메뉴"`),badge:_(g(`받은편지함`,`active icon={<Icon name="mail" className="menu_icon" />} extra={<Badge count color="primary" label="12" />}`)+g(`알림`,`icon={<Icon name="bell" className="menu_icon" />} extra={<Badge dotOnly color="danger" ariaLabel="새 알림" />}`)+g(`메시지`,`icon={<Icon name="mail" className="menu_icon" />} extra={<Badge color="default" label="Beta" />}`),`bordered ariaLabel="알림 메뉴"`),horizontalSubmenu:_(g(`홈`,`active`)+`<MenuSubmenu label="컴포넌트" submenuId="menu-sub-h-components">`+g(`Button`)+g(`Input`)+g(`Select`)+`</MenuSubmenu>`+g(`토큰`)+g(`접근성`),`mode="horizontal" bordered ariaLabel="상단 메뉴"`),layout:`<div className="menu_demo-row"><div className="menu_demo-sidebar">${_(g(`개요`,`active icon={<Icon name="grid" className="menu_icon" />}`)+`<MenuSubmenu label="문서" expanded submenuId="menu-sub-sidebar" icon={<Icon name="book" className="menu_icon" />}>`+g(`시작하기`)+g(`컴포넌트`)+`</MenuSubmenu>`+g(`설정`,`icon={<Icon name="settings" className="menu_icon" />}`),`bordered compact ariaLabel="앱 사이드바"`)}</div><div className="menu_demo-content"><p>선택한 메뉴에 해당하는 콘텐츠 영역입니다. 사이드바 메뉴와 함께 레이아웃을 구성할 때 사용합니다.</p></div></div>`,variant:_(g(`항목 A`,`active`)+g(`항목 B`)+g(`항목 C`),`bordered compact ariaLabel="컴팩트 메뉴"`)+_(g(`항목 A`,`active`)+g(`항목 B`)+g(`항목 C`),`bordered dark ariaLabel="다크 서피스 메뉴"`)},v=(e,t=``,n=``,r=``)=>n||r?`<MenuItem label="${e}"${t?` ${t}`:``}>${n?`<template #icon><Icon name="${n}" class="menu_icon" /></template>`:``}${r?`<template #extra>${r}</template>`:``}</MenuItem>`:`<MenuItem label="${e}"${t?` ${t}`:``} />`,y=(e,t=`bordered`)=>`<Menu ${t}>${e}</Menu>`,fe={basic:y(v(`대시보드`,`active`)+v(`분석`)+v(`사용자`)+v(`설정`),`bordered aria-label="주요 메뉴"`),horizontal:y(v(`개요`,`active`)+v(`컴포넌트`)+v(`토큰`)+v(`접근성`),`mode="horizontal" bordered aria-label="섹션 메뉴"`),icon:y(v(`대시보드`,`active`,`grid`)+v(`사용자`,``,`user`)+v(`문서`,``,`book`)+v(`설정`,``,`settings`),`bordered aria-label="앱 메뉴"`),submenu:y(v(`홈`,`active`,`home`)+`<MenuSubmenu label="컴포넌트" expanded submenu-id="menu-sub-components"><template #icon><Icon name="grid" class="menu_icon" /></template>`+v(`Button`)+v(`Input`)+v(`Card`)+`</MenuSubmenu><MenuSubmenu label="패턴" submenu-id="menu-sub-patterns"><template #icon><Icon name="book" class="menu_icon" /></template>`+v(`폼 레이아웃`)+v(`데이터 테이블`)+`</MenuSubmenu>`,`bordered aria-label="컴포넌트 메뉴"`),group:y(`<MenuGroup title="계정" />`+v(`프로필`,`active`)+v(`보안`)+`<MenuDivider /><MenuGroup title="앱" />`+v(`알림`)+v(`언어`)+`<MenuDivider />`+v(`로그아웃`),`bordered aria-label="설정 메뉴"`),disabled:y(v(`조회`,`active`)+v(`편집`)+v(`삭제`,`disabled`)+v(`관리자 설정`,`disabled`),`bordered aria-label="권한 메뉴"`),badge:y(v(`받은편지함`,`active`,`mail`,`<Badge count color="primary" label="12" />`)+v(`알림`,``,`bell`,`<Badge dot-only color="danger" aria-label="새 알림" />`)+v(`메시지`,``,`mail`,`<Badge color="default" label="Beta" />`),`bordered aria-label="알림 메뉴"`),horizontalSubmenu:y(v(`홈`,`active`)+`<MenuSubmenu label="컴포넌트" submenu-id="menu-sub-h-components">`+v(`Button`)+v(`Input`)+v(`Select`)+`</MenuSubmenu>`+v(`토큰`)+v(`접근성`),`mode="horizontal" bordered aria-label="상단 메뉴"`),layout:`<div class="menu_demo-row"><div class="menu_demo-sidebar">${y(v(`개요`,`active`,`grid`)+`<MenuSubmenu label="문서" expanded submenu-id="menu-sub-sidebar"><template #icon><Icon name="book" class="menu_icon" /></template>`+v(`시작하기`)+v(`컴포넌트`)+`</MenuSubmenu>`+v(`설정`,``,`settings`),`bordered compact aria-label="앱 사이드바"`)}</div><div class="menu_demo-content"><p>선택한 메뉴에 해당하는 콘텐츠 영역입니다. 사이드바 메뉴와 함께 레이아웃을 구성할 때 사용합니다.</p></div></div>`,variant:y(v(`항목 A`,`active`)+v(`항목 B`)+v(`항목 C`),`bordered compact aria-label="컴팩트 메뉴"`)+y(v(`항목 A`,`active`)+v(`항목 B`)+v(`항목 C`),`bordered dark aria-label="다크 서피스 메뉴"`)},b=`{% from "components/navigation/Menu/menu.njk" import menu, menuItem, menuSubmenu, menuGroup, menuDivider %}`,x=(e,t=``)=>`{{ menuItem(label='${e}'${t?`, ${t}`:``}) }}`,S=(e,t)=>`{% call menu(${t}) %}
  ${e.replaceAll(`
`,`
  `)}
{% endcall %}`,pe=(e,t,n,r=``)=>`{% call menuSubmenu(label='${e}', id='${t}'${r?`, ${r}`:``}) %}
  ${n.replaceAll(`
`,`
  `)}
{% endcall %}`,me=Object.fromEntries(se.map(e=>[e,oe(e)]))})))()}function C(e,t,n={}){let r=n.classes||`tabs_line`,i=n.label||`탭`,a=t.map((t,n)=>`<button type="button" class="tabs_tab${n===0?` is-active`:``}${t.disabled?` is-disabled`:``}" role="tab" id="${e}-tab-${n}" aria-selected="${n===0}" aria-controls="${e}-panel-${n}"${n?` tabindex="-1"`:``}${t.disabled?` aria-disabled="true" disabled`:``}>${t.icon?ye(t.icon):``}<span class="tabs_label">${t.label}</span>${t.badge?`<span class="tabs_badge">${t.badge}</span>`:``}</button>`).join(``),o=t.map((t,n)=>`<div class="tabs_panel${n===0?` is-active`:``}" role="tabpanel" id="${e}-panel-${n}" aria-labelledby="${e}-tab-${n}"${n?` hidden`:``}>${t.content}</div>`).join(``),s=n.nav?`<button type="button" class="btn btn_outline color_default btn_icon-only tabs_nav" data-tabs-scroll="prev" aria-label="이전 탭">‹</button>`:``,c=n.nav?`<button type="button" class="btn btn_outline color_default btn_icon-only tabs_nav" data-tabs-scroll="next" aria-label="다음 탭">›</button>`:``;return`<div class="tabs ${r}" data-component="Tabs" data-tabs><div class="tabs_bar">${s}<div class="tabs_list-wrap${n.nav?` tabs_scroll-viewport`:``}"><div class="tabs_list" role="tablist" aria-label="${i}">${a}</div></div>${c}${n.extra?`<div class="tabs_extra">${n.extra}</div>`:``}</div><div class="tabs_panels">${o}</div></div>`}function w(e,t,n={}){let r=t.map((t,n)=>{let r=t.badge?`, badge=${t.label===`읽지 않음`?`unreadBadge`:`importantBadge`}`:``;return`  {{ tab(id='${e}-tab-${n}', panelId='${e}-panel-${n}', label='${t.label}'${n===0?`, active=true`:``}${t.disabled?`, disabled=true`:``}${t.icon?`, icon='${t.icon}'`:``}${r}) }}`}).join(`
`),i=t.map((t,n)=>`  {% call tabPanel(id='${e}-panel-${n}', tabId='${e}-tab-${n}'${n===0?`, active=true`:``}) %}${t.content}{% endcall %}`).join(`
`),a=[`tabs=tabItems`,`panels=tabPanels`,n.variant&&`variant='${n.variant}'`,n.size&&`size='${n.size}'`,n.layout&&`layout='${n.layout}'`,n.vertical&&`vertical=true`,n.scrollable&&`scrollable=true`,n.indicator&&`indicator='${n.indicator}'`,n.mode&&`mode='${n.mode}'`,n.ariaLabel&&`ariaLabel='${n.ariaLabel}'`,n.className&&`className='${n.className}'`,n.extra&&`extra=${n.extra}`].filter(Boolean).join(`, `);return`${xe}${n.imports?`\n${n.imports}`:``}${n.prelude?`\n\n${n.prelude}`:``}
...
${i}
{% endset %}
{{ tabs(${a}) }}`}function ge(e){return e===`dynamic`?`<Tabs mode="dynamic" :items="items" aria-label="동적 탭"><template #panel="{ value }"><p>{{ value === 'overview' ? '프로젝트 개요와 목표를 설명하는 영역입니다.' : value === 'team' ? '팀 구성원과 역할을 표시합니다.' : '최근 활동 로그와 타임라인을 보여줍니다.' }}</p></template></Tabs>`:e===`icon`?`<Tabs aria-label="대시보드 섹션"><TabPanel label="대시보드" active><template #icon><Icon name="grid" class="tabs_icon" /></template>대시보드 콘텐츠</TabPanel><TabPanel label="사용자"><template #icon><Icon name="user" class="tabs_icon" /></template>사용자 콘텐츠</TabPanel><TabPanel label="문서"><template #icon><Icon name="book" class="tabs_icon" /></template>문서 콘텐츠</TabPanel></Tabs>`:e===`badge`?`<Tabs aria-label="알림 센터"><TabPanel label="전체" active>전체 알림 목록</TabPanel><TabPanel label="읽지 않음"><template #badge><Badge count color="primary" label="12" /></template>읽지 않은 알림</TabPanel><TabPanel label="중요"><template #badge><Badge dot-only color="danger" aria-label="새 중요 알림" /></template>중요 알림</TabPanel></Tabs>`:e===`extra`?`<Tabs aria-label="파일 보기"><template #extra><Button variant="filled" color="primary" size="sm" label="새 파일" /></template>${D(`내 파일`,`내 파일 목록`,`active`)}${D(`공유됨`,`공유된 파일 목록`)}${D(`최근`,`최근 파일 목록`)}</Tabs>`:we[e].replaceAll(`ariaLabel=`,`aria-label=`).replaceAll(`className=`,`class=`)}function _e(e){let t=e===`icon`,n=e===`badge`,r=e===`extra`,i=[`import { Tabs, TabPanel, TabMenu } from '@uxkm/react/tabs';`,t&&`import Icon from '@uxkm/react/icon';`,n&&`import Badge from '@uxkm/react/badge';`,r&&`import Button from '@uxkm/react/button';`].filter(Boolean).join(`
...
`),o=`${i}\n\nexport function Example() { return <>${we[e]}</>; }`,s=`<script setup>\n${a}\n<\/script>\n<template>\n${ge(e)}\n</template>`;return[{id:`html`,label:`HTML`,fileName:`Tabs.html · ${e}`,code:be[e]},{id:`gulp`,label:`Gulp`,fileName:`tabs.njk · ${e}`,code:Se[e]},{id:`vue`,label:`Vue`,fileName:`@uxkm/vue/tabs · ${e}`,code:s},{id:`nuxt`,label:`Nuxt`,fileName:`@uxkm/vue/tabs · ${e}`,code:s},{id:`react`,label:`React`,fileName:`@uxkm/react/tabs · ${e}`,code:o},{id:`next`,label:`Next`,fileName:`@uxkm/react/tabs · ${e}`,code:o}]}var ve,ye,T,E,be,xe,Se,D,Ce,we,Te;function Ee(){return(Ee=e((()=>{ve=[`basic`,`dynamic`,`indicator`,`equal`,`layoutScroll`,`card`,`pill`,`size`,`icon`,`badge`,`extra`,`scroll`,`disabled`,`vertical`],ye=e=>`<svg class="icon tabs_icon" data-component="Icon" data-icon="${e}" aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="${e===`grid`?`M4 4h6v6H4zM14 4h6v6h-6zM4 14h6v6H4zM14 14h6v6h-6z`:e===`user`?`M16 8a4 4 0 1 1-8 0 4 4 0 0 1 8 0ZM4 21a8 8 0 0 1 16 0`:`M4 3h12l4 4v14H4zM16 3v5h5`}"></path></svg>`,T=[{label:`개요`,content:`<p>프로젝트 개요와 목표를 설명하는 영역입니다.</p>`},{label:`팀`,content:`<p>팀 구성원과 역할을 표시합니다.</p>`},{label:`활동`,content:`<p>최근 활동 로그와 타임라인을 보여줍니다.</p>`}],E=[[`홈`,`홈 패널`],[`제품 소개`,`제품 패널`],[`솔루션`,`솔루션 패널`],[`가격 정책`,`가격 패널`],[`고객 사례`,`고객 사례 패널`],[`리소스 센터`,`리소스 패널`],[`기술 지원`,`지원 패널`]].map(([e,t])=>({label:e,content:t})),be={basic:C(`basic`,T,{label:`프로젝트 정보`}),dynamic:C(`dynamic`,T,{classes:`tabs_line tabs_dynamic`,label:`동적 탭`}),indicator:C(`slide-line`,T,{classes:`tabs_line tabs_equal tabs_indicator-slide`,label:`슬라이드 라인 탭`})+`

`+C(`slide-pill`,[{label:`목록`,content:`필 스킨에서 배경 인디케이터가 이동합니다.`},{label:`그리드`,content:`그리드 뷰 콘텐츠`},{label:`보드`,content:`보드 뷰 콘텐츠`}],{classes:`tabs_pill tabs_equal tabs_indicator-slide`,label:`슬라이드 필 탭`}),equal:C(`equal-line`,[...T,{label:`설정`,content:`프로젝트 설정을 관리합니다.`}],{classes:`tabs_line tabs_equal`,label:`균등 분할 탭`})+`

`+C(`equal-pill`,[`일간`,`주간`,`월간`,`연간`].map(e=>({label:e,content:`${e} 통계`})),{classes:`tabs_pill tabs_equal tabs_indicator-slide`,label:`균등 분할 필 탭`}),layoutScroll:C(`nav-scroll`,E,{classes:`tabs_line tabs_scroll-nav tabs_demo-narrow`,label:`네비 스크롤 탭`,nav:!0}),card:C(`card`,[{label:`프로필`,content:`<p>이름, 아바타, 소개 문구를 수정합니다.</p>`},{label:`보안`,content:`<p>비밀번호 변경과 2단계 인증을 설정합니다.</p>`},{label:`알림`,content:`<p>이메일·푸시 알림 수신 여부를 관리합니다.</p>`}],{classes:`tabs_card`,label:`계정 설정`}),pill:C(`pill`,[{label:`목록`,content:`목록 뷰 콘텐츠`},{label:`그리드`,content:`그리드 뷰 콘텐츠`},{label:`보드`,content:`보드 뷰 콘텐츠`}],{classes:`tabs_pill`,label:`보기 모드`}),size:C(`small`,[{label:`Small A`,content:`Small 탭 패널`},{label:`Small B`,content:`Small 탭 패널 B`}],{classes:`tabs_line tabs_sm`,label:`Small 탭`})+`

`+C(`large`,[{label:`Large A`,content:`Large 탭 패널`},{label:`Large B`,content:`Large 탭 패널 B`}],{classes:`tabs_line tabs_lg`,label:`Large 탭`}),icon:C(`icon`,[{label:`대시보드`,content:`대시보드 콘텐츠`,icon:`grid`},{label:`사용자`,content:`사용자 콘텐츠`,icon:`user`},{label:`문서`,content:`문서 콘텐츠`,icon:`book`}],{label:`대시보드 섹션`}),badge:C(`badge`,[{label:`전체`,content:`전체 알림 목록`},{label:`읽지 않음`,content:`읽지 않은 알림`,badge:`<span class="badge badge_count color_primary" data-component="Badge">12</span>`},{label:`중요`,content:`중요 알림`,badge:`<span class="badge_dot-only color_danger" data-component="Badge" role="status" aria-label="새 중요 알림"></span>`}],{label:`알림 센터`}),extra:C(`extra`,[{label:`내 파일`,content:`내 파일 목록`},{label:`공유됨`,content:`공유된 파일 목록`},{label:`최근`,content:`최근 파일 목록`}],{label:`파일 보기`,extra:`<button type="button" class="btn btn_filled color_primary btn_sm">새 파일</button>`}),scroll:C(`scroll`,E.map(e=>({...e,label:e.label.replace(` 소개`,``).replace(` 정책`,``).replace(` 센터`,``).replace(`기술 `,``)})),{classes:`tabs_line tabs_scrollable tabs_demo-narrow`,label:`긴 탭 목록`}),disabled:C(`disabled`,[{label:`공개`,content:`공개 콘텐츠`},{label:`팀`,content:`팀 전용 콘텐츠`},{label:`관리자`,content:`관리자 전용 콘텐츠`,disabled:!0}],{label:`권한별 탭`}),vertical:C(`vertical-line`,[{label:`일반`,content:`일반 설정 패널`},{label:`보안`,content:`보안 설정 패널`},{label:`결제`,content:`결제 설정 패널`}],{classes:`tabs_line tabs_vertical`,label:`수직 라인 탭`})+`

`+C(`vertical-card`,[{label:`문서`,content:`문서 관리 패널`},{label:`미디어`,content:`미디어 관리 패널`},{label:`아카이브`,content:`아카이브 패널`}],{classes:`tabs_card tabs_vertical`,label:`수직 카드 탭`})},xe=`{% from "components/navigation/Tabs/tabs.njk" import tabs, tab, tabPanel %}`,Se={basic:w(`basic`,T,{ariaLabel:`프로젝트 정보`}),dynamic:w(`dynamic`,T,{mode:`dynamic`,ariaLabel:`동적 탭`}),indicator:w(`slide-line`,T,{layout:`equal`,indicator:`slide`,ariaLabel:`슬라이드 라인 탭`})+`

`+w(`slide-pill`,[{label:`목록`,content:`필 스킨에서 배경 인디케이터가 이동합니다.`},{label:`그리드`,content:`그리드 뷰 콘텐츠`},{label:`보드`,content:`보드 뷰 콘텐츠`}],{variant:`pill`,layout:`equal`,indicator:`slide`,ariaLabel:`슬라이드 필 탭`}),equal:w(`equal-line`,[...T,{label:`설정`,content:`프로젝트 설정을 관리합니다.`}],{layout:`equal`,ariaLabel:`균등 분할 탭`})+`

`+w(`equal-pill`,[`일간`,`주간`,`월간`,`연간`].map(e=>({label:e,content:`${e} 통계`})),{variant:`pill`,layout:`equal`,indicator:`slide`,ariaLabel:`균등 분할 필 탭`}),layoutScroll:w(`nav-scroll`,E,{layout:`scroll`,className:`tabs_demo-narrow`,ariaLabel:`네비 스크롤 탭`}),card:w(`card`,[{label:`프로필`,content:`<p>이름, 아바타, 소개 문구를 수정합니다.</p>`},{label:`보안`,content:`<p>비밀번호 변경과 2단계 인증을 설정합니다.</p>`},{label:`알림`,content:`<p>이메일·푸시 알림 수신 여부를 관리합니다.</p>`}],{variant:`card`,ariaLabel:`계정 설정`}),pill:w(`pill`,[{label:`목록`,content:`목록 뷰 콘텐츠`},{label:`그리드`,content:`그리드 뷰 콘텐츠`},{label:`보드`,content:`보드 뷰 콘텐츠`}],{variant:`pill`,ariaLabel:`보기 모드`}),size:w(`small`,[{label:`Small A`,content:`Small 탭 패널`},{label:`Small B`,content:`Small 탭 패널 B`}],{size:`sm`,ariaLabel:`Small 탭`})+`

`+w(`large`,[{label:`Large A`,content:`Large 탭 패널`},{label:`Large B`,content:`Large 탭 패널 B`}],{size:`lg`,ariaLabel:`Large 탭`}),icon:w(`icon`,[{label:`대시보드`,content:`대시보드 콘텐츠`,icon:`grid`},{label:`사용자`,content:`사용자 콘텐츠`,icon:`user`},{label:`문서`,content:`문서 콘텐츠`,icon:`book`}],{ariaLabel:`대시보드 섹션`}),badge:w(`badge`,[{label:`전체`,content:`전체 알림 목록`},{label:`읽지 않음`,content:`읽지 않은 알림`,badge:`badge`},{label:`중요`,content:`중요 알림`,badge:`badge`}],{ariaLabel:`알림 센터`,imports:`{% from "components/data-display/Badge/badge.njk" import badge %}`,prelude:`{% set unreadBadge %}{{ badge(count=true, color='primary', label='12') }}{% endset %}
{% set importantBadge %}{{ badge(dotOnly=true, color='danger', ariaLabel='새 중요 알림') }}{% endset %}`}),extra:w(`extra`,[{label:`내 파일`,content:`내 파일 목록`},{label:`공유됨`,content:`공유된 파일 목록`},{label:`최근`,content:`최근 파일 목록`}],{ariaLabel:`파일 보기`,imports:`{% from "components/basic/Button/button.njk" import button %}`,prelude:`{% set tabsExtra %}{{ button(variant='filled', color='primary', size='sm', label='새 파일') }}{% endset %}`,extra:`tabsExtra`}),scroll:w(`scroll`,E.map(e=>({...e,label:e.label.replace(` 소개`,``).replace(` 정책`,``).replace(` 센터`,``).replace(`기술 `,``)})),{scrollable:!0,className:`tabs_demo-narrow`,ariaLabel:`긴 탭 목록`}),disabled:w(`disabled`,[{label:`공개`,content:`공개 콘텐츠`},{label:`팀`,content:`팀 전용 콘텐츠`},{label:`관리자`,content:`관리자 전용 콘텐츠`,disabled:!0}],{ariaLabel:`권한별 탭`}),vertical:w(`vertical-line`,[{label:`일반`,content:`일반 설정 패널`},{label:`보안`,content:`보안 설정 패널`},{label:`결제`,content:`결제 설정 패널`}],{vertical:!0,ariaLabel:`수직 라인 탭`})+`

`+w(`vertical-card`,[{label:`문서`,content:`문서 관리 패널`},{label:`미디어`,content:`미디어 관리 패널`},{label:`아카이브`,content:`아카이브 패널`}],{variant:`card`,vertical:!0,ariaLabel:`수직 카드 탭`})},D=(e,t,n=``)=>`<TabPanel label="${e}"${n?` ${n}`:``}>${t}</TabPanel>`,Ce=D(`개요`,`<p>프로젝트 개요와 목표를 설명하는 영역입니다.</p>`,`active`)+D(`팀`,`<p>팀 구성원과 역할을 표시합니다.</p>`)+D(`활동`,`<p>최근 활동 로그와 타임라인을 보여줍니다.</p>`),we={basic:`<Tabs ariaLabel="프로젝트 정보">${Ce}</Tabs>`,dynamic:`<Tabs mode="dynamic" items={[{ key: 'overview', label: '개요', active: true }, { key: 'team', label: '팀' }, { key: 'activity', label: '활동' }]} ariaLabel="동적 탭" panel={({ value }) => <p>{value === 'overview' ? '프로젝트 개요와 목표를 설명하는 영역입니다.' : value === 'team' ? '팀 구성원과 역할을 표시합니다.' : '최근 활동 로그와 타임라인을 보여줍니다.'}</p>} />`,indicator:`<Tabs indicator="slide" layout="equal" ariaLabel="슬라이드 라인 탭">${Ce}</Tabs>\n<Tabs variant="pill" indicator="slide" layout="equal" ariaLabel="슬라이드 필 탭">${D(`목록`,`필 스킨에서 배경 인디케이터가 이동합니다.`,`active`)}${D(`그리드`,`그리드 뷰 콘텐츠`)}${D(`보드`,`보드 뷰 콘텐츠`)}</Tabs>`,equal:`<Tabs layout="equal" ariaLabel="균등 분할 탭">${Ce}${D(`설정`,`프로젝트 설정을 관리합니다.`)}</Tabs>\n<Tabs layout="equal" variant="pill" indicator="slide" ariaLabel="균등 분할 필 탭">${[`일간`,`주간`,`월간`,`연간`].map((e,t)=>D(e,`${e} 통계`,t===0?`active`:``)).join(``)}</Tabs>`,layoutScroll:`<Tabs layout="scroll" className="tabs_demo-narrow" ariaLabel="네비 스크롤 탭">${E.map((e,t)=>D(e.label,e.content,t===0?`active`:``)).join(``)}</Tabs>`,card:`<Tabs variant="card" ariaLabel="계정 설정">${D(`프로필`,`<p>이름, 아바타, 소개 문구를 수정합니다.</p>`,`active`)}${D(`보안`,`<p>비밀번호 변경과 2단계 인증을 설정합니다.</p>`)}${D(`알림`,`<p>이메일·푸시 알림 수신 여부를 관리합니다.</p>`)}</Tabs>`,pill:`<Tabs variant="pill" ariaLabel="보기 모드">${D(`목록`,`목록 뷰 콘텐츠`,`active`)}${D(`그리드`,`그리드 뷰 콘텐츠`)}${D(`보드`,`보드 뷰 콘텐츠`)}</Tabs>`,size:`<Tabs size="sm" ariaLabel="Small 탭">${D(`Small A`,`Small 탭 패널`,`active`)}${D(`Small B`,`Small 탭 패널 B`)}</Tabs>\n<Tabs size="lg" ariaLabel="Large 탭">${D(`Large A`,`Large 탭 패널`,`active`)}${D(`Large B`,`Large 탭 패널 B`)}</Tabs>`,icon:`<Tabs ariaLabel="대시보드 섹션">${D(`대시보드`,`대시보드 콘텐츠`,`active icon={<Icon name="grid" className="tabs_icon" />}`)}${D(`사용자`,`사용자 콘텐츠`,`icon={<Icon name="user" className="tabs_icon" />}`)}${D(`문서`,`문서 콘텐츠`,`icon={<Icon name="book" className="tabs_icon" />}`)}</Tabs>`,badge:`<Tabs ariaLabel="알림 센터">${D(`전체`,`전체 알림 목록`,`active`)}${D(`읽지 않음`,`읽지 않은 알림`,`badge={<Badge count color="primary" label="12" />}`)}${D(`중요`,`중요 알림`,`badge={<Badge dotOnly color="danger" ariaLabel="새 중요 알림" />}`)}</Tabs>`,extra:`<Tabs ariaLabel="파일 보기" extra={<Button variant="filled" color="primary" size="sm" label="새 파일" />}>${D(`내 파일`,`내 파일 목록`,`active`)}${D(`공유됨`,`공유된 파일 목록`)}${D(`최근`,`최근 파일 목록`)}</Tabs>`,scroll:`<Tabs scrollable className="tabs_demo-narrow" ariaLabel="긴 탭 목록">${[`홈`,`제품`,`솔루션`,`가격`,`고객 사례`,`리소스`,`지원`].map((e,t)=>D(e,`${e} 패널`,t===0?`active`:``)).join(``)}</Tabs>`,disabled:`<Tabs ariaLabel="권한별 탭">${D(`공개`,`공개 콘텐츠`,`active`)}${D(`팀`,`팀 전용 콘텐츠`)}${D(`관리자`,`관리자 전용 콘텐츠`,`disabled`)}</Tabs>`,vertical:`<Tabs vertical ariaLabel="수직 라인 탭">${D(`일반`,`일반 설정 패널`,`active`)}${D(`보안`,`보안 설정 패널`)}${D(`결제`,`결제 설정 패널`)}</Tabs>\n<Tabs vertical variant="card" ariaLabel="수직 카드 탭">${D(`문서`,`문서 관리 패널`,`active`)}${D(`미디어`,`미디어 관리 패널`)}${D(`아카이브`,`아카이브 패널`)}</Tabs>`},Te=Object.fromEntries(ve.map(e=>[e,_e(e)]))})))()}function De(e){let t=e===`icon`||e===`ellipsis`,n=e===`ellipsis`,r=[`import { Breadcrumb, BreadcrumbItem${n?`, BreadcrumbEllipsis`:``} } from '@uxkm/react/breadcrumb';`,t&&`import Icon from '@uxkm/react/icon';`,n&&`import { useState } from 'react';`].filter(Boolean).join(`
...
const items = (last) => [{ label: '홈', href: '#' }, { label: '가이드', href: '#' }, { label: last, current: true }];`:``,o=n?`${r}\n\nexport function Example() { ${Me[e]} }`:`${r}${a}\n\nexport function Example() { return <>${Me[e]}</>; }`,s=`<script setup>\n${`${i}${a}${n?`
const expanded = ref(false);`:``}`}\n<\/script>\n<template>\n${Ne[e]}\n</template>`;return[{id:`html`,label:`HTML`,fileName:`Breadcrumb.html · ${e}`,code:ke[e]},{id:`gulp`,label:`Gulp`,fileName:`breadcrumb.njk · ${e}`,code:je[e]},{id:`vue`,label:`Vue`,fileName:`@uxkm/vue/breadcrumb · ${e}`,code:s},{id:`nuxt`,label:`Nuxt`,fileName:`@uxkm/vue/breadcrumb · ${e}`,code:s},{id:`react`,label:`React`,fileName:`@uxkm/react/breadcrumb · ${e}`,code:o},{id:`next`,label:`Next`,fileName:`@uxkm/react/breadcrumb · ${e}`,code:o}]}var Oe,O,k,A,ke,j,Ae,je,Me,Ne,Pe;function Fe(){return(Fe=e((()=>{Oe=[`basic`,`separator`,`icon`,`size`,`ellipsis`,`state`],O=(e,t={})=>`<li class="breadcrumb_item${t.current?` is-current`:``}"${t.current?` aria-current="page"`:``}${t.hidden?` data-breadcrumb-hidden hidden`:``}>${t.current?`<span class="breadcrumb_current">${e}</span>`:t.disabled?`<span class="breadcrumb_link is-disabled" aria-disabled="true">${e}</span>`:`<a class="breadcrumb_link${t.icon?` breadcrumb_link-icon`:``}" href="${t.href||`#`}"${t.icon?` aria-label="홈"`:``}>${t.icon?`<svg class="icon breadcrumb_icon" data-component="Icon" data-icon="home" aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="m3 11 9-8 9 8M5 10v11h14V10M10 21v-7h4v7"></path></svg>`:e}</a>`}</li>`,k=(e,t=``,n=`경로`)=>`<nav class="breadcrumb${t}" aria-label="${n}" data-component="Breadcrumb"><ol class="breadcrumb_list">${e}</ol></nav>`,A=e=>O(`홈`)+O(`가이드`)+O(e,{current:!0}),ke={basic:k(O(`홈`)+O(`컴포넌트`)+O(`Breadcrumb`,{current:!0})),separator:[k(A(`Chevron`),``,`경로 — chevron`),k(A(`Slash`),` breadcrumb_sep-slash`,`경로 — slash`),k(A(`Dot`),` breadcrumb_sep-dot`,`경로 — dot`)].join(`
...
`)},j=`{% from "components/navigation/Breadcrumb/breadcrumb.njk" import breadcrumb, breadcrumbItem, breadcrumbEllipsis %}`,Ae=e=>`{% call breadcrumb() %}
...
{% endcall %}`},Me={basic:`<Breadcrumb items={[{ label: '홈', href: '#' }, { label: '컴포넌트', href: '#' }, { label: 'Breadcrumb', current: true }]} />`,separator:`<Breadcrumb ariaLabel="경로 — chevron" items={items('Chevron')} />
...
<Breadcrumb separator="slash" aria-label="경로 — 긴 레이블"><BreadcrumbItem label="홈" href="#" /><BreadcrumbItem label="프로젝트 관리 및 협업 도구 설정" href="#" /><BreadcrumbItem label="사용자 권한 및 역할 기반 접근 제어" current /></Breadcrumb>`},Pe=Object.fromEntries(Oe.map(e=>[e,De(e)]))})))()}function Ie(e){let t=`import { useState } from 'react';\nimport Pagination from '@uxkm/react/pagination';\n\nexport function Example() { ${Ue(e)} return <>${Ve[e]}</>; }`,n=`<script setup>\nimport { ref } from 'vue';\nimport Pagination from '@uxkm/vue/pagination';\n${Ge(e)}\n<\/script>\n<template>\n${We[e]}\n</template>`;return[{id:`html`,label:`HTML`,fileName:`Pagination.html · ${e}`,code:ze[e]},{id:`gulp`,label:`Gulp`,fileName:`pagination.njk · ${e}`,code:Be[e]},{id:`vue`,label:`Vue`,fileName:`@uxkm/vue/pagination · ${e}`,code:n},{id:`nuxt`,label:`Nuxt`,fileName:`@uxkm/vue/pagination · ${e}`,code:n},{id:`react`,label:`React`,fileName:`@uxkm/react/pagination · ${e}`,code:t},{id:`next`,label:`Next`,fileName:`@uxkm/react/pagination · ${e}`,code:t}]}var Le,M,N,Re,ze,P,Be,Ve,He,Ue,We,Ge,Ke;function qe(){return(qe=e((()=>{Le=[`basic`,`minimal`,`size`,`round`,`ellipsis`,`simple`,`toolbar`,`align`,`state`],M=e=>`<svg class="icon icon_sm pagination_icon" data-component="Icon" data-icon="chevron-${e}" aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="${e===`left`?`m15 6-6 6 6 6`:`m9 6 6 6-6 6`}"></path></svg>`,N=(e,t,n,r=``)=>`<nav class="pagination${r}" aria-label="페이지 이동" data-component="Pagination" data-pagination data-current="${e}" data-total="${t}" data-page-size="${n}"><ul class="pagination_list" data-pagination-list><li class="pagination_item"><button type="button" class="btn btn_ghost color_default btn_sm btn_icon-only pagination_btn pagination_prev" data-pagination-prev aria-label="이전 페이지">${M(`left`)}</button></li><li class="pagination_item" data-pagination-next-item><button type="button" class="btn btn_ghost color_default btn_sm btn_icon-only pagination_btn pagination_next" data-pagination-next aria-label="다음 페이지">${M(`right`)}</button></li></ul></nav>`,Re=(e,t,n,r=``)=>`<nav class="pagination pagination_simple${r}" aria-label="페이지 이동" data-component="Pagination" data-pagination data-current="${e}" data-total="${t}" data-page-size="${n}"><button type="button" class="btn btn_ghost color_default btn_sm btn_icon-only pagination_btn pagination_prev" data-pagination-prev aria-label="이전 페이지">${M(`left`)}</button><span class="pagination_simple-text" aria-live="polite"><span class="pagination_simple-current">${e}</span> / <span class="pagination_simple-total">${Math.ceil(t/n)}</span></span><button type="button" class="btn btn_ghost color_default btn_sm btn_icon-only pagination_btn pagination_next" data-pagination-next aria-label="다음 페이지">${M(`right`)}</button></nav>`,ze={basic:N(1,50,10),minimal:N(4,50,10,` pagination_minimal`),size:[N(1,20,10,` pagination_sm`),N(1,20,10),N(1,20,10,` pagination_lg`)].join(`

`),round:N(7,80,10,` pagination_round`),ellipsis:N(10,500,10),simple:[Re(3,120,10),Re(1,120,10,` pagination_minimal`)].join(`
...
{% endcall %}`,align:`${P}\n\n{{ pagination(current=1, total=20, pageSize=10) }}\n\n{{ pagination(current=1, total=20, pageSize=10, align='center') }}\n\n{{ pagination(current=1, total=20, pageSize=10, align='end') }}`,state:`${P}\n\n{{ pagination(current=1, total=30, pageSize=10) }}\n\n{{ pagination(current=10, total=100, pageSize=10) }}`},Ve={basic:`<Pagination current={current} total={50} pageSize={10} onChange={setCurrent} />`,minimal:`<Pagination current={current} total={50} pageSize={10} minimal onChange={setCurrent} />`,size:`<Pagination current={small} total={20} pageSize={10} size="sm" onChange={setSmall} />
...
<Pagination current={last} total={100} pageSize={10} onChange={setLast} />`},He={basic:`1`,minimal:`4`,size:`1`,round:`7`,ellipsis:`10`,simple:`3`,toolbar:`5`,align:`1`,state:`1`},Ue=e=>e===`size`?`const [small, setSmall] = useState(1); const [medium, setMedium] = useState(1); const [large, setLarge] = useState(1);`:e===`simple`?`const [current, setCurrent] = useState(3); const [minimal, setMinimal] = useState(1);`:e===`align`?`const [start, setStart] = useState(1); const [center, setCenter] = useState(1); const [end, setEnd] = useState(1);`:e===`state`?`const [first, setFirst] = useState(1); const [last, setLast] = useState(10);`:`const [current, setCurrent] = useState(${He[e]});`,We={basic:`<Pagination v-model:current="current" :total="50" :page-size="10" />`,minimal:`<Pagination v-model:current="current" :total="50" :page-size="10" minimal />`,size:`<Pagination v-model:current="small" :total="20" :page-size="10" size="sm" />
...
<Pagination v-model:current="last" :total="100" :page-size="10" />`},Ge=e=>e===`size`?`const small = ref(1); const medium = ref(1); const large = ref(1);`:e===`simple`?`const current = ref(3); const minimalPage = ref(1);`:e===`align`?`const start = ref(1); const center = ref(1); const end = ref(1);`:e===`state`?`const first = ref(1); const last = ref(10);`:`const current = ref(${He[e]});`,Ke=Object.fromEntries(Le.map(e=>[e,Ie(e)]))})))()}function Je(e){let t=e===`icon`,n=e===`navigable`,r=`${n?`import { useState } from 'react';
...
const current = ref(2);`:``}\n<\/script>\n<template>\n${V[e]}\n</template>`;return[{id:`html`,label:`HTML`,fileName:`Steps.html · ${e}`,code:Ze[e]},{id:`gulp`,label:`Gulp`,fileName:`steps.njk · ${e}`,code:$e[e]},{id:`vue`,label:`Vue`,fileName:`@uxkm/vue/steps · ${e}`,code:i},{id:`nuxt`,label:`Nuxt`,fileName:`@uxkm/vue/steps · ${e}`,code:i},{id:`react`,label:`React`,fileName:`@uxkm/react/steps · ${e}`,code:r},{id:`next`,label:`Next`,fileName:`@uxkm/react/steps · ${e}`,code:r}]}var Ye,F,Xe,I,Ze,Qe,L,$e,R,z,et,B,tt,V,nt;function rt(){return(rt=e((()=>{Ye=[`basic`,`simple`,`vertical`,`dot`,`icon`,`navigable`,`error`,`align`,`size`,`state`],F={basic:[[`장바구니`,`상품 확인`,`finished`],[`배송 정보`,`주소 입력`,`finished`],[`결제`,`결제 수단 선택`,`active`],[`완료`,`주문 확인`,`wait`]],simple:[[`이메일 인증`,``,`finished`],[`프로필 설정`,``,`active`],[`완료`,``,`wait`]],vertical:[[`기본 정보`,`프로젝트 이름과 설명을 입력합니다.`,`finished`],[`팀 구성`,`멤버를 초대하고 역할을 지정합니다.`,`active`],[`저장소 연결`,`Git 저장소를 연동합니다.`,`wait`],[`검토 및 생성`,`설정을 확인하고 프로젝트를 만듭니다.`,`wait`]],dot:[[`주문 접수`,`6월 20일 09:12`,`finished`],[`배송 준비`,`6월 20일 14:30`,`finished`],[`배송 중`,`예상 도착 6월 21일`,`active`],[`배송 완료`,`대기 중`,`wait`]],icon:[[`계정 생성`,`이메일 인증 완료`,`finished`,`user`],[`팀 초대`,`동료를 초대하세요`,`active`,`mail`],[`첫 프로젝트`,`프로젝트를 만드세요`,`wait`,`grid`]],navigable:[[`환경`,`개발 · 스테이징`,`finished`],[`알림`,`수신 채널 설정`,`active`],[`확인`,`설정 검토`,`wait`]],error:[[`주문 확인`,`상품 및 수량 확인`,`finished`],[`결제 실패`,`카드 승인이 거절되었습니다`,`error`],[`완료`,`주문 접수`,`wait`]],align:[[`기획`,`요구사항 정의`,`finished`],[`개발`,`기능 구현`,`finished`],[`QA`,`품질 검증`,`active`],[`출시`,`프로덕션 배포`,`wait`]],size:[[`1단계`,``,`finished`],[`2단계`,``,`active`],[`3단계`,``,`wait`]],state:[[`is-finished`,`완료된 단계`,`finished`],[`is-active`,`현재 진행 단계`,`active`],[`is-wait`,`아직 도달하지 않은 단계`,`wait`],[`is-error`,`오류가 발생한 단계`,`error`]]},Xe=e=>`<svg class="icon steps_icon" data-component="Icon" data-icon="${e}" aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="${e===`check`?`m5 12 4 4L19 6`:e===`close`?`m6 6 12 12M18 6 6 18`:e===`user`?`M12 4a4 4 0 1 0 0 8 4 4 0 0 0 0-8M4 21a8 8 0 0 1 16 0`:e===`mail`?`M3 5h18v14H3zM3 7l9 6 9-6`:`M4 4h6v6H4zM14 4h6v6h-6zM4 14h6v6H4zM14 14h6v6h-6z`}"></path></svg>`,I=(e,t=``,n=`진행 단계`,r=!1)=>`<ol class="steps${t}${r?` steps_navigable`:``}" aria-label="${n}" data-component="Steps" data-steps>${e.map(([t,n,i,a],o)=>{let s=`<span class="steps_head"><span class="steps_indicator" aria-hidden="true">${a?Xe(a):i===`finished`?Xe(`check`):i===`error`?Xe(`close`):`<span class="steps_index">${o+1}</span>`}</span>${o<e.length-1?`<span class="steps_tail" aria-hidden="true"></span>`:``}</span>`,c=`<span class="steps_content"><span class="steps_title">${t}</span>${n?`<span class="steps_desc">${n}</span>`:``}</span>`;return`<li class="steps_item is-${i}" data-step-index="${o+1}"${i===`active`&&!r?` aria-current="step"`:``}>${r?`<button type="button" class="steps_trigger"${i===`wait`?` disabled`:``}${i===`active`?` aria-current="step"`:``}>${s}${c}</button>`:`${s}${c}`}</li>`}).join(``)}</ol>`,Ze={basic:I(F.basic,``,`주문 진행 단계`),simple:I(F.simple,``,`회원가입 단계`),vertical:I(F.vertical,` steps_vertical steps_demo-narrow`,`프로젝트 생성 단계`),dot:I(F.dot,` steps_dot`,`배송 추적 단계`),icon:I(F.icon,` steps_icon-style`,`온보딩 단계`),navigable:I(F.navigable,``,`설정 마법사 — 탐색 가능`,!0),error:I(F.error,``,`결제 단계 — 오류`),align:I(F.align,` steps_align-center`,`출시 단계`),size:`${I(F.size,` steps_sm`,`Small 단계`)}\n${I([[`요구사항`,`범위 및 일정 확정`,`finished`],[`디자인`,`UI 키트 제작`,`active`],[`개발`,`구현 및 테스트`,`wait`]],` steps_lg`,`Large 단계`)}`,state:F.state.map(e=>I([e],``,`상태 예시 — ${e[0]}`)).join(`
`)},Qe=`{% from "components/navigation/Steps/steps.njk" import steps, step %}`,L=(e,t={})=>{let n=[t.direction&&`direction='${t.direction}'`,t.size&&`size='${t.size}'`,t.dot&&`dot=true`,t.iconStyle&&`iconStyle=true`,t.navigable&&`navigable=true`,t.align&&`align='${t.align}'`,t.ariaLabel&&`ariaLabel='${t.ariaLabel}'`,t.className&&`className='${t.className}'`].filter(Boolean).join(`, `),r=e.map(([n,r,i,a],o)=>`  {{ step(title='${n}'${r?`, description='${r}'`:``}, status='${i}', index=${o+1}${o===e.length-1?`, last=true`:``}${t.navigable?`, navigable=true`:``}${a?`, icon='${a}'`:``}) }}`).join(`
`);return`${Qe}

{% call steps(${n}) %}
${r}
{% endcall %}`},$e={basic:L(F.basic,{ariaLabel:`주문 진행 단계`}),simple:L(F.simple,{ariaLabel:`회원가입 단계`}),vertical:L(F.vertical,{direction:`vertical`,className:`steps_demo-narrow`,ariaLabel:`프로젝트 생성 단계`}),dot:L(F.dot,{dot:!0,ariaLabel:`배송 추적 단계`}),icon:L(F.icon,{iconStyle:!0,ariaLabel:`온보딩 단계`}),navigable:L(F.navigable,{navigable:!0,ariaLabel:`설정 마법사 — 탐색 가능`}),error:L(F.error,{ariaLabel:`결제 단계 — 오류`}),align:L(F.align,{align:`center`,ariaLabel:`출시 단계`}),size:L(F.size,{size:`sm`,ariaLabel:`Small 단계`})+`
...
`)},R=(e,t=!1)=>e.map(([e,n,r,i])=>`<StepsItem title="${e}"${n?` description="${n}"`:``} status="${r}"${t&&i?` icon={<Icon name="${i}" className="steps_icon" />}`:``} />`).join(``),z=(e,t=!1)=>e.map(([e,n,r,i])=>t&&i?`<StepsItem title="${e}"${n?` description="${n}"`:``} status="${r}"><template #icon><Icon name="${i}" class="steps_icon" /></template></StepsItem>`:`<StepsItem title="${e}"${n?` description="${n}"`:``} status="${r}" />`).join(``),et={basic:`ariaLabel="주문 진행 단계"`,simple:`ariaLabel="회원가입 단계"`,vertical:`direction="vertical" className="steps_demo-narrow" ariaLabel="프로젝트 생성 단계"`,dot:`dot ariaLabel="배송 추적 단계"`,icon:`iconStyle ariaLabel="온보딩 단계"`,navigable:`current={current} navigable onChange={setCurrent} ariaLabel="설정 마법사 — 탐색 가능"`,error:`ariaLabel="결제 단계 — 오류"`,align:`align="center" ariaLabel="출시 단계"`,size:`size="sm" ariaLabel="Small 단계"`,state:`ariaLabel="상태 요약"`},B=Object.fromEntries(Ye.map(e=>[e,`<Steps ${et[e]}>${R(F[e],e===`icon`)}</Steps>`])),B.navigable=`<Steps current={current} navigable onChange={setCurrent} ariaLabel="설정 마법사 — 탐색 가능"><StepsItem title="환경" description="개발 · 스테이징" /><StepsItem title="알림" description="수신 채널 설정" /><StepsItem title="확인" description="설정 검토" /></Steps>`,B.size=`<Steps size="sm" ariaLabel="Small 단계">${R(F.size)}</Steps>\n<Steps size="lg" ariaLabel="Large 단계">${R([[`요구사항`,`범위 및 일정 확정`,`finished`],[`디자인`,`UI 키트 제작`,`active`],[`개발`,`구현 및 테스트`,`wait`]])}</Steps>`,B.state=F.state.map(e=>`<Steps ariaLabel="상태 예시 — ${e[0]}">${R([e])}</Steps>`).join(`
`),tt=e=>e.replaceAll(`ariaLabel=`,`aria-label=`).replaceAll(`className=`,`class=`).replace(`iconStyle`,`icon-style`),V=Object.fromEntries(Ye.map(e=>[e,`<Steps ${tt(et[e].replace(`current={current} navigable onChange={setCurrent}`,`v-model:current="current" navigable`))}>${z(F[e],e===`icon`)}</Steps>`])),V.navigable=`<Steps v-model:current="current" navigable aria-label="설정 마법사 — 탐색 가능"><StepsItem title="환경" description="개발 · 스테이징" /><StepsItem title="알림" description="수신 채널 설정" /><StepsItem title="확인" description="설정 검토" /></Steps>`,V.size=`<Steps size="sm" aria-label="Small 단계">${z(F.size)}</Steps>\n<Steps size="lg" aria-label="Large 단계">${z([[`요구사항`,`범위 및 일정 확정`,`finished`],[`디자인`,`UI 키트 제작`,`active`],[`개발`,`구현 및 테스트`,`wait`]])}</Steps>`,V.state=F.state.map(e=>`<Steps aria-label="상태 예시 — ${e[0]}">${z([e])}</Steps>`).join(`
`),nt=Object.fromEntries(Ye.map(e=>[e,Je(e)]))})))()}function it(e){let t=e===`select`,n=`${t?`import { useState } from 'react';
...
import Badge from '@uxkm/react/badge';`:``}\n\nexport function Example(){ ${t?`const [region,setRegion]=useState('서울특별시'); const [sort,setSort]=useState('최신순'); `:``}return <>${lt[e]}</>; }`,r=`<script setup>\n${t?`import { ref } from 'vue';
...
{% call dropdown(${i}) %}
  ${n}
{% endcall %}`},ct={basic:K(`actions-menu`,`작업`,G([`복사`,`이동`,`공유`])),open:K(`category-menu`,`카테고리`,G([`전체`,`디자인`,`개발`,`마케팅`],`전체`),{open:!0,variant:`filled`,color:`primary`}),trigger:[K(`filled-menu`,`Filled`,G([`항목 1`,`항목 2`]),{variant:`filled`,color:`primary`}),K(`outline-menu`,`Outline`,G([`항목 1`,`항목 2`])),K(`ghost-menu`,`Ghost`,G([`항목 1`,`항목 2`]),{variant:`ghost`}),K(`text-menu`,`Text`,G([`항목 1`,`항목 2`]),{variant:`text`,color:`primary`}),K(`more-menu`,`더보기`,G([`편집`,`복제`,`삭제`]),{variant:`ghost`,iconOnly:!0,icon:`menu`})].join(`
...
`})))()}var ht;function gt(){return(gt=e((()=>{ht=`{# Navbar 원본 구현: 브랜드·메뉴·검색·액션·반응형 토글을 호출형 macro로 구성합니다. #}
...
`})))()}var kt;function At(){return(At=e((()=>{kt=`<!-- Menu 원본 구현: 현재 항목과 열림 상태를 관리하고 키보드 탐색, 링크, 접근성 속성을 연결합니다. -->
...
`})))()}var jt;function Mt(){return(Mt=e((()=>{jt=`{# Menu 원본 구현: 메뉴·항목·서브메뉴·그룹·구분선을 호출형 macro로 구성합니다. #}
{% import "components/basic/Icon/icon.njk" as Icon %}
{% import "components/basic/Button/button.njk" as Button %}
...
`})))()}var $t;function en(){return(en=e((()=>{$t=`<!-- Tabs 원본 구현: 현재 항목과 열림 상태를 관리하고 키보드 탐색, 링크, 접근성 속성을 연결합니다. -->
...
`})))()}var tn;function nn(){return(nn=e((()=>{tn=`{# Tabs 원본 구현: 탭·패널·스크롤 제어를 호출형 macro로 구성합니다. #}
{% import "components/basic/Icon/icon.njk" as Icon %}
{% import "components/basic/Button/button.njk" as Button %}
...
`})))()}var bn;function xn(){return(xn=e((()=>{bn=`<!-- Breadcrumb 원본 구현: 현재 항목과 열림 상태를 관리하고 키보드 탐색, 링크, 접근성 속성을 연결합니다. -->
...
`})))()}var Sn;function Cn(){return(Cn=e((()=>{Sn=`{# Breadcrumb 원본 구현: 경로·항목·말줄임을 호출형 macro로 구성합니다. #}
{% import "components/basic/Icon/icon.njk" as Icon %}
...
`})))()}var In;function Ln(){return(Ln=e((()=>{In=`<!-- Pagination 원본 구현: 현재 항목과 열림 상태를 관리하고 키보드 탐색, 링크, 접근성 속성을 연결합니다. -->
...
`})))()}var Rn;function zn(){return(zn=e((()=>{Rn=`{# Pagination 원본 구현: 페이지 이동·툴바·점프 입력을 호출형 macro로 구성합니다. #}
{% import "components/basic/Button/button.njk" as Button %}
...
`})))()}var Bn;function Vn(){return(Vn=e((()=>{Bn=`/**
 * Pagination 원본 구현.
 * 현재 항목과 열림 상태를 관리하고 키보드 탐색, 링크, 접근성 속성을 연결합니다.
 */
...
`})))()}var Wn;function Gn(){return(Gn=e((()=>{Wn=`<!-- Steps 원본 구현: 현재 항목과 열림 상태를 관리하고 키보드 탐색, 링크, 접근성 속성을 연결합니다. -->
...
`})))()}var Kn;function qn(){return(qn=e((()=>{Kn=`{# Steps 원본 구현: 단계 목록과 개별 단계를 호출형 macro로 구성합니다. #}
{% import "components/basic/Icon/icon.njk" as Icon %}
...
{% endcall %}
`})))()}var Jn;function Yn(){return(Yn=e((()=>{Jn=`/**
 * Steps 원본 구현.
 * 현재 항목과 열림 상태를 관리하고 키보드 탐색, 링크, 접근성 속성을 연결합니다.
 */
...
`})))()}var nr;function rr(){return(rr=e((()=>{nr=`<!-- Dropdown 원본 구현: 현재 항목과 열림 상태를 관리하고 키보드 탐색, 링크, 접근성 속성을 연결합니다. -->
...
`})))()}var ir;function ar(){return(ar=e((()=>{ir=`{# Dropdown 원본 구현: 트리거와 메뉴 패널을 호출형 macro로 구성합니다. #}
{% import "components/basic/Button/button.njk" as Button %}
...
{% call dropdown(triggerLabel='작업') %}
  ...
  {% endcall %}
{% endcall %}
`})))()}var or;function sr(){return(sr=e((()=>{or=`/**
 * Dropdown 원본 구현.
 * 현재 항목과 열림 상태를 관리하고 키보드 탐색, 링크, 접근성 속성을 연결합니다.
 *
...
`,`
...
</w2:group>`}function pr(e){let t={basic:`tabs`,dynamic:`tabs`,indicator:`tabs tabs_indicator-slide`,equal:`tabs tabs_equal`,layoutScroll:`tabs tabs_scroll`,card:`tabs tabs_card`,pill:`tabs tabs_pill`,size:`tabs tabs_sm`,icon:`tabs`,badge:`tabs`,extra:`tabs`,scroll:`tabs tabs_scroll`,disabled:`tabs`,vertical:`tabs tabs_vertical`},n=(e,t,n=!1,r=!1)=>`<xf:trigger id="${e}" type="button" class="tabs_tab${n?` is-active`:``}" role="tab" aria-selected="${n}" aria-controls="${e}Panel"${r?` disabled="true"`:``} ev:onclick="scwin.tabs_select">
...
</w2:group>`}function mr(e){let t=e===`size`?` breadcrumb_sm`:``,n=(e,t,n=!1)=>n?`<w2:textbox id="${e}" class="breadcrumb_item is-current" tagname="span" aria-current="page" label="${t}"></w2:textbox>`:`<w2:textbox id="${e}" class="breadcrumb_item" tagname="a" href="#" label="${t}"></w2:textbox>`,r=`<w2:textbox id="bcSep" class="breadcrumb_separator" aria-hidden="true" label="/"></w2:textbox>`;return e===`ellipsis`?`<w2:group id="breadcrumbEllipsis" class="breadcrumb" tagname="nav" aria-label="경로">
...
</w2:group>`}function hr(e){let t={basic:`pagination`,minimal:`pagination pagination_minimal`,size:`pagination pagination_sm`,round:`pagination pagination_round`,ellipsis:`pagination`,simple:`pagination pagination_simple`,toolbar:`pagination`,align:`pagination pagination_center`,state:`pagination`},n=(e,t,n=!1,r=!1)=>`<xf:trigger id="${e}" type="button" class="pagination_item${n?` is-active`:``}"${n?` aria-current="page"`:``}${r?` disabled="true"`:``} ev:onclick="scwin.pagination_goto">
  <xf:label><![CDATA[${t}]]></xf:label>
  ...
  ${n(`pg${e}Next`,`다음`)}
</w2:group>`}function gr(e){let t={basic:`steps`,simple:`steps steps_simple`,vertical:`steps steps_vertical`,dot:`steps steps_dot`,icon:`steps`,navigable:`steps`,error:`steps`,align:`steps steps_center`,size:`steps steps_sm`,state:`steps`},n=(e,t,n=`wait`)=>`<w2:group id="${e}" class="steps_item is-${n}"${n===`current`?` aria-current="step"`:``}>
  ...
  ${n(`steps${e}3`,`완료`,r)}
</w2:group>`}function _r(e){let t=e===`open`||e===`example`?` is-open`:``;return`<w2:group id="dropdown${e}" class="dropdown${t}">
...
</w2:group>`}var $,vr,yr,br,xr,Sr,Cr,wr,Tr,Er,Dr,Or,kr,Ar,jr;function Mr(){return(Mr=e((()=>{ie(),he(),Ee(),Fe(),qe(),rt(),ft(),mt(),gt(),vt(),bt(),St(),wt(),Et(),Ot(),At(),Mt(),Pt(),It(),Rt(),Bt(),Ht(),Wt(),Kt(),Jt(),Xt(),Qt(),en(),nn(),an(),sn(),ln(),dn(),pn(),hn(),_n(),yn(),xn(),Cn(),Tn(),Dn(),kn(),jn(),Nn(),Fn(),Ln(),zn(),Vn(),Un(),Gn(),qn(),Yn(),Zn(),$n(),tr(),rr(),ar(),sr(),lr(),$=(...e)=>e.join(`

`),vr=Z(`Navbar`,`navbar`,{html:pt,gulp:ht,react:$(_t,`<!-- NavbarList.jsx -->\n${yt}`,`<!-- NavbarItem.jsx -->\n${xt}`),vue:$(Ct,`<!-- NavbarList.vue -->\n${Tt}`,`<!-- NavbarItem.vue -->\n${Dt}`)}),yr=Z(`Menu`,`menu`,{html:kt,gulp:jt,react:$(Nt,`<!-- MenuItem.jsx -->\n${Ft}`,`<!-- MenuSubmenu.jsx -->\n${Lt}`,`<!-- MenuGroup.jsx -->\n${zt}`,`<!-- MenuDivider.jsx -->\n${Vt}`),vue:$(Ut,`<!-- MenuItem.vue -->\n${Gt}`,`<!-- MenuSubmenu.vue -->\n${qt}`,`<!-- MenuGroup.vue -->\n${Yt}`,`<!-- MenuDivider.vue -->\n${Zt}`)}),br=Z(`Tabs`,`tabs`,{html:$t,gulp:tn,react:$(rn,`<!-- TabsTab.jsx -->\n${on}`,`<!-- TabPanel.jsx -->\n${cn}`,`<!-- TabMenu.jsx -->\n${un}`),vue:$(fn,`<!-- TabsTab.vue -->\n${mn}`,`<!-- TabPanel.vue -->\n${gn}`,`<!-- TabMenu.vue -->\n${vn}`)}),xr=Z(`Breadcrumb`,`breadcrumb`,{html:bn,gulp:Sn,react:$(wn,`<!-- BreadcrumbItem.jsx -->\n${En}`,`<!-- BreadcrumbEllipsis.jsx -->\n${On}`),vue:$(An,`<!-- BreadcrumbItem.vue -->\n${Mn}`,`<!-- BreadcrumbEllipsis.vue -->\n${Pn}`)}),Sr=Z(`Pagination`,`pagination`,{html:In,gulp:Rn,react:Bn,vue:Hn}),Cr=Z(`Steps`,`steps`,{html:Wn,gulp:Kn,react:$(Jn,`<!-- StepsItem.jsx -->\n${Xn}`),vue:$(Qn,`<!-- StepsItem.vue -->\n${er}`)}),wr=Z(`Dropdown`,`dropdown`,{html:nr,gulp:ir,react:or,vue:cr}),Tr=Q(`Navbar`,re,dr),Er=Q(`Menu`,me,fr),Dr=Q(`Tabs`,Te,pr),Or=Q(`Breadcrumb`,Pe,mr),kr=Q(`Pagination`,Ke,hr),Ar=Q(`Steps`,nt,gr),jr=Q(`Dropdown`,dt,_r)})))()}export{Mr as a,vr as c,kr as d,Cr as f,Dr as h,jr as i,Tr as l,br as m,Or as n,yr as o,Ar as p,wr as r,Er as s,xr as t,Sr as u};