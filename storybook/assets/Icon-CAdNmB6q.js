import{n as e}from"./rolldown-runtime-DkW27tQK.js";import{t}from"./jsx-runtime-DeHZSEgm.js";import{d as n,i as r,l as i}from"./blocks-52meEcN7.js";import{n as a,t as o}from"./FrameworkTabs-CzSpr2B2.js";import{a as s,c,d as l,f as u,i as ee,l as d,n as te,o as ne,p as re,r as f,s as p,t as ie,u as ae}from"./Icon.stories-C3q1qQ44.js";import{i as m,r as h}from"./react-ByN0pObn.js";import{n as g,t as _}from"./ComponentApiDocs-C7CctYcc.js";var v;function y(){return(y=e((()=>{v=`<!-- Icon 원본 구현: 시맨틱 마크업과 공통 CSS 클래스를 조합합니다. -->
...
`)}\n  </>\n  );\n}`}function D(e){let t={};return e.replace(/([\w-]+)(?:="([^"]*)")?/g,(e,n,r)=>(t[n]=r??!0,``)),t}function O(e){let t=D(e),n=String(t.name??`plus`),r=[`icon`,t.size&&t.size!==`md`&&`icon_${t.size}`,t.spin&&`icon_spin`],i=[t.color&&`color_${t.color}`,t.inline&&`icon_inline`],a=`<svg class="${r.filter(Boolean).join(` `)}" aria-hidden="true" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">${I[n]??I.plus}</svg>`;if(t.button)return`<button class="icon_button ${i.filter(Boolean).join(` `)}" type="button" aria-label="${t[`aria-label`]??n}">${a}</button>`;if(t.circle||t.square)return`<span class="${[t.circle?`icon_circle`:`icon_square`,...i,t.pulse&&`icon_pulse`,t.circle&&t.size===`sm`&&`icon_circle-sm`,t.circle&&t.size===`lg`&&`icon_circle-lg`].filter(Boolean).join(` `)}" aria-hidden="true">${a}</span>`;let o=t[`aria-label`],s=i.filter(Boolean).join(` `);return(s?a.replace(`class="`,`class="${s} `):a).replace(`aria-hidden="true"`,o?`role="img" aria-label="${o}"`:`aria-hidden="true"`)}function k(e,t){return e===`custom`?`<div class="icon_group">
  <svg class="icon" aria-hidden="true" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    <circle cx="11" cy="11" r="8"></circle>
    <path d="m21 21-4.35-4.35"></path>
  </svg>
...
</div>`:e===`gallery`?`<!-- common-icons 목록을 순회해 같은 구조를 반복합니다. -->\n<div class="icon_grid">\n  <div class="icon_grid-item">${O(`name="search" size="lg"`)}<span>search</span></div>\n  <!-- ... -->\n</div>`:t.replace(/<Icon\s*([^>]*)>([\s\S]*?)<\/Icon>/g,(e,t,n)=>{let r=D(t),i=r.name?String(r.name):void 0,a=`<svg class="icon" aria-hidden="true" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">${n.trim()}</svg>`;return r.circle||r.square?`<span class="${[r.circle?`icon_circle`:`icon_square`,r.color&&`color_${r.color}`,r.pulse&&`icon_pulse`,r.circle&&r.size===`sm`&&`icon_circle-sm`,r.circle&&r.size===`lg`&&`icon_circle-lg`].filter(Boolean).join(` `)}" aria-hidden="true">${a}</span>`:i?O(t):a}).replace(/<Icon\s+([^>]*?)\s*\/>/g,(e,t)=>O(t))}function A(e){let t=[],n=/:([\w-]+)="([^"]*)"|([\w-]+)(?:="([^"]*)")?/g,r;for(;r=n.exec(e);){if(r[1]){let e=L[r[1]]??r[1];t.push(`${e}=${r[2]}`);continue}let e=r[3],n=r[4],i=L[e]??e;n==null?t.push(`${i}=true`):/^-?\d+(\.\d+)?$/.test(n)?t.push(`${i}=${n}`):t.push(`${i}='${n}'`)}return t.join(`, `)}function j(e,t){let n=` `.repeat(t);return e.split(`
`).map(e=>e.trim()?`${n}${e}`:e).join(`
`)}function M(e,t){if(e===`gallery`)return`{% from "components/basic/Icon/icon.njk" import icon %}
...
`);return n=n.replace(/<Icon\s+([^>]*?)\s*\/>/g,(e,t)=>{let n=A(t.trim());return n?`{{ icon(${n}) }}`:`{{ icon() }}`}),n=n.replace(/<Icon\s*([^>]*)>([\s\S]*?)<\/Icon>/g,(e,t,n)=>{let r=A(t.trim()),i=j(n.trim(),2);return r?`{% call icon(${r}) %}\n${i}\n{% endcall %}`:`{% call icon() %}\n${i}\n{% endcall %}`}),`{% from "components/basic/Icon/icon.njk" import icon %}\n\n${n.trim()}`}function N(e,t,n=``){let r=D(e),i=String(r.name??(n||`plus`)),a=`/images/icons/${i}.svg`,o=r.size&&r.size!==`md`?`icon_${r.size}`:``,s=r.color?`color_${r.color}`:``,c=[`icon`,o,s,r.inline&&`icon_inline`,r.spin&&`icon_spin`].filter(Boolean).join(` `),l=String(r[`aria-label`]??i);if(r.button)return`<xf:trigger
  type="button"
  id="${t}"
  class="icon_button ${s}"
  title="${l}"
...
`).map(e=>`  ${e}`).join(`
`)}
</w2:group>`}function oe(e,t){let n=`${e[0].toUpperCase()}${e.slice(1)}`,r=0,i=0;if(e===`gallery`)return`<!-- 아이콘 경로는 프로젝트 정적 리소스 위치에 맞게 변경합니다. -->
...
`]}),`
`,(0,Q.jsx)(o,{examples:P,showFullCode:!0}),`
...
`,(0,Q.jsx)(o,{examples:R.background}),`
...
`,(0,Q.jsx)(o,{examples:R.gallery}),`
`,(0,Q.jsx)(t.h2,{id:`접근성`,children:`접근성`}),`
`,(0,Q.jsxs)(t.p,{children:[`장식용 아이콘은 `,(0,Q.jsx)(t.code,{children:`aria-hidden`}),`으로 숨기고, 의미가 있는 아이콘은 `,(0,Q.jsx)(t.code,{children:`aria-label`}),` 또는 `,(0,Q.jsx)(t.code,{children:`title`}),`로 이름을 제공합니다. `,(0,Q.jsx)(t.code,{children:`button`}),`·`,(0,Q.jsx)(t.code,{children:`circle`}),`·`,(0,Q.jsx)(t.code,{children:`square`}),` 래퍼에서는 내부 SVG를 `,(0,Q.jsx)(t.code,{children:`aria-hidden`}),`으로 두고 접근성 이름은 래퍼의 `,(0,Q.jsx)(t.code,{children:`aria-label`}),`에 둡니다. WebSquare의 `,(0,Q.jsx)(t.code,{children:`w2:image`}),`는 장식용이면 빈 `,(0,Q.jsx)(t.code,{children:`alt`}),`, 의미 있는 이미지에는 대체 텍스트를 지정합니다.`]}),`
`,(0,Q.jsx)(_,{sections:Y})]})}function ce(e={}){let{wrapper:t}={...m(),...e.components};return t?(0,Q.jsx)(t,{...e,children:(0,Q.jsx)(Z,{...e})}):Z(e)}var Q;function $(){return($=e((()=>{Q=t(),h(),n(),a(),g(),z(),X(),re()})))()}$();export{ce as default};