import{n as e}from"./rolldown-runtime-DkW27tQK.js";import{t}from"./jsx-runtime-DeHZSEgm.js";import{a as n,c as r,d as i,f as a,i as o,l as s,m as c,n as l,o as u,p as ee,r as d,s as f,t as te,u as ne}from"./Link.stories-Ck0joga7.js";import{i as p,r as m}from"./react-DL__92zz.js";import{c as h,r as g,u as _}from"./blocks-Cy0HofPr.js";import{i as v,n as re,r as y,t as ie}from"./ComponentApiDocs-Ci2QMeRq.js";var b;function x(){return(x=e((()=>{b=`<!-- Link 원본 구현: 시맨틱 마크업과 공통 CSS 클래스를 조합합니다. -->
...
`})))()}function O(e){let t={};return e.replace(/([\w-]+)(?:="([^"]*)")?/g,(e,n,r)=>(t[n]=r??!0,``)),t}function k(e){return`<svg class="link_icon" aria-hidden="true" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">${F[e]||F.external}</svg>`}function A(e){return[`link`,`color_${e.color||`primary`}`,e.size&&`size_${e.size}`,e.underline&&`link_underline`,e[`no-underline`]&&`link_no-underline`,e.standalone&&`link_standalone`,e.nav&&`link_nav`,e.block&&`link_block`,e.back&&`link_back`,e[`icon-only`]&&`link_icon-only`,e.active&&`is-active`,e.disabled&&`is-disabled`].filter(Boolean).join(` `)}function j(e){let t=O(e),n=String(t.as||`a`),r=A(t),i=t[`icon-only`]?``:String(t.label||``),a=t.icon?k(String(t.icon)):``,o=t[`icon-after`]?k(String(t[`icon-after`])):``;return`<${n} ${[`class="${r}"`,`data-ripple="true"`,n===`a`?`href="${t.href||`#`}"`:`type="button"`,t.target?`target="${t.target}"`:``,t.rel?`rel="${t.rel}"`:``,t[`aria-label`]?`aria-label="${t[`aria-label`]}"`:``,t.active?`aria-current="page"`:``,t.disabled?`aria-disabled="true" tabindex="-1"`:``,n===`button`&&t.disabled?`disabled`:``].filter(Boolean).join(` `)}>${a}${i}${o}</${n}>`}function ae(e){return e.replace(/<Link\s+([^>]*?)\s*\/>/g,(e,t)=>j(t))}function oe(e){let t=[],n=/:([\w-]+)="([^"]*)"|([\w-]+)(?:="([^"]*)")?/g,r;for(;r=n.exec(e);){if(r[1]){let e=I[r[1]]??r[1];t.push(`${e}=${r[2]}`);continue}let e=r[3],n=r[4],i=I[e]??e;n==null?t.push(`${i}=true`):/^-?\d+(\.\d+)?$/.test(n)?t.push(`${i}=${n}`):t.push(`${i}='${n}'`)}return t.join(`, `)}function se(e){let t=e.replace(/<Link\s+([^>]*?)\s*\/>/g,(e,t)=>{let n=oe(t.trim());return n?`{{ link(${n}) }}`:`{{ link() }}`});return t=t.replace(/(<(?:div|nav|section|ul|ol|aside|header|main|footer)\b[^>]*)>\{\{/gi,`$1>
  {{`).replace(/\}\}\{\{/g,`}}
  {{`).replace(/\}\}\s*(<\/(?:div|nav|section|ul|ol|aside|header|main|footer)>)/gi,`}}
$1`).replace(/\n{3,}/g,`

`),`{% from "components/basic/Link/link.njk" import link %}\n\n${t.trim()}`}function ce(e){return e.replace(/class=/g,`className=`).replace(/<Link\s+([^>]*?)\s*\/>/g,(e,t)=>{let n=O(t),r=Object.entries(n).filter(([e])=>e!==`icon`&&e!==`icon-after`).map(([e,t])=>{let n=e.replace(/-([a-z])/g,(e,t)=>t.toUpperCase());return t===!0||P.has(e)&&t===`true`?n:`${n}="${t}"`});return n.icon&&r.push(`icon={<Icon className="link_icon" name="${n.icon}" />}`),n[`icon-after`]&&r.push(`iconAfter={<Icon className="link_icon" name="${n[`icon-after`]}" />}`),`<Link ${r.join(` `)} />`})}function le(e){return e.replace(/<Link\s+([^>]*?)\s*\/>/g,(e,t)=>{let n=O(t),r=n.icon,i=n[`icon-after`],a=Object.entries(n).filter(([e])=>e!==`icon`&&e!==`icon-after`).map(([e,t])=>t===!0||P.has(e)&&t===`true`?e:`${e}="${t}"`).join(` `);return!r&&!i?`<Link ${a} />`:[`<Link ${a}>`,r?`  <template #icon><Icon class="link_icon" name="${r}" /></template>`:``,i?`  <template #icon-after><Icon class="link_icon" name="${i}" /></template>`:``,`</Link>`].filter(Boolean).join(`
`)})}function ue(e,t){let n=`${t[0].toUpperCase()}${t.slice(1)}`,r=0,i=0,a=e.replace(/<Link\s+([^>]*?)\s*\/>/g,(e,t)=>{let i=O(t),a=`link${n}${++r}`,o=A(i),s=String(i.label??i[`aria-label`]??`링크`),c=i.icon??i[`icon-after`],l=c?`<!-- 아이콘 ${c}: 프로젝트 이미지 또는 CSS 리소스에 연결 -->\n`:``;if(i.as===`button`)return`${l}<xf:trigger\n  type="button"\n  id="${a}"\n  class="${o}"${i.disabled?`
  disabled="true"`:`
...
`).map(e=>`  ${e}`).join(`
`)}
</w2:group>`}function de(e){let t=N[e],n=ae(t),r=se(t),i=ce(t),a=le(t);return[{id:`html`,label:`HTML`,fileName:`apps/html/src/components/basic/Link/Link.html · ${e}`,code:n},{id:`gulp`,label:`Gulp`,fileName:`apps/gulp/src/components/basic/Link/link.njk · ${e}`,code:`{# Link · ${e} #}\n${r}`},{id:`vue`,label:`Vue`,fileName:`@uxkm/vue/link → apps/vue/src/components/basic/Link/Link.vue · ${e}`,code:`<script setup>\nimport Link from '@uxkm/vue/link';\nimport Icon from '@uxkm/vue/icon';\n<\/script>\n\n<template>\n${a.split(`
`).map(e=>`  ${e}`).join(`
...
`,(0,Q.jsx)(y,{examples:L.navigation}),`
...
`,(0,Q.jsx)(y,{examples:L.context}),`
`,(0,Q.jsx)(ie,{sections:Y})]})}function fe(e={}){let{wrapper:t}={...p(),...e.components};return t?(0,Q.jsx)(t,{...e,children:(0,Q.jsx)(Z,{...e})}):Z(e)}var Q;function $(){return($=e((()=>{Q=t(),m(),_(),v(),re(),R(),X(),c()})))()}$();export{fe as default};