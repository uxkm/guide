...
`,(0,u.jsx)(s,{columns:[`대상`,`구현 방식`,`주요 역할`],rows:[[`HTML`,`시맨틱 HTML + 공통 동작 JS`,`마크업 기준과 직접 사용`],[`Gulp`,`Nunjucks 템플릿`,`정적 사이트 조립과 재사용`],[`Vue`,`Vue 3 SFC`,`반응형 상태와 Vue API`],[`React`,`React 컴포넌트`,`props 기반 합성과 상태`],[`Nuxt`,`Vue 구현 재사용`,`NuxtLink, NuxtImage, 앱 레이아웃 통합`],[`Next.js`,`React 구현 재사용`,`Link, Image, App Router 통합`],[`WebSquare`,`XML 호환 XHTML 조각`,`WebSquare 화면 XML에서 공통 클래스 재사용`]]}),`
