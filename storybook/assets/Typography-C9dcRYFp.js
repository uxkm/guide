import{n as e}from"./rolldown-runtime-DkW27tQK.js";import{t}from"./jsx-runtime-DeHZSEgm.js";import{_ as n,a as r,c as i,d as a,f as o,g as s,h as c,i as l,l as ee,m as te,n as ne,o as re,p as u,r as d,s as f,t as p,u as m}from"./Typography.stories-Cz8mN6Ce.js";import{i as h,r as ie}from"./react-pLZlIR__.js";import{c as ae,r as g,u as _}from"./blocks-B4JiLJlK.js";import{i as oe,n as se,r as v,t as y}from"./ComponentApiDocs-DwOtyIDn.js";var b;function x(){return(x=e((()=>{b=`<!-- Typography 원본 구현: 시맨틱 마크업과 공통 CSS 클래스를 조합합니다. -->
...
<p class="text_ellipsis" data-component="TypoText">한 줄로 말줄임되는 긴 본문입니다.</p>`})))()}var S;function C(){return(C=e((()=>{S=`{# Typography 원본 구현: 시맨틱 루트와 공통 CSS 클래스를 조합합니다. #}
...
`})))()}function O(e){let t={};return e.replace(/([\w-]+)="([^"]*)"/g,(e,n,r)=>(t[n]=r,``)),t}function ce(e,t){let n=O(e),r=n.variant||`text`,i=N[r]||N.text,a=n.tag||i.tag,o=Number(n.ellipsis),s=[i.className,n.color&&`color_${n.color}`,n.size&&`size_${n.size}`,o===1?`text_ellipsis`:[2,3].includes(o)?`text_ellipsis-${o}`:``].filter(Boolean),c=[s.length?`class="${s.join(` `)}"`:``,r===`link`?`href="${n.href||`#`}"`:``,r===`label`&&n[`html-for`]?`for="${n[`html-for`]}"`:``,r===`blockquote`&&n.cite?`cite="${n.cite}"`:``].filter(Boolean).join(` `);return`<${a}${c?` ${c}`:``}>${t}</${a}>`}function le(e){let t=e.replace(/<TypoTitle\s*([^>]*)>([\s\S]*?)<\/TypoTitle>/g,(e,t,n)=>{let r=O(t),i=[1,2,3,4,5].includes(Number(r.level))?Number(r.level):1;return`<h${i} class="${[`typo_title-${i}`,r.color&&`color_${r.color}`].filter(Boolean).join(` `)}">${n}</h${i}>`});return t=t.replace(/<TypoText\s*([^>]*)>([\s\S]*?)<\/TypoText>/g,(e,t,n)=>ce(t,n)),t}function ue(e){let t=[],n=/:([\w-]+)="([^"]*)"|([\w-]+)(?:="([^"]*)")?/g,r;for(;r=n.exec(e);){if(r[1]){let e=P[r[1]]??r[1];t.push(`${e}=${r[2]}`);continue}let e=r[3],n=r[4],i=P[e]??e;n==null?t.push(`${i}=true`):/^-?\d+(\.\d+)?$/.test(n)?t.push(`${i}=${n}`):t.push(`${i}='${n}'`)}return t.join(`, `)}function de(e,t){let n=` `.repeat(t);return e.split(`
`).map(e=>e.trim()?`${n}${e}`:e).join(`
`)}function fe(e){let t=e.trim();return!!t&&!t.includes(`
`)&&!t.includes(`'`)&&!/<\/?[A-Za-z]|[{%]/.test(t)}function k(e,t,n){let r=RegExp(`<${t}\\b([^>]*)>((?:(?!<\\/?Typo(?:Title|Text)\\b)[\\s\\S])*?)<\\/${t}>`,`g`);return e.replace(r,(e,t,r)=>{let i=ue(t.trim()),a=r.trim();if(fe(a))return`{{ ${n}(${i?`${i}, label='${a}'`:`label='${a}'`}) }}`;if(!a)return i?`{{ ${n}(${i}) }}`:`{{ ${n}() }}`;let o=de(a,2);return i?`{% call ${n}(${i}) %}\n${o}\n{% endcall %}`:`{% call ${n}() %}\n${o}\n{% endcall %}`})}function pe(e){let t=e.replace(/\r\n?/g,`
`),n=``;for(;n!==t;)n=t,t=k(t,`TypoTitle`,`typoTitle`),t=k(t,`TypoText`,`typoText`);return`{% from "components/basic/Typography/typography.njk" import typoTitle, typoText %}\n\n${t.trim()}`}function me(e){return`import { TypoText, TypoTitle } from '@uxkm/react/typography';\n\nexport function Example() {\n  return (\n  <>\n${e.replace(/class=/g,`className=`).replace(/html-for=/g,`htmlFor=`).replace(/style="max-width: 320px; width: 100%;"/g,`style={{ maxWidth: 320, width: '100%' }}`).split(`
`).map(e=>e.trim()?`    ${e.trimStart()}`:``).join(`
`)}\n  </>\n  );\n}`}function he(e){return`<script setup>\nimport { TypoText, TypoTitle } from '@uxkm/vue/typography';\n<\/script>\n\n<template>\n${e.split(`
`).map(e=>`  ${e}`).join(`
`)}\n</template>`}function A(e){return e.replace(/&(?!amp;|lt;|gt;|quot;|apos;|#\d+;|#x[\da-f]+;)/gi,`&amp;`).replace(/"/g,`&quot;`)}function ge(e,t){let n=`${t[0].toUpperCase()}${t.slice(1)}`,r=0,i=0;return`<w2:group
  id="typography${n}Example">
${e.split(`
`).map(e=>e.replace(RegExp(`^(\\s*)<(h[1-5]|p|span|strong|em|del|mark|sub|sup|small|code|kbd|pre|label|a)([^>]*)>([^<]*)</\\2>\\s*$`),(e,t,i,a,o)=>{let s=O(a),c=a.match(/\sclass="([^"]*)"/)?.[1]??``,l=`typo${n}${++r}`;return i===`a`?`<w2:anchor\n  id="${l}"\n  outerDiv="false"\n  href="${A(s.href||`#`)}"${c?`\n  class="${c}"`:``}>\n  <xf:label><![CDATA[${o}]]></xf:label>\n</w2:anchor>`.split(`
`).map(e=>`${t}${e}`).join(`
`):`<w2:textbox\n  ${[`id="${l}"`,`tagname="${i}"`,c?`class="${c}"`:``,i===`label`&&s.for?`for="${s.for}"`:``,`label="${A(o)}"`].filter(Boolean).join(`
  `)}></w2:textbox>`.split(`
`).map(e=>`${t}${e}`).join(`
`)})).join(`
...
`).map(e=>`  ${e}`).join(`
`)}
</w2:group>`}function _e(e){let t=M[e],n=le(t),r=pe(t),i=me(t),a=he(t);return[{id:`html`,label:`HTML`,fileName:`apps/html/src/components/basic/Typography/Typography.html · ${e}`,code:n},{id:`gulp`,label:`Gulp`,fileName:`apps/gulp/src/components/basic/Typography/typography.njk · ${e}`,code:`{# Typography · ${e} #}\n${r}`},{id:`vue`,label:`Vue`,fileName:`@uxkm/vue/typography → apps/vue/src/components/basic/Typography · ${e}`,code:a},{id:`nuxt`,label:`Nuxt`,fileName:`@uxkm/vue/typography → apps/vue/src/components/basic/Typography · ${e}`,code:a},{id:`react`,label:`React`,fileName:`@uxkm/react/typography → apps/react/src/components/basic/Typography · ${e}`,code:i},{id:`next`,label:`Next`,fileName:`@uxkm/react/typography → apps/react/src/components/basic/Typography · ${e}`,code:i},{id:`websquare`,label:`WebSquare`,fileName:`Typography.xml · ${e}`,code:ge(n,e)}]}var j,M,N,P,F;function I(){return(I=e((()=>{x(),C(),T(),D(),j=[{id:`html`,label:`HTML`,fileName:`apps/html/src/components/basic/Typography/Typography.html`,code:b},{id:`gulp`,label:`Gulp`,fileName:`apps/gulp/src/components/basic/Typography/typography.njk`,code:S},{id:`vue`,label:`Vue`,fileName:`apps/vue/src/components/basic/Typography/Typography.vue`,code:E},{id:`nuxt`,label:`Nuxt`,fileName:`@uxkm/vue/typography → Typography.vue`,code:E},{id:`react`,label:`React`,fileName:`apps/react/src/components/basic/Typography/Typography.jsx`,code:w},{id:`next`,label:`Next`,fileName:`@uxkm/react/typography → Typography.jsx`,code:w}],M={titles:`<div class="typo_stack-lg">
...
`]}),`
`,(0,Q.jsx)(v,{examples:j,showFullCode:!0}),`
...
`,(0,Q.jsx)(g,{of:m,sourceState:`none`}),`
`,(0,Q.jsx)(v,{examples:F.sizes}),`
...
`,(0,Q.jsx)(v,{examples:F.supporting}),`
...
`,(0,Q.jsx)(v,{examples:F.titleColors}),`
...
`,(0,Q.jsx)(v,{examples:F.nestedLists}),`
...
`,(0,Q.jsx)(v,{examples:F.stack}),`
`,(0,Q.jsx)(y,{sections:Y})]})}function ve(e={}){let{wrapper:t}={...h(),...e.components};return t?(0,Q.jsx)(t,{...e,children:(0,Q.jsx)(Z,{...e})}):Z(e)}var Q;function $(){return($=e((()=>{Q=t(),ie(),_(),oe(),se(),I(),X(),n()})))()}$();export{ve as default};