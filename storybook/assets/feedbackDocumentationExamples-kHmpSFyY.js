...
<template>
${l(t.vue,2)}
</template>`;return[{id:`html`,label:`HTML`,fileName:`Alert.html · ${e}`,code:t.html},{id:`gulp`,label:`Gulp`,fileName:`alert.njk · ${e}`,code:c[e]},{id:`vue`,label:`Vue`,fileName:`@uxkm/vue/alert · ${e}`,code:s},{id:`nuxt`,label:`Nuxt`,fileName:`@uxkm/vue/alert · ${e}`,code:s},{id:`react`,label:`React`,fileName:`@uxkm/react/alert · ${e}`,code:o},{id:`next`,label:`Next`,fileName:`@uxkm/react/alert · ${e}`,code:o}]}var r,i,a,o,s,c,l,u;function d(){return(d=e((()=>{r={info:`    <circle cx="12" cy="12" r="10"></circle>
...
</form>`}},a=`{% from "components/feedback/Alert/alert.njk" import alert %}`,o=`{% from "components/basic/Button/button.njk" import button %}`,s=`{% from "components/basic/Link/link.njk" import link %}`,c={basic:`${a}
...
{% endfor %}`,descriptionOnly:`${a}

{{ alert(description='새 기능이 추가되었습니다. 설정에서 확인해 보세요.') }}
{{ alert(color='success', role='status', description='프로필이 업데이트되었습니다.') }}`,noIcon:`${a}

{{ alert(color='warning', title='점검 안내', description='오늘 02:00~04:00 서비스 점검이 예정되어 있습니다.', showIcon=false) }}
{{ alert(description='이 페이지는 데모 목적으로만 사용됩니다.', showIcon=false) }}`,size:`${a}

{{ alert(size='sm', title='Small', description='폼 필드 아래 등 좁은 영역에 사용합니다.') }}
{{ alert(title='Medium', description='기본 크기입니다.') }}
{{ alert(size='lg', title='Large', description='페이지 상단 등 눈에 띄는 안내에 사용합니다.') }}`,closable:`${a}

{{ alert(title='새 소식', description='대시보드가 새롭게 개편되었습니다.', closable=true) }}
{{ alert(color='warning', description='브라우저가 오래되었습니다. 최신 버전으로 업데이트해 주세요.', closable=true) }}`,actions:`${a}
${o}
${s}
...
${s}
{% from "components/form/FormLayout/form-layout.njk" import formLayout, formField %}
...
{% call formLayout(fit=true, novalidate=true) %}
  ...
  {% endcall %}
{% endcall %}`},l=(e,t)=>e.split(`
`).map(e=>`${` `.repeat(t)}${e}`).join(`
`),u=Object.fromEntries(Object.entries(i).map(([e,t])=>[e,n(e,t)]))})))()}function f(e,t){let n=e===`action`,r=e===`placement`,i=[`import Snackbar from '@uxkm/react/snackbar';`,n&&`import Button from '@uxkm/react/button';`,r&&`import SnackbarRegion from '@uxkm/react/snackbar-region';`].filter(Boolean).join(`
...
`),o=`${i}\n\nexport function Example() {\n  return (\n${g(t.react,/^\s*<>/.test(t.react)?2:4)}\n  );\n}`,s=`<script setup>\n${a}\n<\/script>\n\n<template>\n${g(t.vue,2)}\n</template>`;return[{id:`html`,label:`HTML`,fileName:`Snackbar.html · ${e}`,code:t.html},{id:`gulp`,label:`Gulp`,fileName:`snackbar.njk · ${e}`,code:h[e]},{id:`vue`,label:`Vue`,fileName:`@uxkm/vue/snackbar · ${e}`,code:s},{id:`nuxt`,label:`Nuxt`,fileName:`@uxkm/vue/snackbar · ${e}`,code:s},{id:`react`,label:`React`,fileName:`@uxkm/react/snackbar · ${e}`,code:o},{id:`next`,label:`Next`,fileName:`@uxkm/react/snackbar · ${e}`,code:o}]}var p,m,h,g,ee;function te(){return(te=e((()=>{p={basic:{html:`<div class="snackbar color_info snackbar_motion-none is-open" role="status"><span class="snackbar_message">새로운 업데이트가 있습니다.</span></div>
...
{% endcall %}`},g=(e,t)=>e.split(`
`).map(e=>`${` `.repeat(t)}${e}`).join(`
`),ee=Object.fromEntries(Object.entries(p).map(([e,t])=>[e,f(e,t)]))})))()}function ne(e){switch(e){case`basic`:return`${y}

${x(`modal-basic`,`모달 열기`)}

${S()}
${C(`modal-basic`,`알림`,`<p>Modal 대화상자 본문입니다.</p>`,`, footer=footer`)}`;case`open`:return`${y}

${S()}
${C(`modal-open`,`알림`,`<p>로드 시 열린 상태로 표시되는 Modal입니다.</p>`,`, open=true, footer=footer`)}`;case`size`:return`${y}

${x(`modal-size-sm`,`Small`)}
${x(`modal-size-lg`,`Large`)}

${C(`modal-size-sm`,`Small`,`<p>좁은 너비</p>`,`, size='sm'`)}

${C(`modal-size-lg`,`Large`,`<p>넓은 너비</p>`,`, size='lg'`)}`;case`footer`:return`${y}
${b}

${x(`modal-footer`,`새 항목 추가`)}
...
{% endcall %}`;case`footerAlign`:return`${y}

${S()}
${C(`modal-align`,`푸터 정렬`,`<p>균등 정렬</p>`,`, open=true, footer=footer, footerAlign='even'`)}`;case`confirm`:return`${y}

${x(`modal-confirm`,`삭제`)}

${S(`삭제`)}
${C(`modal-confirm`,`항목 삭제`,`<p>이 작업은 되돌릴 수 없습니다.</p>`,`, size='sm', footer=footer`)}`;case`form`:return`${y}
${b}
...
{% endcall %}`;case`scroll`:return`${y}

${x(`modal-scroll`,`이용 약관 보기`)}

${C(`modal-scroll`,`이용 약관`,`<p>긴 본문…</p>`,`, scrollable=true`)}`;case`nested`:return`${y}

${x(`modal-parent`,`첫 번째 Modal 열기`)}
...
${C(`modal-child`,`두 번째 Modal`,`<p>최상위 대화상자</p>`,`, size='sm', style='--modal-stack-level: 1'`)}`;case`noBackdrop`:return`${y}

${x(`modal-plain`,`백드롭 없이 열기`)}

${C(`modal-plain`,`백드롭 없음`,`<p>배경을 가리지 않습니다.</p>`,`, size='sm', backdrop=false`)}`;default:return y}}function re(e,t){let n=e===`footer`||e===`form`,r=e===`size`?`  const [size, setSize] = useState(null);`:e===`nested`?`  const [parentOpen, setParentOpen] = useState(false);
...
import Input from '@uxkm/react/input';`:``}\n\nexport function Example() {\n${r}\n  return (\n${w(t.react,/^\s*<>/.test(t.react)?2:4)}\n  );\n}`,o=`<script setup>\nimport { ref } from 'vue';\nimport Modal from '@uxkm/vue/modal';\nimport Button from '@uxkm/vue/button';${n?`
import Input from '@uxkm/vue/input';`:``}\n${i}\n<\/script>\n\n<template>\n${w(t.vue,2)}\n</template>`;return[{id:`html`,label:`HTML`,fileName:`Modal.html · ${e}`,code:t.html},{id:`gulp`,label:`Gulp`,fileName:`modal.njk · ${e}`,code:ne(e)},{id:`vue`,label:`Vue`,fileName:`@uxkm/vue/modal · ${e}`,code:o},{id:`nuxt`,label:`Nuxt`,fileName:`@uxkm/vue/modal · ${e}`,code:o},{id:`react`,label:`React`,fileName:`@uxkm/react/modal · ${e}`,code:a},{id:`next`,label:`Next`,fileName:`@uxkm/react/modal · ${e}`,code:a}]}var _,v,y,b,x,S,C,ie,w,ae;function oe(){return(oe=e((()=>{_=(e,t)=>`<button type="button" class="btn btn_filled color_primary" data-modal-trigger="#${e}" aria-controls="${e}">${t}</button>`,v=(e,t,n,r=``,i=``)=>`<div id="${e}" class="modal${r}" data-modal ${i} role="dialog" aria-modal="true" aria-labelledby="${e}-title" tabindex="-1" hidden>
...
</div>`,y=`{% from "components/feedback/Modal/modal.njk" import modal %}
{% from "components/basic/Button/button.njk" import button %}`,b=`{% from "components/form/Input/input.njk" import input, inputField %}`,x=(e,t)=>`{{ button(
  label='${t}',
  dataAttributes={
    'data-modal-trigger': '#${e}',
    'aria-controls': '${e}',
    'aria-expanded': 'false'
  }
) }}`,S=(e=`확인`)=>`{% set footer %}
...
{% endset %}`,C=(e,t,n,r=``)=>`{% call modal(id='${e}', title='${t}'${r}) %}
  ${n}
{% endcall %}`,ie={basic:{html:`${_(`modal-basic`,`모달 열기`)}\n${v(`modal-basic`,`알림`,`<p>Modal 대화상자 본문입니다.</p>`)}`,react:`<><Button label="모달 열기" onClick={() => setOpen(true)} /><Modal open={open} title="알림" onClose={() => setOpen(false)}><p>Modal 대화상자 본문입니다.</p></Modal></>`,vue:`<Button label="모달 열기" @click="open = true" /><Modal :open="open" title="알림" @close="open = false"><p>Modal 대화상자 본문입니다.</p></Modal>`},open:{html:v(`modal-open`,`알림`,`<p>로드 시 열린 상태로 표시되는 Modal입니다.</p>`,` is-open`).replace(` hidden`,``),react:`<Modal open={open} title="알림" onClose={() => setOpen(false)} footer={<><Button variant="ghost" label="취소" /><Button label="확인" /></>}><p>로드 시 열린 상태로 표시되는 Modal입니다.</p></Modal>`,vue:`<Modal :open="open" title="알림" @close="open = false"><p>로드 시 열린 상태로 표시되는 Modal입니다.</p><template #footer><Button variant="ghost" label="취소" /><Button label="확인" /></template></Modal>`},size:{html:`${_(`modal-size-sm`,`Small`)}\n${_(`modal-size-lg`,`Large`)}\n${v(`modal-size-sm`,`Small`,`<p>좁은 너비</p>`,` modal_sm`)}\n${v(`modal-size-lg`,`Large`,`<p>넓은 너비</p>`,` modal_lg`)}`,react:`<><Button label="Small" onClick={() => setSize('sm')} /><Button label="Large" onClick={() => setSize('lg')} /><Modal open={Boolean(size)} size={size} title="크기" onClose={() => setSize(null)} /></>`,vue:`<Button label="Small" @click="size = 'sm'" /><Button label="Large" @click="size = 'lg'" /><Modal :open="Boolean(size)" :size="size" title="크기" @close="size = null" />`},footer:{html:`${_(`modal-footer`,`새 항목 추가`)}\n${v(`modal-footer`,`새 항목 추가`,`<label>이름 <input class="input"></label>`)}`,react:`<Modal open={open} title="새 항목 추가" footer={<><Button variant="ghost" label="취소" /><Button label="저장" /></>}><Input label="이름" /></Modal>`,vue:`<Modal :open="open" title="새 항목 추가"><Input label="이름" /><template #footer><Button variant="ghost" label="취소" /><Button label="저장" /></template></Modal>`},footerAlign:{html:v(`modal-align`,`푸터 정렬`,`<p>균등 정렬</p>`).replace(`modal_footer"`,`modal_footer modal_footer-even"`),react:`<Modal open footerAlign="even" footer={<><Button label="취소" /><Button label="확인" /></>} />`,vue:`<Modal open footer-align="even"><template #footer><Button label="취소" /><Button label="확인" /></template></Modal>`},confirm:{html:`${_(`modal-confirm`,`삭제`)}\n${v(`modal-confirm`,`항목 삭제`,`<p>이 작업은 되돌릴 수 없습니다.</p>`,` modal_sm`)}`,react:`<Modal open={open} size="sm" title="항목 삭제" onClose={() => setOpen(false)}><p>이 작업은 되돌릴 수 없습니다.</p></Modal>`,vue:`<Modal :open="open" size="sm" title="항목 삭제" @close="open = false"><p>이 작업은 되돌릴 수 없습니다.</p></Modal>`},form:{html:`${_(`modal-form`,`프로필 편집`)}\n${v(`modal-form`,`프로필 편집`,`<label>이름 <input class="input" value="홍길동"></label>`)}`,react:`<Modal open={open} title="프로필 편집"><Input label="이름" defaultValue="홍길동" /></Modal>`,vue:`<Modal :open="open" title="프로필 편집"><Input label="이름" model-value="홍길동" /></Modal>`},scroll:{html:`${_(`modal-scroll`,`이용 약관 보기`)}\n${v(`modal-scroll`,`이용 약관`,`<p>긴 본문…</p>`,` modal_scrollable`)}`,react:`<Modal open={open} scrollable title="이용 약관"><p>긴 본문…</p></Modal>`,vue:`<Modal :open="open" scrollable title="이용 약관"><p>긴 본문…</p></Modal>`},nested:{html:`${_(`modal-parent`,`첫 번째 Modal 열기`)}\n${v(`modal-parent`,`첫 번째 Modal`,`<button data-modal-trigger="#modal-child">두 번째 Modal 열기</button>`)}\n${v(`modal-child`,`두 번째 Modal`,`<p>최상위 대화상자</p>`,` modal_sm`,`style="--modal-stack-level: 1"`)}`,react:`<><Modal open={parentOpen} title="첫 번째 Modal"><Button label="두 번째 Modal 열기" onClick={() => setChildOpen(true)} /></Modal><Modal open={childOpen} size="sm" title="두 번째 Modal" onClose={() => setChildOpen(false)} /></>`,vue:`<Modal :open="parentOpen" title="첫 번째 Modal"><Button label="두 번째 Modal 열기" @click="childOpen = true" /></Modal><Modal :open="childOpen" size="sm" title="두 번째 Modal" @close="childOpen = false" />`},noBackdrop:{html:`${_(`modal-plain`,`백드롭 없이 열기`)}\n${v(`modal-plain`,`백드롭 없음`,`<p>배경을 가리지 않습니다.</p>`,` modal_sm`,`data-modal-backdrop="false"`)}`,react:`<Modal open={open} backdrop={false} size="sm" title="백드롭 없음" onClose={() => setOpen(false)} />`,vue:`<Modal :open="open" :backdrop="false" size="sm" title="백드롭 없음" @close="open = false" />`}},w=(e,t)=>e.split(`
`).map(e=>`${` `.repeat(t)}${e}`).join(`
`),ae=Object.fromEntries(Object.entries(ie).map(([e,t])=>[e,re(e,t)]))})))()}function se(e){switch(e){case`basic`:return`${D}

${O(`drawer-basic`,`Drawer 열기`)}

${k()}
${A(`drawer-basic`,`상세 정보`,`<p>Drawer 패널 본문입니다.</p>`,`, footer=footer`)}`;case`open`:return`${D}

${k()}
${A(`drawer-open`,`알림`,`<p>로드 시 열린 Drawer입니다.</p>`,`, open=true, openOnLoad=true, footer=footer`)}`;case`placement`:return`${D}

${O(`drawer-left`,`왼쪽`)}

${A(`drawer-left`,`왼쪽 패널`,`<p>왼쪽에서 열립니다.</p>`,`, placement='left'`)}`;case`size`:return`${D}

${A(`drawer-lg`,`Large`,`<p>넓은 패널</p>`,`, size='lg', open=true`)}`;case`footer`:return`${D}
${le}

${O(`drawer-footer`,`항목 편집`)}
...
{% endcall %}`;case`footerAlign`:return`${D}

${k()}
${A(`drawer-align`,`푸터 정렬`,`<p>균등 정렬</p>`,`, placement='bottom', open=true, footer=footer, footerAlign='even'`)}`;case`extra`:return`${D}
${ue}

{% set status %}
  {{ tag(label='완료', color='success') }}
{% endset %}
${A(`drawer-extra`,`주문 #1042`,`<p>주문 상세 정보</p>`,`, open=true, extra=status`)}`;case`menu`:return`${D}
...
{% endcall %}`;case`nested`:return`${D}

${O(`drawer-parent`,`첫 번째 Drawer 열기`)}
...
${A(`drawer-child`,`두 번째 Drawer`,`<p>최상위 패널</p>`,`, size='sm', style='--drawer-stack-level: 1'`)}`;case`dragSheet`:return`${D}

${O(`drawer-drag`,`드래그 시트 열기`)}

${A(`drawer-drag`,`공유 · 액션`,`<p>하단 액션 시트입니다.</p>`,`, placement='bottom', draggable=true`)}`;case`noBackdrop`:return`${D}

${A(`drawer-plain`,`백드롭 없음`,`<p>패널만 표시합니다.</p>`,`, size='sm', noBackdrop=true, open=true`)}`;case`motion`:return`${D}

${O(`drawer-fade`,`Fade 열기`)}

${A(`drawer-fade`,`Fade 효과`,`<p>motion=&quot;fade&quot; 전환입니다.</p>`,`, motion='fade'`)}`;case`speed`:return`${D}

${O(`drawer-slow`,`Slow 열기`)}

${A(`drawer-slow`,`Slow 속도`,`<p>speed=&quot;slow&quot; 전환입니다.</p>`,`, speed='slow'`)}`;default:return D}}function ce(e,t){let n=`import { useState } from 'react';\nimport Drawer from '@uxkm/react/drawer';\nimport Button from '@uxkm/react/button';\n\nexport function Example() {\n  const [open, setOpen] = useState(${e===`open`?`true`:`false`});\n  return (\n${j(t.react,/^\s*<>/.test(t.react)?2:4)}\n  );\n}`,r=`<script setup>\nimport { ref } from 'vue';\nimport Drawer from '@uxkm/vue/drawer';\nimport Button from '@uxkm/vue/button';\nconst open = ref(${e===`open`?`true`:`false`});\n<\/script>\n\n<template>\n${j(t.vue,2)}\n</template>`;return[{id:`html`,label:`HTML`,fileName:`Drawer.html · ${e}`,code:t.html},{id:`gulp`,label:`Gulp`,fileName:`drawer.njk · ${e}`,code:se(e)},{id:`vue`,label:`Vue`,fileName:`@uxkm/vue/drawer · ${e}`,code:r},{id:`nuxt`,label:`Nuxt`,fileName:`@uxkm/vue/drawer · ${e}`,code:r},{id:`react`,label:`React`,fileName:`@uxkm/react/drawer · ${e}`,code:n},{id:`next`,label:`Next`,fileName:`@uxkm/react/drawer · ${e}`,code:n}]}var T,E,D,le,ue,O,k,A,de,j,fe;function pe(){return(pe=e((()=>{T=(e,t)=>`<button class="btn btn_filled color_primary" data-drawer-trigger="#${e}" aria-controls="${e}">${t}</button>`,E=(e,t,n,r=`drawer_placement-right`,i=``)=>`<div id="${e}" class="drawer" data-drawer ${i} role="dialog" aria-modal="true" aria-labelledby="${e}-title" tabindex="-1" hidden>
...
</div>`,D=`{% from "components/feedback/Drawer/drawer.njk" import drawer %}
{% from "components/basic/Button/button.njk" import button %}`,le=`{% from "components/form/Input/input.njk" import input, inputField %}`,ue=`{% from "components/data-display/Tag/tag.njk" import tag %}`,O=(e,t)=>`{{ button(
  label='${t}',
  dataAttributes={
    'data-drawer-trigger': '#${e}',
    'aria-controls': '${e}',
    'aria-expanded': 'false'
  }
) }}`,k=(e=`확인`)=>`{% set footer %}
...
{% endset %}`,A=(e,t,n,r=``)=>`{% call drawer(id='${e}', title='${t}'${r}) %}
  ${n}
{% endcall %}`,de={basic:{html:`${T(`drawer-basic`,`Drawer 열기`)}\n${E(`drawer-basic`,`상세 정보`,`<p>Drawer 패널 본문입니다.</p>`)}`,react:`<><Button label="Drawer 열기" onClick={() => setOpen(true)} /><Drawer open={open} title="상세 정보" onClose={() => setOpen(false)}><p>Drawer 패널 본문입니다.</p></Drawer></>`,vue:`<Button label="Drawer 열기" @click="open = true" /><Drawer :open="open" title="상세 정보" @close="open = false"><p>Drawer 패널 본문입니다.</p></Drawer>`},open:{html:E(`drawer-open`,`알림`,`<p>로드 시 열린 Drawer입니다.</p>`).replace(`class="drawer"`,`class="drawer is-open"`).replace(` hidden`,``),react:`<Drawer defaultOpen title="알림"><p>로드 시 열린 Drawer입니다.</p></Drawer>`,vue:`<Drawer default-open title="알림"><p>로드 시 열린 Drawer입니다.</p></Drawer>`},placement:{html:`${T(`drawer-left`,`왼쪽`)}\n${E(`drawer-left`,`왼쪽 패널`,`<p>왼쪽에서 열립니다.</p>`,`drawer_placement-left`)}`,react:`<Drawer open={open} placement="left" title="왼쪽 패널" onClose={() => setOpen(false)} />`,vue:`<Drawer :open="open" placement="left" title="왼쪽 패널" @close="open = false" />`},size:{html:E(`drawer-lg`,`Large`,`<p>넓은 패널</p>`,`drawer_panel drawer_placement-right drawer_lg`).replace(`drawer_panel drawer_panel`,`drawer_panel`),react:`<Drawer open={open} size="lg" title="Large" />`,vue:`<Drawer :open="open" size="lg" title="Large" />`},footer:{html:`${T(`drawer-footer`,`항목 편집`)}\n${E(`drawer-footer`,`항목 편집`,`<label>이름 <input class="input"></label>`)}`,react:`<Drawer open={open} title="항목 편집" footer={<><Button variant="ghost" label="취소" /><Button label="저장" /></>} />`,vue:`<Drawer :open="open" title="항목 편집"><template #footer><Button variant="ghost" label="취소" /><Button label="저장" /></template></Drawer>`},footerAlign:{html:E(`drawer-align`,`푸터 정렬`,`<p>균등 정렬</p>`,`drawer_placement-bottom`).replace(`drawer_footer"`,`drawer_footer drawer_footer-even"`),react:`<Drawer open={open} placement="bottom" footerAlign="even" footer={<><Button label="취소" /><Button label="확인" /></>} />`,vue:`<Drawer :open="open" placement="bottom" footer-align="even"><template #footer><Button label="취소" /><Button label="확인" /></template></Drawer>`},extra:{html:E(`drawer-extra`,`주문 #1042`,`<p>주문 상세 정보</p>`).replace(`<button class="drawer_close"`,`<span class="tag">완료</span><button class="drawer_close"`),react:`<Drawer open title="주문 #1042" extra={<Tag>완료</Tag>} />`,vue:`<Drawer open title="주문 #1042"><template #extra><Tag>완료</Tag></template></Drawer>`},menu:{html:`${T(`drawer-menu`,`메뉴 열기`)}\n${E(`drawer-menu`,`앱 메뉴`,`<nav class="menu"><a class="menu_link" href="#dashboard">대시보드</a></nav>`,`drawer_placement-left`)}`,react:`<Drawer open={open} placement="left" title="앱 메뉴"><nav className="menu">대시보드 · 분석 · 설정</nav></Drawer>`,vue:`<Drawer :open="open" placement="left" title="앱 메뉴"><nav class="menu">대시보드 · 분석 · 설정</nav></Drawer>`},nested:{html:`${T(`drawer-parent`,`첫 번째 Drawer 열기`)}\n${E(`drawer-parent`,`첫 번째 Drawer`,`<button data-drawer-trigger="#drawer-child">두 번째 열기</button>`)}\n${E(`drawer-child`,`두 번째 Drawer`,`<p>최상위 패널</p>`,`drawer_placement-right drawer_sm`,`style="--drawer-stack-level: 1"`)}`,react:`<><Drawer open={parentOpen} title="첫 번째 Drawer" /><Drawer open={childOpen} size="sm" title="두 번째 Drawer" /></>`,vue:`<Drawer :open="parentOpen" title="첫 번째 Drawer" /><Drawer :open="childOpen" size="sm" title="두 번째 Drawer" />`},dragSheet:{html:`${T(`drawer-drag`,`드래그 시트 열기`)}\n${E(`drawer-drag`,`공유 · 액션`,`<p>하단 액션 시트입니다.</p>`,`drawer_placement-bottom drawer_draggable`,`data-drawer-draggable="true"`)}`,react:`<Drawer open={open} placement="bottom" draggable title="공유 · 액션"><p>하단 액션 시트입니다.</p></Drawer>`,vue:`<Drawer :open="open" placement="bottom" draggable title="공유 · 액션"><p>하단 액션 시트입니다.</p></Drawer>`},noBackdrop:{html:E(`drawer-plain`,`백드롭 없음`,`<p>패널만 표시합니다.</p>`,`drawer_placement-right drawer_sm`,`data-drawer-backdrop="false"`),react:`<Drawer open={open} noBackdrop size="sm" title="백드롭 없음" />`,vue:`<Drawer :open="open" no-backdrop size="sm" title="백드롭 없음" />`},motion:{html:`${T(`drawer-fade`,`Fade 열기`)}\n${E(`drawer-fade`,`Fade 효과`,`<p>motion=&quot;fade&quot; 전환입니다.</p>`,`drawer_placement-right drawer_motion-fade`,`data-drawer-motion="fade"`)}`,react:`<Drawer open={open} motion="fade" title="Fade 효과" onClose={() => setOpen(false)}><p>motion=&quot;fade&quot; 전환입니다.</p></Drawer>`,vue:`<Drawer :open="open" motion="fade" title="Fade 효과" @close="open = false"><p>motion=&quot;fade&quot; 전환입니다.</p></Drawer>`},speed:{html:`${T(`drawer-slow`,`Slow 열기`)}\n${E(`drawer-slow`,`Slow 속도`,`<p>speed=&quot;slow&quot; 전환입니다.</p>`,`drawer_placement-right`,`data-drawer-speed="slow" style="--drawer-motion-scale: 1.5"`)}`,react:`<Drawer open={open} speed="slow" title="Slow 속도" onClose={() => setOpen(false)}><p>speed=&quot;slow&quot; 전환입니다.</p></Drawer>`,vue:`<Drawer :open="open" speed="slow" title="Slow 속도" @close="open = false"><p>speed=&quot;slow&quot; 전환입니다.</p></Drawer>`}},j=(e,t)=>e.split(`
`).map(e=>`${` `.repeat(t)}${e}`).join(`
`),fe=Object.fromEntries(Object.entries(de).map(([e,t])=>[e,ce(e,t)]))})))()}function me(e){let t=M[e];if(e===`hover`)return`import Popover from '@uxkm/react/popover';
...
<template>
  <Popover title="빠른 메모">
    ...
    <template #footer><Button variant="ghost" size="sm" label="취소" /><Button color="danger" size="sm" label="삭제" /></template>`:``;return`<script setup>\nimport Popover from '@uxkm/vue/popover';\nimport Button from '@uxkm/vue/button';\n<\/script>\n\n<template>\n  <Popover ${n?`${n} `:``}${r.trim()}>\n    <template #trigger><Button variant="outline" ${t.button||``} label="${t.label}" /></template>\n    <p>${t.body}</p>${i}\n  </Popover>\n</template>`}function ge(e){let t=M[e],n=[`popover`,e===`open`&&`is-open`,e===`size`&&`popover_sm`,e===`offset`&&`popover_offset-lg`,e===`placement`&&`popover_placement-top-center`,e===`arrowAnchor`&&`popover_arrow-anchor-target`,e===`noArrow`&&`popover_no-arrow`].filter(Boolean).join(` `),r=e===`footer`?`
...
{% endcall %}`;if(e===`hover`)return`{% from "components/feedback/Popover/popover.njk" import popover %}
...
<p>
  ...
  {% endcall %}
  서비스를 이용할 수 있습니다.
</p>`;let n=[`id='popover-${e}'`,t.heading?`title='${t.heading}'`:`panelLabel='Popover 안내'`,`triggerLabel='${t.label}'`,e===`open`&&`open=true`,e===`size`&&`size='sm'`,e===`offset`&&`offset='lg'`,e===`placement`&&`placement='top-center'`,e===`arrowAnchor`&&`arrowAnchor='target'`,e===`noArrow`&&`noArrow=true`,e===`open`||e===`trigger`?`triggerVariant='filled'`:``,e===`size`||e===`offset`||e===`placement`||e===`arrowAnchor`?`triggerSize='sm'`:``,e===`title`||e===`noArrow`?`triggerVariant='ghost'`:``].filter(Boolean);return`${N}
...
{% call tooltip(
  ...
  짧은 도움말 텍스트입니다.
{% endcall %}`;if(e===`inline`)return`{% from "components/feedback/Tooltip/tooltip.njk" import tooltip %}
{% from "components/basic/Link/link.njk" import link %}
...
{% endcall %}`;let n=[`id='tooltip-${e}'`,`triggerLabel='${t.label}'`,e===`open`&&`open=true`,e===`size`&&`size='sm'`,e===`offset`&&`offset='lg'`,e===`click`&&`trigger='click'`,e===`placement`&&`placement='top'`,e===`inverse`&&`inverse=true`,e===`arrowAnchor`&&`arrowAnchor='target'`,e===`noArrow`&&`noArrow=true`,e===`open`||e===`trigger`?`triggerVariant='filled'`:``,e===`size`||e===`offset`||e===`arrowAnchor`?`triggerVariant='ghost'`:``,e===`size`||e===`offset`||e===`placement`||e===`arrowAnchor`?`triggerSize='sm'`:``,e===`inverse`||e===`noArrow`?`triggerVariant='ghost'`:``].filter(Boolean);return`${F}
...
<Progress percent={72} block showInfo label="progress_block — 전체 너비" />`},Ne={...I,inside:`${I.inside}\n<Progress percent={100} inside />`},Pe=Object.fromEntries(Object.entries(Ne).map(([e,t])=>[e,t.replaceAll(`percent={`,`:percent="`).replaceAll(`} `,`" `).replaceAll(`} />`,`" />`).replaceAll(`showInfo`,`show-info`).replaceAll(`ariaLabel`,`aria-label`)])),Fe=`{% from "components/feedback/Progress/progress.njk" import progress, progressCircle %}`,Ie={basic:`{{ progress(percent=0) }}
{{ progress(percent=30) }}
{{ progress(percent=60) }}
{{ progress(percent=100, status='success') }}`,label:`{{ progress(percent=42, showInfo=true, label='파일 업로드', ariaLabel='파일 업로드 진행률') }}
{{ progress(percent=67, showInfo=true, label='데이터 동기화', ariaLabel='데이터 동기화 진행률') }}`,color:`{{ progress(percent=50, color='primary', ariaLabel='Primary 진행률') }}
{{ progress(percent=100, color='success', ariaLabel='Success 진행률') }}
{{ progress(percent=75, color='warning', ariaLabel='Warning 진행률') }}
{{ progress(percent=35, color='danger', ariaLabel='Danger 진행률') }}`,size:`{{ progress(percent=40, size='sm') }}
{{ progress(percent=55) }}
{{ progress(percent=70, size='lg') }}`,striped:`{{ progress(percent=45, striped=true, ariaLabel='줄무늬 진행률') }}
{{ progress(percent=65, striped=true, animated=true, ariaLabel='애니메이션 진행률') }}`,indeterminate:`{{ progress(indeterminate=true, ariaLabel='처리 중') }}
{{ progress(indeterminate=true, color='success', label='동기화 중…', ariaLabel='동기화 중') }}`,inside:`{{ progress(percent=25, inside=true) }}
{{ progress(percent=80, inside=true, size='lg', color='success') }}`,circle:`{{ progressCircle(percent=25) }}
{{ progressCircle(percent=68, color='success') }}
{{ progressCircle(percent=50, size='sm', color='warning') }}
{{ progressCircle(percent=90, size='lg', color='danger') }}`,width:`{{ progress(percent=50, showInfo=true, label='progress_fit') }}
{{ progress(percent=72, block=true, showInfo=true, label='progress_block — 전체 너비') }}`},Le=e=>`${Fe}

${Ie[e]}`,Re=Object.fromEntries(Me.map(e=>[e,je(e)]))})))()}function L(e=`color_primary`,t=``,n=`로딩 중`){return`<div class="spin ${e}" role="status" aria-live="polite" aria-busy="true" aria-label="${n}"><span class="spin_indicator" aria-hidden="true"></span>${t?`<p class="spin_tip">${t}</p>`:``}</div>`}function Be(e){return{basic:[L()],size:[L(`spin_sm color_primary`),L(),L(`spin_lg color_primary`)],color:[L(`color_primary`,``,`Primary 로딩`),L(`color_success`,``,`Success 로딩`),L(`color_warning`,``,`Warning 로딩`),L(`color_danger`,``,`Danger 로딩`)],tip:[L(`color_primary`,`데이터를 불러오는 중…`,`데이터를 불러오는 중`),L(`spin_lg color_primary`,`파일 업로드 중…`,`파일 업로드 중`)],inline:[L(`spin_sm spin_inline color_primary`,`저장 중…`,`저장 중`),`<button class="btn btn_filled color_primary is-loading" disabled aria-busy="true">제출 중…</button>`],block:[L(`spin_block color_primary`,`잠시만 기다려 주세요…`)],overlay:[`<div class="spin_wrap spin_wrap-block is-loading" aria-busy="true"><div class="spin_wrap-body"><div class="card card_shadow"><div class="card_header"><h3 class="card_title">대시보드</h3></div><div class="card_body"><p>차트와 통계가 이 영역에 표시됩니다. 데이터를 불러오는 동안 오버레이가 콘텐츠를 덮습니다.</p></div></div></div>${L(`spin_overlay color_primary`,`데이터 로딩 중…`,`데이터 로딩 중`)}</div>`,`<div class="spin_wrap spin_wrap-block spin_wrap-blur is-loading" aria-busy="true"><div class="spin_wrap-body"><div class="card card_shadow"><div class="card_header"><h3 class="card_title">블러 효과</h3></div><div class="card_body"><p><code>spin_wrap-blur</code>를 함께 사용하면 콘텐츠에 블러가 적용됩니다.</p></div></div></div>${L(`spin_overlay color_primary`,``,`처리 중`)}</div>`]}[e].join(`

`)}function Ve(e){let t=Be(e),n=e===`overlay`?`SpinWrap`:`Spin`,r=`import { ${n} } from '@uxkm/react/spin';${e===`inline`?`
import Button from '@uxkm/react/button';`:``}`,i=`import { ${n} } from '@uxkm/vue/spin';${e===`inline`?`
import Button from '@uxkm/vue/button';`:``}`,a=`${r}\n\nexport function Example() { return <>${R[e]}</>; }`,o=`<script setup>\n${i}\n<\/script>\n<template>\n${Ue[e]}\n</template>`;return[{id:`html`,label:`HTML`,fileName:`Spin.html · ${e}`,code:t},{id:`gulp`,label:`Gulp`,fileName:`spin.njk · ${e}`,code:Ke(e)},{id:`vue`,label:`Vue`,fileName:`@uxkm/vue/spin · ${e}`,code:o},{id:`nuxt`,label:`Nuxt`,fileName:`@uxkm/vue/spin · ${e}`,code:o},{id:`react`,label:`React`,fileName:`@uxkm/react/spin · ${e}`,code:a},{id:`next`,label:`Next`,fileName:`@uxkm/react/spin · ${e}`,code:a}]}var He,R,Ue,We,Ge,Ke,qe;function Je(){return(Je=e((()=>{He=[`basic`,`size`,`color`,`tip`,`inline`,`block`,`overlay`],R={basic:`<Spin />`,size:`<Spin size="sm" />
<Spin />
...
<SpinWrap block blur loading ariaLabel="처리 중"><div className="card card_shadow"><div className="card_header"><h3 className="card_title">블러 효과</h3></div><div className="card_body"><p><code className="typo_code">spin_wrap-blur</code>를 함께 사용하면 콘텐츠에 블러가 적용됩니다.</p></div></div></SpinWrap>`},Ue=Object.fromEntries(Object.entries(R).map(([e,t])=>[e,t.replaceAll(`ariaLabel`,`aria-label`).replaceAll(`className=`,`class=`)])),We=`{% from "components/feedback/Spin/spin.njk" import spin, spinWrap %}`,Ge={basic:`{{ spin() }}`,size:`{{ spin(size='sm') }}
{{ spin() }}
{{ spin(size='lg') }}`,color:`{{ spin(color='primary', ariaLabel='Primary 로딩') }}
{{ spin(color='success', ariaLabel='Success 로딩') }}
{{ spin(color='warning', ariaLabel='Warning 로딩') }}
{{ spin(color='danger', ariaLabel='Danger 로딩') }}`,tip:`{{ spin(tip='데이터를 불러오는 중…', ariaLabel='데이터를 불러오는 중') }}
{{ spin(size='lg', tip='파일 업로드 중…', ariaLabel='파일 업로드 중') }}`,inline:`{% from "components/basic/Button/button.njk" import button %}

<p>
  {{ spin(size='sm', inline=true, tip='저장 중…', ariaLabel='저장 중') }}
</p>
{{ button(label='제출 중…', loading=true, disabled=true) }}`,block:`{{ spin(block=true, tip='잠시만 기다려 주세요…') }}`,overlay:`{% from "components/data-display/Card/card.njk" import card, cardHeader, cardBody %}
...
${Ge[e]}`,qe=Object.fromEntries(He.map(e=>[e,Ve(e)]))})))()}function Ye(e){let t=`import Skeleton from '@uxkm/react/skeleton';\n\nexport function Example() { return <>${Ze[e]}</>; }`,n=`<script setup>\nimport Skeleton from '@uxkm/vue/skeleton';\n<\/script>\n<template>\n${Qe[e]}\n</template>`;return[{id:`html`,label:`HTML`,fileName:`Skeleton.html · ${e}`,code:$e[e]},{id:`gulp`,label:`Gulp`,fileName:`skeleton.njk · ${e}`,code:et[e]},{id:`vue`,label:`Vue`,fileName:`@uxkm/vue/skeleton · ${e}`,code:n},{id:`nuxt`,label:`Nuxt`,fileName:`@uxkm/vue/skeleton · ${e}`,code:n},{id:`react`,label:`React`,fileName:`@uxkm/react/skeleton · ${e}`,code:t},{id:`next`,label:`Next`,fileName:`@uxkm/react/skeleton · ${e}`,code:t}]}var Xe,Ze,Qe,$e,et,tt;function nt(){return(nt=e((()=>{Xe=[`basic`,`shape`,`card`,`static`],Ze={basic:`<div style={{ maxWidth: 'var(--input-max-width)', width: '100%' }}><Skeleton paragraph={3} /></div>`,shape:`<Skeleton avatar paragraph={0} />
...
</div>`,static:`<div style={{ maxWidth: 'var(--input-max-width)', width: '100%' }}><Skeleton active={false} paragraph={3} /></div>`},Qe={basic:`<div style="max-width: var(--input-max-width); width: 100%;"><Skeleton :paragraph="3" /></div>`,shape:`<Skeleton avatar :paragraph="0" />
...
</div>`,static:`<div style="max-width: var(--input-max-width); width: 100%;"><Skeleton :active="false" :paragraph="3" /></div>`},$e={basic:`<div class="skeleton_group" role="status" aria-live="polite" aria-busy="true" aria-label="콘텐츠 로딩 중">
...
</div>`},et={basic:`{% from "components/feedback/Skeleton/skeleton.njk" import skeleton %}

<div style="max-width: var(--input-max-width); width: 100%;">
  {{ skeleton(paragraph=3) }}
</div>`,shape:`{% from "components/feedback/Skeleton/skeleton.njk" import skeleton %}

{{ skeleton(avatar=true, paragraph=0, ariaLabel='아바타 로딩 중') }}
{{ skeleton(round=true, ariaLabel='원형 콘텐츠 로딩 중', style='margin-top: var(--space-md)') }}`,card:`{% from "components/feedback/Skeleton/skeleton.njk" import skeleton, skeletonItem, skeletonRow %}
...
{% call card(
  ...
  {% call cardBody() %}
    ...
    {% endcall %}
  {% endcall %}
{% endcall %}`,static:`{% from "components/feedback/Skeleton/skeleton.njk" import skeleton %}

<div style="max-width: var(--input-max-width); width: 100%;">
  {{ skeleton(active=false, paragraph=3) }}
</div>`},tt=Object.fromEntries(Xe.map(e=>[e,Ye(e)]))})))()}function rt(e){let t=e===`footer`?`
...
import { Card, CardBody } from '@uxkm/vue/card';`:``,s=`import Empty from '@uxkm/react/empty';${t}${r}${a}\n\nexport function Example() { return <>${B[e]}</>; }`,c=`<script setup>\nimport Empty from '@uxkm/vue/empty';${n}${i}${o}\n<\/script>\n<template>\n${ct[e]}\n</template>`;return[{id:`html`,label:`HTML`,fileName:`Empty.html · ${e}`,code:st[e]},{id:`gulp`,label:`Gulp`,fileName:`empty.njk · ${e}`,code:lt[e]},{id:`vue`,label:`Vue`,fileName:`@uxkm/vue/empty · ${e}`,code:c},{id:`nuxt`,label:`Nuxt`,fileName:`@uxkm/vue/empty · ${e}`,code:c},{id:`react`,label:`React`,fileName:`@uxkm/react/empty · ${e}`,code:s},{id:`next`,label:`Next`,fileName:`@uxkm/react/empty · ${e}`,code:s}]}var it,at,ot,z,st,B,ct,lt,ut;function dt(){return(dt=e((()=>{it=[`basic`,`footer`,`custom`,`size`,`simple`,`block`],at=`<div class="empty_image" data-slot="icon" aria-hidden="true"><svg class="icon" data-component="Icon" data-icon="inbox" aria-hidden="true" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 5h16l2 9v5H2v-5l2-9ZM2 14h5l2 3h6l2-3h5"></path></svg></div>`,ot=`<div class="empty_image" data-slot="icon" aria-hidden="true"><svg class="icon" data-component="Icon" data-icon="search" aria-hidden="true" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="7"></circle><path d="m20 20-4-4"></path></svg></div>`,z=(e,t=``,n=``,r=at,i=`<p>${e}</p>`)=>`<div class="empty${t?` ${t}`:``}" role="status">${r?`\n  ${r}`:``}\n  <div class="empty_desc" data-slot="description">${i}</div>${n?`\n  <div class="empty_footer">${n}</div>`:``}\n</div>`,st={basic:z(`데이터가 없습니다`),footer:z(`등록된 프로젝트가 없습니다. 새 프로젝트를 만들어 시작해 보세요.`,``,`<button type="button" class="btn btn_filled color_primary btn_sm">프로젝트 만들기</button>`),custom:z(`검색 결과가 없습니다. 다른 키워드로 다시 검색해 보세요.`,``,``,ot,`<strong>검색 결과가 없습니다</strong><span>다른 키워드로 다시 검색해 보세요.</span>`),size:[z(`Small`,`empty_sm`),z(`Medium (기본)`),z(`Large`,`empty_lg`)].join(`

`),simple:z(`일정 없음`,`empty_sm empty_simple`,``,``),block:`<article class="card card_shadow" data-component="Card" style="max-width: var(--input-max-width); width: 100%;"><div class="card_header"><div class="card_header-main"><h3 class="card_title">알림 목록</h3></div></div><div class="card_body">${z(`새 알림이 없습니다`,`empty_block`)}</div></article>
<div class="table_wrap" style="max-width: var(--input-max-width); width: 100%; margin-top: var(--space-md);"><table class="table table_bordered"><thead><tr><th scope="col">이름</th><th scope="col">상태</th><th scope="col">날짜</th></tr></thead><tbody><tr><td colspan="3" style="padding: 0; border: none;">${z(`표시할 항목이 없습니다`,`empty_sm empty_simple empty_block`,``,``)}</td></tr></tbody></table></div>`},B={basic:`<Empty description="데이터가 없습니다" />`,footer:`<Empty description="등록된 프로젝트가 없습니다. 새 프로젝트를 만들어 시작해 보세요." footer={<Button variant="filled" color="primary" size="sm" label="프로젝트 만들기" />} />`,custom:`<Empty icon={<Icon name="search" />}><strong>검색 결과가 없습니다</strong><span>다른 키워드로 다시 검색해 보세요.</span></Empty>`,size:`<Empty size="sm" description="Small" />
...
<div className="table_wrap" style={{ maxWidth: 'var(--input-max-width)', width: '100%', marginTop: 'var(--space-md)' }}><table className="table table_bordered"><thead><tr><th scope="col">이름</th><th scope="col">상태</th><th scope="col">날짜</th></tr></thead><tbody><tr><td colSpan={3} style={{ padding: 0, border: 'none' }}><Empty block simple size="sm" description="표시할 항목이 없습니다" /></td></tr></tbody></table></div>`},ct={basic:`<Empty description="데이터가 없습니다" />`,footer:`<Empty description="등록된 프로젝트가 없습니다. 새 프로젝트를 만들어 시작해 보세요."><template #footer><Button variant="filled" color="primary" size="sm" label="프로젝트 만들기" /></template></Empty>`,custom:`<Empty><template #icon><Icon name="search" /></template><template #description><strong>검색 결과가 없습니다</strong><span>다른 키워드로 다시 검색해 보세요.</span></template></Empty>`,size:`<Empty size="sm" description="Small" />
<Empty description="Medium (기본)" />
<Empty size="lg" description="Large" />`,simple:`<Empty simple size="sm" description="일정 없음" />`,block:B.block.replaceAll(`className=`,`class=`).replace(`colSpan={3}`,`colspan="3"`).replaceAll(`style={{ maxWidth: 'var(--input-max-width)', width: '100%' }}`,`style="max-width: var(--input-max-width); width: 100%;"`).replace(`style={{ maxWidth: 'var(--input-max-width)', width: '100%', marginTop: 'var(--space-md)' }}`,`style="max-width: var(--input-max-width); width: 100%; margin-top: var(--space-md);"`).replace(`style={{ padding: 0, border: 'none' }}`,`style="padding: 0; border: none;"`)},lt={basic:`{% from "components/feedback/Empty/empty.njk" import empty %}

{{ empty(description='데이터가 없습니다') }}`,footer:`{% from "components/feedback/Empty/empty.njk" import empty %}
{% from "components/basic/Button/button.njk" import button %}
...
{% endcall %}`,size:`{% from "components/feedback/Empty/empty.njk" import empty %}

{{ empty(size='sm', description='Small') }}
{{ empty(description='Medium (기본)') }}
{{ empty(size='lg', description='Large') }}`,simple:`{% from "components/feedback/Empty/empty.njk" import empty %}

{{ empty(simple=true, size='sm', description='일정 없음') }}`,block:`{% from "components/feedback/Empty/empty.njk" import empty %}
...
{% endcall %}`},ut=Object.fromEntries(it.map(e=>[e,rt(e)]))})))()}var ft;function pt(){return(pt=e((()=>{ft=`<!-- Alert 원본 구현: 피드백 상태와 노출 동작을 관리하고 필요한 접근성 역할과 사용자 이벤트를 연결합니다. -->
...
`})))()}var mt;function ht(){return(ht=e((()=>{mt=`{# Alert 원본 구현: 상태 메시지와 액션을 macro로 구성합니다. #}
...
`})))()}var bt;function xt(){return(xt=e((()=>{bt=`<!-- Snackbar 원본 구현: 피드백 상태와 노출 동작을 관리하고 필요한 접근성 역할과 사용자 이벤트를 연결합니다. -->
...
`})))()}var St;function Ct(){return(Ct=e((()=>{St=`{# Snackbar 원본 구현: 메시지·액션·배치 영역을 macro로 구성합니다. #}
{% import "components/basic/Icon/icon.njk" as Icon %}
...
`})))()}var Ot;function kt(){return(kt=e((()=>{Ot=`<!--
  Snackbar 원본 구현.
  피드백 상태와 노출 동작을 관리하고 필요한 접근성 역할과 사용자 이벤트를 연결합니다.
-->
<script setup>
...
`})))()}var Mt;function Nt(){return(Nt=e((()=>{Mt=`<!-- Modal 원본 구현: 피드백 상태와 노출 동작을 관리하고 필요한 접근성 역할과 사용자 이벤트를 연결합니다. -->
...
`})))()}var Pt;function Ft(){return(Ft=e((()=>{Pt=`{# Modal 원본 구현: 대화상자의 루트·헤더·본문·푸터를 macro로 조합합니다. #}
{% import "components/basic/Button/button.njk" as Button %}
...
{% endcall %}
`})))()}var It;function Lt(){return(Lt=e((()=>{It=`/**
 * Modal 원본 구현.
 * 피드백 상태와 노출 동작을 관리하고 필요한 접근성 역할과 사용자 이벤트를 연결합니다.
 */
...
`})))()}var Bt;function Vt(){return(Vt=e((()=>{Bt=`<!-- Drawer 원본 구현: 피드백 상태와 노출 동작을 관리하고 필요한 접근성 역할과 사용자 이벤트를 연결합니다. -->
...
`})))()}var Ht;function Ut(){return(Ut=e((()=>{Ht=`{# Drawer 원본 구현: 패널의 루트·헤더·본문·푸터를 macro로 조합합니다. #}
{% import "components/basic/Button/button.njk" as Button %}
...
`})))()}var Kt;function qt(){return(qt=e((()=>{Kt=`<!--
  Drawer 원본 구현.
  피드백 상태와 노출 동작을 관리하고 필요한 접근성 역할과 사용자 이벤트를 연결합니다.
-->
<script setup>
...
`})))()}var Jt;function Yt(){return(Yt=e((()=>{Jt=`<!-- Popover 원본 구현: 피드백 상태와 노출 동작을 관리하고 필요한 접근성 역할과 사용자 이벤트를 연결합니다. -->
...
`})))()}var Xt;function Zt(){return(Zt=e((()=>{Xt=`{# Popover 원본 구현: 트리거·패널·헤더·본문·푸터를 macro로 조합합니다. #}
{% import "components/basic/Button/button.njk" as Button %}
...
`})))()}var nn;function rn(){return(rn=e((()=>{nn=`<!-- Tooltip 원본 구현: 피드백 상태와 노출 동작을 관리하고 필요한 접근성 역할과 사용자 이벤트를 연결합니다. -->
...
`})))()}var an;function on(){return(on=e((()=>{an=`{# Tooltip 원본 구현: 트리거와 말풍선을 호출형 macro로 조합합니다. #}
{% import "components/basic/Button/button.njk" as Button %}
...
`})))()}var sn;function cn(){return(cn=e((()=>{sn=`/**
 * Tooltip 원본 구현.
 * 피드백 상태와 노출 동작을 관리하고 필요한 접근성 역할과 사용자 이벤트를 연결합니다.
 */
import { cloneElement, isValidElement, useEffect, useId, useMemo, useRef, useState } from 'react';
...
`})))()}var dn;function fn(){return(fn=e((()=>{dn=`<!-- Progress 원본 구현: 피드백 상태와 노출 동작을 관리하고 필요한 접근성 역할과 사용자 이벤트를 연결합니다. -->
...
`})))()}var pn;function mn(){return(mn=e((()=>{pn=`{# Progress 원본 구현: 선형·원형 진행률을 호출형 macro로 구성합니다. #}
...
`})))()}var hn;function gn(){return(gn=e((()=>{hn=`/**
 * Progress 원본 구현.
 * 피드백 상태와 노출 동작을 관리하고 필요한 접근성 역할과 사용자 이벤트를 연결합니다.
 */
...
`})))()}var Cn;function wn(){return(wn=e((()=>{Cn=`<!-- Spin 원본 구현: 피드백 상태와 노출 동작을 관리하고 필요한 접근성 역할과 사용자 이벤트를 연결합니다. -->
...
`})))()}var Tn;function En(){return(En=e((()=>{Tn=`{# Spin 원본 구현: 단독 로더와 콘텐츠 오버레이를 호출형 macro로 구성합니다. #}
...
{# include 시 기본 데모 (from import 시에는 실행되지 않음) #}
{{ spin() }}
`})))()}var Dn;function On(){return(On=e((()=>{Dn=`/**
 * Spin 원본 구현.
 * 피드백 상태와 노출 동작을 관리하고 필요한 접근성 역할과 사용자 이벤트를 연결합니다.
 */
...
export default Spin;
`})))()}var kn;function An(){return(An=e((()=>{kn=`/**
 * SpinWrap 원본 구현.
 * 피드백 상태와 노출 동작을 관리하고 필요한 접근성 역할과 사용자 이벤트를 연결합니다.
 */
...
`})))()}var jn;function Mn(){return(Mn=e((()=>{jn=`<!--
  Spin 원본 구현.
  피드백 상태와 노출 동작을 관리하고 필요한 접근성 역할과 사용자 이벤트를 연결합니다.
-->
<script setup>
...
`})))()}var Nn;function Pn(){return(Pn=e((()=>{Nn=`<!--
  SpinWrap 원본 구현.
  피드백 상태와 노출 동작을 관리하고 필요한 접근성 역할과 사용자 이벤트를 연결합니다.
-->
<script setup>
...
`})))()}var Fn;function In(){return(In=e((()=>{Fn=`<!-- Skeleton 원본 구현: 피드백 상태와 노출 동작을 관리하고 필요한 접근성 역할과 사용자 이벤트를 연결합니다. -->
...
`})))()}var Ln;function Rn(){return(Rn=e((()=>{Ln=`{# Skeleton 원본 구현: 그룹·행·개별 플레이스홀더를 호출형 macro로 구성합니다. #}
...
{# include 시 기본 데모 (from import 시에는 실행되지 않음) #}
{{ skeleton(paragraph=3) }}
`})))()}var zn;function Bn(){return(Bn=e((()=>{zn=`/**
 * Skeleton 원본 구현.
 * 피드백 상태와 노출 동작을 관리하고 필요한 접근성 역할과 사용자 이벤트를 연결합니다.
 */
import { useMemo } from 'react';
...
`})))()}var Vn;function Hn(){return(Hn=e((()=>{Vn=`<!--
  Skeleton 원본 구현.
  피드백 상태와 노출 동작을 관리하고 필요한 접근성 역할과 사용자 이벤트를 연결합니다.
-->
<script setup>
...
`})))()}var Un;function Wn(){return(Wn=e((()=>{Un=`<!-- Empty 원본 구현: 피드백 상태와 노출 동작을 관리하고 필요한 접근성 역할과 사용자 이벤트를 연결합니다. -->
...
`})))()}var Gn;function Kn(){return(Kn=e((()=>{Gn=`{# Empty 원본 구현: 이미지·설명·푸터 영역을 호출형 macro로 구성합니다. #}
{% import "components/basic/Icon/icon.njk" as Icon %}

{% macro emptyImage(iconName='inbox', content='', className='') %}
{%- set image = caller() if caller else content -%}
<div class="empty_image{{ (' ' + className) if className else '' }}" data-slot="icon" aria-hidden="true">
  {% if image %}{{ image | safe }}{% else %}{{ Icon.icon(name=iconName) }}{% endif %}
</div>
{% endmacro %}

{% macro emptyDescription(description='', content='', className='') %}
{%- set body = caller() if caller else content -%}
<div class="empty_desc{{ (' ' + className) if className else '' }}" data-slot="description">
  {% if body %}{{ body | safe }}{% else %}<p>{{ description }}</p>{% endif %}
</div>
{% endmacro %}

{% macro emptyFooter(content='', className='') %}
{%- set footer = caller() if caller else content -%}
<div class="empty_footer{{ (' ' + className) if className else '' }}">
  {{ footer | safe }}
</div>
{% endmacro %}
...
{%- set customDescription = caller() if caller else descriptionSlot -%}
{%- set customIcon = iconSlot if iconSlot else image -%}
{%- set resolvedSize = size if size in ['sm', 'md', 'lg'] else 'md' -%}
<div
  class="empty{% if resolvedSize != 'md' %} empty_{{ resolvedSize }}{% endif %}{% if simple %} empty_simple{% endif %}{% if block %} empty_block{% endif %}{{ (' ' + className) if className else '' }}"
  data-component="Empty"
  role="status"
>
  {% if not simple %}
    {{ emptyImage(iconName=iconName, content=customIcon) }}
  {% endif %}
  {% if customDescription or description %}
    {{ emptyDescription(description=description, content=customDescription) }}
  {% endif %}
  {% if footer %}{{ emptyFooter(content=footer) }}{% endif %}
</div>
{% endmacro %}

{# include 시 기본 데모 (from import 시에는 실행되지 않음) #}
{{ empty(description='표시할 데이터가 없습니다.') }}
`})))()}var qn;function Jn(){return(Jn=e((()=>{qn=`/**
 * Empty 원본 구현.
 * 피드백 상태와 노출 동작을 관리하고 필요한 접근성 역할과 사용자 이벤트를 연결합니다.
 */
import Icon from '../../basic/Icon/Icon.jsx';
...
`})))()}var Yn;function Xn(){return(Xn=e((()=>{Yn=`<!--
  Empty 원본 구현.
  피드백 상태와 노출 동작을 관리하고 필요한 접근성 역할과 사용자 이벤트를 연결합니다.
-->
<script setup>
...
`})))()}function V(e,t,n){return[{id:`html`,label:`HTML`,fileName:`apps/html/src/components/feedback/${e}/${e}.html`,code:n.html},{id:`gulp`,label:`Gulp`,fileName:`apps/gulp/src/components/feedback/${e}/${t}.njk`,code:n.gulp},{id:`vue`,label:`Vue`,fileName:`apps/vue/src/components/feedback/${e}/${e}.vue`,code:n.vue},{id:`nuxt`,label:`Nuxt`,fileName:`@uxkm/vue/${t} → ${e}.vue`,code:n.vue},{id:`react`,label:`React`,fileName:`apps/react/src/components/feedback/${e}/${e}.jsx`,code:n.react},{id:`next`,label:`Next`,fileName:`@uxkm/react/${t} → ${e}.jsx`,code:n.react}]}function H(e,t,n){return Object.fromEntries(Object.entries(t).map(([t,r])=>[t,[...r,{id:`websquare`,label:`WebSquare`,fileName:`WebSquare 화면 XML · ${e} · ${t}`,code:`<!-- 화면 XML 루트에 w2, xf, ev 네임스페이스를 선언합니다. -->\n${n(t)}`}]]))}function U(e,t={}){let n=[`alert`,`color_${t.color??`info`}`,t.classes].filter(Boolean).join(` `),r=t.showIcon===!1?``:`<w2:textbox id="${e}Icon" class="alert_icon" aria-hidden="true" label="●"></w2:textbox>`,i=t.title?`<w2:textbox id="${e}Title" class="alert_title" label="${t.title}"></w2:textbox>`:``,a=t.description?`<w2:textbox id="${e}Desc" class="alert_desc" tagname="p" label="${t.description}"></w2:textbox>`:``,o=t.actions?`<w2:group id="${e}Actions" class="alert_actions">
...
`,`
  `):``}
</w2:group>`}function Zn(e){switch(e){case`descriptionOnly`:return U(`descriptionOnly`,{color:`info`,description:`변경 사항이 저장되었습니다.`});case`noIcon`:return U(`noIcon`,{color:`success`,title:`완료`,description:`요청이 처리되었습니다.`,showIcon:!1});case`size`:return[U(`sizeSm`,{classes:`alert_sm`,title:`Small`,description:`작은 알림입니다.`}),U(`sizeMd`,{title:`Medium`,description:`기본 크기 알림입니다.`}),U(`sizeLg`,{classes:`alert_lg`,title:`Large`,description:`큰 알림입니다.`})].join(`

`);case`closable`:return U(`closable`,{color:`warning`,title:`주의`,description:`이 작업은 되돌릴 수 없습니다.`,closable:!0});case`actions`:return U(`actions`,{color:`info`,title:`업데이트 안내`,description:`새 버전이 준비되었습니다.`,actions:!0,closable:!0});case`banner`:return U(`banner`,{color:`warning`,classes:`alert_banner`,title:`시스템 점검`,description:`오늘 밤 점검이 예정되어 있습니다.`,closable:!0});case`context`:return`<w2:group id="contextForm" class="form form_vertical form_fit" tagname="form">
  ${U(`context`,{color:`error`,title:`입력 오류`,description:`필수 항목을 확인해 주세요.`}).replaceAll(`
`,`
  `)}
</w2:group>`;default:return[U(`basicInfo`,{color:`info`,title:`정보`,description:`변경 사항이 저장되었습니다.`}),U(`basicSuccess`,{color:`success`,title:`성공`,description:`요청이 완료되었습니다.`}),U(`basicWarning`,{color:`warning`,title:`경고`,description:`제한 시간을 확인해 주세요.`}),U(`basicError`,{color:`error`,title:`오류`,description:`다시 시도해 주세요.`})].join(`

`)}}function W(e,t,n={}){let r=[`snackbar`,`color_${n.color??`info`}`,n.classes].filter(Boolean).join(` `),i=n.showIcon===!1?``:`<w2:textbox id="${e}Icon" class="snackbar_icon" aria-hidden="true" label="●"></w2:textbox>`,a=n.action?`<xf:trigger id="${e}Action" type="button" class="snackbar_action btn btn_ghost btn_sm" ev:onclick="scwin.snackbar_action">
...
`,`
  `):``}
</w2:group>`}function Qn(e){switch(e){case`simple`:return W(`simple`,`저장되었습니다.`,{showIcon:!1});case`shape`:return[W(`shapeDefault`,`기본 모서리`),W(`shapeRound`,`캡슐형`,{classes:`snackbar_round`})].join(`

`);case`motion`:return[W(`motionFade`,`Fade`,{classes:`snackbar_motion-fade`}),W(`motionSlide`,`Slide`,{classes:`snackbar_motion-slide`}),W(`motionNone`,`None`,{classes:`snackbar_motion-none`})].join(`

`);case`size`:return[W(`sizeSm`,`Small`,{classes:`snackbar_sm`}),W(`sizeMd`,`Medium`),W(`sizeLg`,`Large`,{classes:`snackbar_lg`})].join(`

`);case`action`:return W(`action`,`항목이 삭제되었습니다.`,{color:`success`,action:!0});case`closable`:return W(`closable`,`새 알림이 있습니다.`,{closable:!0});case`duration`:return`<w2:group id="durationRegion" class="snackbar_region snackbar_region-bottom-center">
  ${W(`duration`,`3초 후 자동으로 닫힙니다.`,{classes:`snackbar_motion-fade`}).replaceAll(`
`,`
  `)}
</w2:group>
<!-- data-snackbar-duration="3000" 과 scwin.snackbar_timeout으로 자동 닫기를 연결합니다. -->`;case`placement`:return`<w2:group id="placementRegion" class="snackbar_region snackbar_region-top-right">
  ${W(`placementA`,`첫 번째 알림`).replaceAll(`
`,`
  `)}
  ${W(`placementB`,`두 번째 알림`,{color:`success`}).replaceAll(`
`,`
  `)}
</w2:group>`;default:return[W(`basicInfo`,`정보가 업데이트되었습니다.`),W(`basicSuccess`,`저장이 완료되었습니다.`,{color:`success`}),W(`basicWarning`,`연결이 불안정합니다.`,{color:`warning`}),W(`basicError`,`요청에 실패했습니다.`,{color:`error`,role:`alert`})].join(`

`)}}function G(e,t,n,r={}){let i=r.open===!1?``:` is-open`,a=[`modal`,r.classes,i.trim()].filter(Boolean).join(` `),o=r.backdrop===!1?``:`<w2:group id="${e}Backdrop" class="modal_backdrop" ev:onclick="scwin.modal_close"></w2:group>`,s=r.footerAlign?` modal_footer-${r.footerAlign}`:``,c=r.footer?`<w2:group id="${e}Footer" class="modal_footer${s}">
...
${l}`:l}function $n(e){switch(e){case`open`:return G(`open`,`알림`,`로드 시 열린 상태로 표시되는 Modal입니다.`,{open:!0,footer:!0});case`size`:return[G(`sizeSm`,`Small`,`좁은 너비`,{classes:`modal_sm`,triggerLabel:`Small`}),G(`sizeLg`,`Large`,`넓은 너비`,{classes:`modal_lg`,triggerLabel:`Large`})].join(`

`);case`footer`:return G(`footer`,`새 항목 추가`,`이름 입력 필드를 배치합니다.`,{footer:!0,triggerLabel:`새 항목 추가`});case`footerAlign`:return G(`footerAlign`,`푸터 정렬`,`균등 정렬 예시입니다.`,{footer:!0,footerAlign:`even`,open:!0});case`confirm`:return G(`confirm`,`항목 삭제`,`이 작업은 되돌릴 수 없습니다.`,{classes:`modal_sm`,footer:!0,triggerLabel:`삭제`});case`form`:return G(`form`,`프로필 편집`,`이름 등 폼 필드를 배치합니다.`,{footer:!0,triggerLabel:`프로필 편집`});case`scroll`:return G(`scroll`,`이용 약관`,`긴 본문은 modal_body 안에서 스크롤합니다.`,{classes:`modal_scrollable`,triggerLabel:`이용 약관 보기`});case`nested`:return`${G(`nestedParent`,`첫 번째 Modal`,`두 번째 Modal을 엽니다.`,{footer:!0,triggerLabel:`첫 번째 Modal 열기`})}

${G(`nestedChild`,`두 번째 Modal`,`최상위 대화상자만 조작합니다.`,{classes:`modal_sm`,open:!0,footer:!0})}`;case`noBackdrop`:return G(`noBackdrop`,`백드롭 없음`,`배경을 가리지 않습니다.`,{classes:`modal_sm`,backdrop:!1,triggerLabel:`백드롭 없이 열기`});default:return G(`basic`,`알림`,`Modal 대화상자 본문입니다.`,{footer:!0,triggerLabel:`모달 열기`})}}function K(e,t,n,r={}){let i=r.placement??`right`,a=r.open===!1?``:` is-open`,o=[`drawer`,`drawer_placement-${i}`,r.classes,r.draggable&&`drawer_draggable`,a.trim()].filter(Boolean).join(` `),s=r.backdrop===!1?``:`<w2:group id="${e}Backdrop" class="drawer_backdrop" ev:onclick="scwin.drawer_close"></w2:group>`,c=r.draggable?`<w2:group id="${e}Handle" class="drawer_handle" aria-hidden="true"></w2:group>`:``,l=r.extra?`<w2:textbox id="${e}Extra" class="tag" label="${r.extra}"></w2:textbox>`:``,u=r.footerAlign?` drawer_footer-${r.footerAlign}`:``,d=r.footer?`<w2:group id="${e}Footer" class="drawer_footer${u}">
...
${f}`:f}function er(e){switch(e){case`open`:return K(`open`,`알림`,`로드 시 열린 Drawer입니다.`,{open:!0});case`placement`:return K(`placement`,`왼쪽 패널`,`왼쪽에서 열립니다.`,{placement:`left`,triggerLabel:`왼쪽`});case`size`:return K(`size`,`Large`,`넓은 패널`,{classes:`drawer_lg`,triggerLabel:`Large`});case`footer`:return K(`footer`,`항목 편집`,`이름 입력 필드를 배치합니다.`,{footer:!0,triggerLabel:`항목 편집`});case`footerAlign`:return K(`footerAlign`,`푸터 정렬`,`균등 정렬 예시입니다.`,{placement:`bottom`,footer:!0,footerAlign:`even`,open:!0});case`extra`:return K(`extra`,`주문 #1042`,`주문 상세 정보`,{extra:`완료`,open:!0});case`menu`:return K(`menu`,`앱 메뉴`,`대시보드 · 분석 · 설정`,{placement:`left`,triggerLabel:`메뉴 열기`});case`nested`:return`${K(`nestedParent`,`첫 번째 Drawer`,`두 번째 Drawer를 엽니다.`,{triggerLabel:`첫 번째 Drawer 열기`})}

${K(`nestedChild`,`두 번째 Drawer`,`최상위 패널만 조작합니다.`,{classes:`drawer_sm`,open:!0})}`;case`dragSheet`:return K(`dragSheet`,`공유 · 액션`,`하단 액션 시트입니다.`,{placement:`bottom`,draggable:!0,triggerLabel:`드래그 시트 열기`});case`noBackdrop`:return K(`noBackdrop`,`백드롭 없음`,`패널만 표시합니다.`,{classes:`drawer_sm`,backdrop:!1,open:!0});case`motion`:return K(`motion`,`Fade 효과`,`motion="fade" 전환입니다.`,{classes:`drawer_motion-fade`,triggerLabel:`Fade 열기`});case`speed`:return K(`speed`,`Slow 속도`,`speed="slow" 전환입니다.`,{triggerLabel:`Slow 열기`});default:return K(`basic`,`상세 정보`,`Drawer 패널 본문입니다.`,{footer:!0,triggerLabel:`Drawer 열기`})}}function q(e,t,n,r,i={}){let a=i.open?` is-open`:``,o=[`popover`,i.classes,a.trim()].filter(Boolean).join(` `),s=i.noArrow?``:`<w2:textbox id="${e}Arrow" class="popover_arrow" aria-hidden="true" label=""></w2:textbox>`,c=i.footer?`<w2:group id="${e}Footer" class="popover_footer">
...
</w2:group>`}function tr(e){switch(e){case`noLabel`:return q(`noLabel`,`라벨 없음`,``,`짧은 안내 문구만 필요할 때 사용합니다.`);case`open`:return q(`open`,`열림`,`열린 Popover`,`정적 데모용 열린 상태입니다.`,{open:!0});case`size`:return q(`size`,`Small`,`작은 Popover`,`작은 패널입니다.`,{classes:`popover_sm`});case`offset`:return q(`offset`,`lg`,`lg`,`넓은 간격`,{classes:`popover_offset-lg`});case`title`:return q(`title`,`도움말`,`배송 안내`,`평일 오후 2시 이전 주문은 당일 출고됩니다.`);case`footer`:return q(`footer`,`삭제 확인`,`항목 삭제`,`이 작업은 되돌릴 수 없습니다. 계속하시겠습니까?`,{footer:!0});case`hover`:return q(`hover`,`에스크로`,`에스크로 설명`,`구매자가 상품 수령을 확인할 때까지 대금을 안전하게 보관하는 결제 방식입니다.`);case`trigger`:return q(`trigger`,`Button`,`Button 트리거`,`버튼 트리거 예시입니다.`);case`placement`:return q(`placement`,`위`,`위쪽 가운데 배치`,`트리거 위 · 가운데`,{placement:`top-center`});case`form`:return q(`form`,`메모 추가`,`빠른 메모`,`내용 입력 필드를 배치합니다.`,{footer:!0});case`arrowAnchor`:return q(`arrowAnchor`,`S`,`target`,`화살표가 트리거 중앙을 가리킵니다.`);case`noArrow`:return q(`noArrow`,`화살표 없음`,`화살표 없는 Popover`,`화살표가 표시되지 않는 패널입니다.`,{noArrow:!0});default:return q(`basic`,`Popover 열기`,`안내`,`트리거를 클릭하면 이 패널이 표시됩니다.`)}}function J(e,t,n,r={}){let i=r.open?` is-open`:``,a=[`tooltip`,r.inverse&&`tooltip_inverse`,r.classes,i.trim()].filter(Boolean).join(` `),o=r.noArrow?``:`<w2:textbox id="${e}Arrow" class="tooltip_arrow" aria-hidden="true" label=""></w2:textbox>`;return`<w2:group id="${e}" class="${a}"${r.placement?` data-tooltip-placement="${r.placement}"`:``}>
...
</w2:group>`}function nr(e){switch(e){case`slot`:return J(`slot`,`슬롯 예시`,`짧은 도움말 텍스트입니다.`);case`open`:return J(`open`,`열림`,`정적 데모용 열린 상태입니다.`,{open:!0});case`size`:return J(`size`,`Small`,`작은 말풍선`,{classes:`tooltip_sm`});case`offset`:return J(`offset`,`lg`,`넓은 간격`,{classes:`tooltip_offset-lg`});case`inline`:return J(`inline`,`에스크로`,`구매자가 상품 수령을 확인할 때까지 대금을 안전하게 보관하는 결제 방식입니다.`,{classes:`tooltip_inline`});case`trigger`:return J(`trigger`,`Button`,`버튼 트리거 예시입니다.`);case`click`:return J(`click`,`클릭하여 보기`,`클릭으로 열고 닫는 Tooltip입니다.`);case`placement`:return J(`placement`,`위`,`트리거 위`,{placement:`top`});case`inverse`:return J(`inverse`,`역색 Tooltip`,`어두운 배경의 말풍선입니다.`,{inverse:!0});case`disabled`:return J(`disabled`,`제출 불가`,`필수 항목을 모두 입력해야 제출할 수 있습니다.`);case`arrowAnchor`:return J(`arrowAnchor`,`S`,`화살표가 트리거 중앙을 가리킵니다.`);case`noArrow`:return J(`noArrow`,`화살표 없음`,`화살표가 표시되지 않는 말풍선입니다.`,{noArrow:!0});default:return J(`basic`,`마우스 오버`,`짧은 도움말 텍스트입니다.`)}}function Y(e,t,n={}){let r=n.ariaLabel??n.label??`진행률`;if(n.circle)return`<w2:group id="${e}" class="progress progress_circle${n.classes?` ${n.classes}`:``}" role="progressbar" aria-label="${r}" aria-valuenow="${t}" aria-valuemin="0" aria-valuemax="100">
...
</w2:group>`}function rr(e){switch(e){case`label`:return[Y(`labelUpload`,42,{classes:`progress_fit color_primary`,label:`파일 업로드`}),Y(`labelSync`,67,{classes:`progress_fit color_primary`,label:`데이터 동기화`})].join(`

`);case`color`:return[Y(`colorPrimary`,50,{classes:`progress_fit color_primary`}),Y(`colorSuccess`,100,{classes:`progress_fit color_success`})].join(`

`);case`size`:return[Y(`sizeSm`,40,{classes:`progress_fit progress_sm color_primary`}),Y(`sizeMd`,55,{classes:`progress_fit color_primary`}),Y(`sizeLg`,70,{classes:`progress_fit progress_lg color_primary`})].join(`

`);case`striped`:return Y(`striped`,45,{classes:`progress_fit progress_striped color_primary`});case`indeterminate`:return Y(`indeterminate`,0,{classes:`progress_fit color_primary is-indeterminate`,ariaLabel:`처리 중`});case`inside`:return Y(`inside`,80,{classes:`progress_fit progress_inside progress_lg color_success`});case`circle`:return[Y(`circleA`,25,{circle:!0}),Y(`circleB`,68,{circle:!0,classes:`color_success`})].join(`

`);case`width`:return[Y(`widthFit`,50,{classes:`progress_fit color_primary`,label:`progress_fit`}),Y(`widthBlock`,72,{classes:`progress_block color_primary`,label:`progress_block — 전체 너비`})].join(`

`);default:return[Y(`basic0`,0,{classes:`progress_fit`}),Y(`basic30`,30,{classes:`progress_fit`}),Y(`basic60`,60,{classes:`progress_fit`}),Y(`basic100`,100,{classes:`progress_fit color_success`})].join(`

`)}}function X(e,t={}){let n=t.tip?`<w2:textbox id="${e}Tip" class="spin_tip" label="${t.tip}"></w2:textbox>`:``,r=`<w2:group id="${e}" class="spin${t.classes?` ${t.classes}`:``}" role="status" aria-live="polite" aria-busy="true" aria-label="${t.ariaLabel??t.tip??`로딩 중`}">
...
`,`
  `)}
</w2:group>`:r}function ir(e){switch(e){case`size`:return[X(`sizeSm`,{classes:`spin_sm color_primary`}),X(`sizeMd`,{classes:`color_primary`}),X(`sizeLg`,{classes:`spin_lg color_primary`})].join(`

`);case`color`:return[X(`colorPrimary`,{classes:`color_primary`,ariaLabel:`Primary 로딩`}),X(`colorSuccess`,{classes:`color_success`,ariaLabel:`Success 로딩`})].join(`

`);case`tip`:return X(`tip`,{classes:`color_primary`,tip:`데이터를 불러오는 중…`});case`inline`:return X(`inline`,{classes:`spin_sm spin_inline color_primary`,tip:`저장 중…`});case`block`:return X(`block`,{classes:`spin_block color_primary`,tip:`잠시만 기다려 주세요…`});case`overlay`:return X(`overlay`,{classes:`spin_overlay color_primary`,tip:`데이터 로딩 중…`,overlay:!0});default:return X(`basic`,{classes:`color_primary`})}}function Z(e,t={}){let n=t.lines??3,r=t.staticMode?` skeleton_static`:``,i=t.avatar?`<w2:group id="${e}Row" class="skeleton_row">
...
`,`
  `)}
</w2:group>`}function ar(e){switch(e){case`shape`:return[Z(`shapeAvatar`,{avatar:!0,lines:0}),Z(`shapeRound`,{classes:`skeleton_circle`,lines:0})].join(`
...
`);case`card`:return`<w2:group id="cardRoot" class="card card_shadow" role="status" aria-live="polite" aria-busy="true" aria-label="카드 콘텐츠 로딩 중">
  ${Z(`card`,{avatar:!0,lines:2}).replaceAll(`
`,`
  `)}
</w2:group>`;case`static`:return Z(`static`,{lines:3,staticMode:!0});default:return Z(`basic`,{lines:3})}}function Q(e,t,n={}){let r=n.simple?``:`<w2:textbox id="${e}Image" class="empty_image" aria-hidden="true" label=""></w2:textbox>`,i=n.footer?`<w2:group id="${e}Footer" class="empty_footer">
...
`,`
  `):``}
</w2:group>`}function or(e){switch(e){case`footer`:return Q(`footer`,`등록된 프로젝트가 없습니다. 새 프로젝트를 만들어 시작해 보세요.`,{footer:!0});case`custom`:return Q(`custom`,`검색 결과가 없습니다. 다른 키워드로 다시 검색해 보세요.`);case`size`:return[Q(`sizeSm`,`Small`,{classes:`empty_sm`}),Q(`sizeMd`,`Medium (기본)`),Q(`sizeLg`,`Large`,{classes:`empty_lg`})].join(`

`);case`simple`:return Q(`simple`,`일정 없음`,{classes:`empty_sm empty_simple`,simple:!0});case`block`:return`<w2:group id="blockCard" class="card card_shadow">
  <w2:textbox id="blockTitle" class="card_title" tagname="h3" label="알림 목록"></w2:textbox>
  ${Q(`block`,`새 알림이 없습니다`,{classes:`empty_block`}).replaceAll(`
`,`
  `)}
</w2:group>`;default:return Q(`basic`,`데이터가 없습니다`)}}var sr,$,cr,lr,ur,dr,fr,pr,mr,hr,gr,_r,vr,yr,br,xr,Sr,Cr,wr,Tr;function Er(){return(Er=e((()=>{d(),te(),oe(),pe(),xe(),ke(),ze(),Je(),nt(),dt(),pt(),ht(),_t(),yt(),xt(),Ct(),Tt(),Dt(),kt(),jt(),Nt(),Ft(),Lt(),zt(),Vt(),Ut(),Gt(),qt(),Yt(),Zt(),$t(),tn(),rn(),on(),cn(),un(),fn(),mn(),gn(),vn(),bn(),Sn(),wn(),En(),On(),An(),Mn(),Pn(),In(),Rn(),Bn(),Hn(),Wn(),Kn(),Jn(),Xn(),sr=V(`Alert`,`alert`,{html:ft,gulp:mt,react:gt,vue:vt}),$=V(`Snackbar`,`snackbar`,{html:bt,gulp:St,react:`${wt}\n\n<!-- SnackbarRegion.jsx -->\n${Et}`,vue:`${Ot}\n\n<!-- SnackbarRegion.vue -->\n${At}`}),cr=V(`Modal`,`modal`,{html:Mt,gulp:Pt,react:It,vue:Rt}),lr=V(`Drawer`,`drawer`,{html:Bt,gulp:Ht,react:Wt,vue:Kt}),ur=V(`Popover`,`popover`,{html:Jt,gulp:Xt,react:Qt,vue:en}),dr=V(`Tooltip`,`tooltip`,{html:nn,gulp:an,react:sn,vue:ln}),fr=V(`Progress`,`progress`,{html:dn,gulp:pn,react:`${hn}\n\n<!-- ProgressCircle.jsx -->\n${_n}`,vue:`${yn}\n\n<!-- ProgressCircle.vue -->\n${xn}`}),pr=V(`Spin`,`spin`,{html:Cn,gulp:Tn,react:`${Dn}\n\n<!-- SpinWrap.jsx -->\n${kn}`,vue:`${jn}\n\n<!-- SpinWrap.vue -->\n${Nn}`}),mr=V(`Skeleton`,`skeleton`,{html:Fn,gulp:Ln,react:zn,vue:Vn}),hr=V(`Empty`,`empty`,{html:Un,gulp:Gn,react:qn,vue:Yn}),gr=H(`Alert`,u,Zn),_r=H(`Snackbar`,ee,Qn),vr=H(`Modal`,ae,$n),yr=H(`Drawer`,fe,er),br=H(`Popover`,be,tr),xr=H(`Tooltip`,Oe,nr),Sr=H(`Progress`,Re,rr),Cr=H(`Spin`,qe,ir),wr=H(`Skeleton`,tt,ar),Tr=H(`Empty`,ut,or)})))()}export{_r as _,hr as a,dr as b,cr as c,br as d,fr as f,$ as g,wr as h,yr as i,vr as l,mr as m,gr as n,Tr as o,Sr as p,lr as r,Er as s,sr as t,ur as u,pr as v,xr as x,Cr as y};