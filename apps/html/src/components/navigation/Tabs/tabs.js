...
export function initTabs(root = document) {
  ...
  groups.forEach((tabs) => {
    ...
    const enabled = () => [
      ...tabs.querySelectorAll('[role="tab"]:not(:disabled):not([aria-disabled="true"])'),
    ];
    function updateVisualState() {
        ...
        Object.assign(
          ...
          vertical
            ? {
                ...
                transform: `translate3d(${left + tabRect.width - 2}px, ${top}px, 0)`,
              }
            : pill
              ? {
              ...
              : {
                  width: `${tabRect.width}px`,
                  height: '2px',
                  transform: `translate3d(${left}px, ${top + tabRect.height - 2}px, 0)`,
                },
        );
      }
      ...
      if (prevButton) {
        prevButton.hidden = !overflow;
        prevButton.disabled = !overflow || list.scrollLeft <= 1;
      }
      if (nextButton) {
        nextButton.hidden = !overflow;
        nextButton.disabled = !overflow || list.scrollLeft >= max - 1;
      }
    }
    ...
    list?.addEventListener('keydown', (event) => {
      ...
      if (event.key === (vertical ? 'ArrowDown' : 'ArrowRight'))
        next = (current + 1) % tabsList.length;
      else if (event.key === (vertical ? 'ArrowUp' : 'ArrowLeft'))
        next = (current - 1 + tabsList.length) % tabsList.length;
      else if (event.key === 'Home') next = 0;
      else if (event.key === 'End') next = tabsList.length - 1;
      else return;
      event.preventDefault();
      select(tabsList[next]);
      tabsList[next].focus();
    });
    prevButton?.addEventListener('click', () =>
      list?.scrollBy({ left: -Math.max(list.clientWidth * 0.75, 120), behavior: 'smooth' }),
    );
    ...
    const observer =
      typeof ResizeObserver === 'undefined'
        ? null
        : new ResizeObserver(() => requestAnimationFrame(updateVisualState));
    if (list) {
      observer?.observe(list);
      list.querySelectorAll('.tabs_tab').forEach((tab) => observer?.observe(tab));
      list.setAttribute(
        'aria-orientation',
        tabs.classList.contains('tabs_vertical') ? 'vertical' : 'horizontal',
      );
    }
