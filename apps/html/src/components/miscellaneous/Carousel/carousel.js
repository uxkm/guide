...
function initElement(element, root) {
  ...
  const slidesPerView = slidesPerViewValue === 'auto' ? 'auto' : numberAttr(element, 'data-swiper-slides-per-view', 1);
  ...
  const canLoop = wantsLoop && (effect === 'coverflow'
    ? slides.length >= (coverflowSides === 1 ? 6 : coverflowSides === 1.5 ? 8 : 10)
    : slidesPerView === 'auto'
      ? slides.length >= 4
      : slides.length >= Math.max(2, Math.ceil(Number(slidesPerView) || 1) + 1 + (element.hasAttribute('data-swiper-centered') ? 1 : 0)));
  const startIndex = normalizeInitialSlide(numberAttr(element, 'data-swiper-initial-slide', 0), slides.length, canLoop);
  function updateCoverflowLayout() {
    ...
    carousel.style.setProperty('--carousel-coverflow-slide-width', `${Math.round(slideWidth * 100) / 100}px`);
    return Math.round(effectiveGap * 100) / 100;
  }
  const resolvedSpaceBetween = updateCoverflowLayout();
  const paginationEl = element.querySelector('.swiper-pagination') ?? carousel.querySelector(':scope > .swiper-pagination');
  const thumbsSelector = element.dataset.swiperThumbs;
  const thumbsElement = thumbsSelector ? root.querySelector(thumbsSelector) ?? document.querySelector(thumbsSelector) : null;
  const config = {
    ...
    watchSlidesProgress: element.hasAttribute('data-swiper-watch-slides-progress') || carousel.classList.contains('carousel_thumbs'),
  };
  ...
  if (paginationEl) config.pagination = {
  ...
  if (element.hasAttribute('data-swiper-autoplay')) config.autoplay = {
    delay: numberAttr(element, 'data-swiper-delay', 4000), disableOnInteraction: false,
  };
  if (gridRows > 1) config.grid = { rows: gridRows, fill: element.dataset.swiperGridFill === 'column' ? 'column' : 'row' };
  ...
  if (effect === 'coverflow') Object.assign(config, {
    effect: 'coverflow',
    coverflowEffect: element.dataset.swiperCoverflowStyle === 'rise'
  ...
  if (effect === 'coverflow' && canLoop) Object.assign(config, { loopAdditionalSlides: 0, loopAddBlankSlides: false });
  ...
  window.requestAnimationFrame(() => {
    ...
    swiper.update();
    if (canLoop && typeof swiper.slideToLoop === 'function') swiper.slideToLoop(startIndex, 0, false);
  });
...
export function initCarousel(root = document) {
  ...
  elements.filter((element) => !element.hasAttribute('data-swiper-thumbs')).forEach((element) => initElement(element, root));
  elements.filter((element) => element.hasAttribute('data-swiper-thumbs')).forEach((element) => initElement(element, root));
}
