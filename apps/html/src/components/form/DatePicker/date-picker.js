...
export function initDatePicker(root = document) {
  ...
  pickers.forEach((picker) => {
    ...
    const format = (next) =>
      next
        ? new Intl.DateTimeFormat('ko-KR', {
            ...
            month: '2-digit',
            day: '2-digit',
          }).format(new Date(`${next}T00:00:00`))
        : '';
    const setOpen = (open) => {
      ...
      if (open)
        requestAnimationFrame(() =>
          panel.querySelector('.is-selected:not(:disabled), .calendar_day:not(:disabled)')?.focus(),
        );
    };
    const sync = () => {
    ...
    const commit = (next) => {
      value = next;
      sync();
      picker.dispatchEvent(new CustomEvent('change', { bubbles: true, detail: { value: next } }));
    };
    const render = () => {
      ...
      const cells = Array.from({ length: 42 }, (_, index) => {
        ...
        const next = serialize(date);
        const disabled =
          (picker.dataset.min && next < picker.dataset.min) ||
          (picker.dataset.max && next > picker.dataset.max);
        return `<button type="button" class="calendar_day${date.getMonth() !== month.getMonth() ? ' is-other-month' : ''}${next === today ? ' is-today' : ''}${next === value ? ' is-selected' : ''}" data-date="${next}"${disabled ? ' disabled' : ''}${next === today ? ' aria-current="date"' : ''}${next === value ? ' aria-selected="true"' : ''}>${date.getDate()}</button>`;
      }).join('');
    ...
    picker.addEventListener('click', (event) => {
      ...
      if (target.closest('.date_picker_btn')) {
        setOpen(!picker.classList.contains('is-open'));
        return;
      }
      if (target.closest('.date_picker_input')) {
        setOpen(true);
        return;
      }
      if (target.closest('.date_picker_clear, [data-date-clear]')) {
        commit('');
        render();
        return;
      }
      if (target.closest('[data-date-prev], [data-date-next]')) {
        month = new Date(
          month.getFullYear(),
          month.getMonth() + (target.closest('[data-date-next]') ? 1 : -1),
          1,
        );
        render();
        return;
      }
      if (target.closest('[data-date-today]')) {
        const next = serialize(new Date());
        commit(next);
        month = new Date();
        render();
        setOpen(false);
      ...
      if (day && !day.disabled) {
        ...
        month = new Date(`${day.dataset.date}T00:00:00`);
        render();
        setOpen(false);
      }
    });
