export function initModal(root = document) {
  ...
  const close = (modal, reason = 'close', sourceEvent) => {
    ...
    modal.dispatchEvent(new CustomEvent('close', { bubbles: true, detail: { reason, sourceEvent } }));
  };
  ...
  modals.forEach((modal) => {
    ...
    modal.querySelectorAll('[data-modal-close]').forEach((button) => button.addEventListener('click', (event) => close(modal, button.classList.contains('modal_backdrop') ? 'backdrop' : 'close', event)));
  });
