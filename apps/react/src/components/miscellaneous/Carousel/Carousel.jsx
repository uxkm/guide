...
export function Carousel({
  ariaLabel = '콘텐츠 슬라이드', children, className = '', style, size = 'md', multi = false,
  dotsOutside = false, autoplayToggle = false, effect, autoplay = false, delay = 4000,
  loop = false, initialSlide = 0, slidesPerView = 1, slidesPerGroup = 1, spaceBetween = 0,
  centered = false, pagination = true, navigation = true, gridRows = 1, gridFill = 'row',
  coverflowStyle = 'tilt', coverflowSides = 1.5, coverflowEffect, thumbs = null,
  thumbsControl = false, watchSlidesProgress = false, freeMode = false, onSwiper,
  ripple = true, ...props
}) {
  ...
  useEffect(() => {
    ...
    const canLoop = (() => {
      ...
      return slideCount >= Math.max(2, Math.ceil(Number(resolvedSlidesPerView) || 1) + 1 + (centered ? 1 : 0));
    })();
    const startIndex = normalizeInitialSlide(initialSlide, slideCount, canLoop);
    instanceRef.current?.destroy(true, true);
    const paginationEl = rootRef.current?.querySelector(':scope > .swiper-pagination') ?? swiperRef.current.querySelector('.swiper-pagination');
    const resolvedSpaceBetween = updateCoverflowLayout();
    const config = {
      initialSlide: startIndex, slidesPerView: resolvedSlidesPerView,
      slidesPerGroup, spaceBetween: resolvedSpaceBetween, centeredSlides: centered || isCoverflow,
      loop: canLoop,
    ...
    if (isCoverflow) Object.assign(config, {
      effect: 'coverflow',
      coverflowEffect: {
        ...(coverflowStyle === 'rise'
    ...
    if (isCoverflow && canLoop) Object.assign(config, { loopAdditionalSlides: 0, loopAddBlankSlides: false });
    ...
    const frame = window.requestAnimationFrame(() => {
      ...
      instance.update();
      if (canLoop && typeof instance.slideToLoop === 'function') instance.slideToLoop(startIndex, 0, false);
    });
    return () => {
  ...
  const classes = [
    'carousel', 'carousel_demo', multi && 'carousel_multi', dotsOutside && 'carousel_dots-outside',
    isCoverflow && 'carousel_coverflow', isCoverflow && coverflowStyle === 'rise' && 'carousel_coverflow-rise',
    isCoverflow && `carousel_coverflow-sides-${coverflowSideCount === 1.5 ? '15' : coverflowSideCount}`,
    isGrid && 'carousel_grid', pagination === 'custom' && 'carousel_pagination-custom',
    pagination === 'progressbar' && 'carousel_pagination-progress', thumbsControl && 'carousel_thumbs',
    size !== 'md' && ['sm', 'lg'].includes(size) && `carousel_${size}`,
    (autoplay || autoplayToggle) && !isPlaying && 'is-autoplay-paused', className,
  ].filter(Boolean).join(' ');
  ...
  return (
    <CarouselContext.Provider value={context}>
      <div {...props} ref={rootRef} className={classes} style={style} data-component="Carousel">
        <div ref={swiperRef} className="swiper" role="region" aria-roledescription="carousel" aria-label={ariaLabel} data-swiper="" data-swiper-effect={effect || undefined} data-swiper-space-between={spaceBetween || undefined}>
          <div className="swiper-wrapper">{children}</div>
          {showNavigation ? <div className="swiper-button-prev" aria-label="이전 슬라이드" data-ripple={ripple ? 'surface' : 'false'} /> : null}
          {showNavigation ? <div className="swiper-button-next" aria-label="다음 슬라이드" data-ripple={ripple ? 'surface' : 'false'} /> : null}
          {showPagination && !dotsOutside ? <div className="swiper-pagination" /> : null}
        </div>
        {showPagination && dotsOutside ? <div className="swiper-pagination" /> : null}
        {autoplay || autoplayToggle ? <button type="button" className="carousel_toggle" data-carousel-autoplay-toggle="" data-ripple={ripple ? 'surface' : 'false'} aria-label={isPlaying ? '슬라이드 정지' : '슬라이드 재생'} aria-pressed={isPlaying} onClick={toggleAutoplay}><svg className="carousel_toggle-icon carousel_toggle-icon-pause" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><rect x="6" y="5" width="4" height="14" rx="1" /><rect x="14" y="5" width="4" height="14" rx="1" /></svg><svg className="carousel_toggle-icon carousel_toggle-icon-play" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M8 5v14l11-7z" /></svg></button> : null}
      </div>
