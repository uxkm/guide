...
export function Popover({
  id, placement = 'bottom', size = 'md', offset = 'md', open, defaultOpen = false,
  offsetTop, offsetRight, offsetBottom, offsetLeft,
  trigger = 'click', triggerContent, title, panelLabel, footer, children = 'Popover',
  noArrow = false, closable, disabled = false, className = '', onOpenChange, closeLabel = '닫기',
  panelAlign = 'start', arrowAnchor = 'content', arrowTargetAlign = 'center', ...props
}) {
  ...
  useEffect(() => {
    ...
    const closeOutside = (event) => {
      if (!triggerRef.current?.contains(event.target) && !panelRef.current?.contains(event.target)) setVisible(false, 'outside', event);
    };
  ...
  useEffect(() => {
    if (!visible || !anchor || !panelRef.current || !['target', 'mixed'].includes(arrowAnchor)) return;
