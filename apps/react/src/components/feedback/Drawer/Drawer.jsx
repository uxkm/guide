...
export function Drawer({
  id, title, size = 'md', placement = 'right', backdrop = true, noBackdrop = false,
  open, defaultOpen = false, openOnLoad = false, draggable = false,
  footerAlign = 'end', footerRatio = '1-1', footerNoPadBottom = false,
  header, extra, footer, children = 'Drawer', className = '', closeLabel = '닫기',
  onClose, ...props
}) {
  ...
  const endDrag = (event) => {
    ...
    if (currentHeight < closeThreshold || (!startedExpanded && state.deltaY < -state.collapsed * 0.4)) {
  ...
  return createPortal(
    <div {...props} ref={rootRef} id={drawerId} className={rootClasses} data-component="Drawer" data-drawer="" data-drawer-draggable={showDragHandle ? 'true' : undefined} data-drawer-backdrop={backdrop && !noBackdrop ? undefined : 'false'} role="dialog" aria-modal="true" aria-labelledby={title || header ? titleId : undefined} tabIndex={-1}>
      <div className="drawer_backdrop" aria-hidden="true" onClick={(event) => backdrop && !noBackdrop && requestClose('backdrop', event)} />
      <div ref={panelRef} className={panelClasses}>
        {showDragHandle && <div className="drawer_handle" data-drawer-drag-handle="" aria-hidden="true" onPointerDown={startDrag} onPointerMove={moveDrag} onPointerUp={endDrag} onPointerCancel={endDrag}><span className="drawer_handle-bar" /></div>}
        {(title || header) && <div className="drawer_header" onPointerDown={showDragHandle ? startDrag : undefined} onPointerMove={showDragHandle ? moveDrag : undefined} onPointerUp={showDragHandle ? endDrag : undefined} onPointerCancel={showDragHandle ? endDrag : undefined}>{header ?? <h2 className="drawer_title" id={titleId}>{title}</h2>}{extra && <div className="drawer_extra">{extra}</div>}<Button variant="ghost" iconOnly className="drawer_close" ariaLabel={closeLabel} iconBefore={<Icon name="close" size="sm" className="drawer_close-icon" />} onClick={(event) => requestClose('close', event)} /></div>}
