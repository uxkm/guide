...
export function Navbar({ brand, size = 'md', borderless = false, dark = false, sticky = false, responsive = false, collapseId, brandContent, brandIcon, items, search, actions, children, className = '', ...props }) {
  ...
  return <header {...props} className={classes} data-component="Navbar" data-navbar={responsive || undefined}><div className="navbar_container"><a href="#" className="navbar_brand" onClick={(event) => event.preventDefault()}>{brandContent ?? <>{brandIcon}{brand}</>}</a>{responsive && <Button variant="ghost" iconOnly className="navbar_toggle" ariaLabel={open ? '메뉴 닫기' : '메뉴 열기'} aria-expanded={open} aria-controls={targetId} onClick={() => setOpen((value) => !value)} iconBefore={<><Icon name="menu" size="sm" className="navbar_toggle-icon-open" /><Icon name="close" size="sm" className="navbar_toggle-icon-close" /></>} />}<div className="navbar_collapse" id={targetId}><nav className="navbar_nav" aria-label="주요 메뉴">{navItems}</nav>{search && <div className="navbar_search">{search}</div>}{actions && <div className="navbar_actions">{actions}</div>}</div></div></header>;
}
