...
export function initDrawer(root = document) {
    ...
    drawer
      ...
      .forEach((button) =>
        button.addEventListener('click', (event) =>
          close(drawer, button.classList.contains('drawer_backdrop') ? 'backdrop' : 'close', event),
        ),
      );
    if (drawer.dataset.drawerOpenOnLoad === 'true' || drawer.classList.contains('is-open'))
      open(drawer);
  });
  ...
  root.addEventListener(
    ...
    (event) => {
      ...
      if (
        ...
        event.target.closest('.drawer_close, .drawer_extra, a, input, textarea, select')
      )
        return;
      ...
      const collapsed =
        panel.classList.contains('is-expanded') && stored > 0
          ? stored
      ...
      dragState = {
        drawer,
        panel,
        target: event.target.closest('[data-drawer-drag-handle], .drawer_header'),
        ...
        expanded: Math.max(
          collapsed + 48,
          (panel.ownerDocument.defaultView?.innerHeight || collapsed) * 0.9,
        ),
        deltaY: 0,
      };
  ...
  const endDrag = (event) => {
    ...
    if (
      height < state.collapsed * 0.55 ||
      (!startedExpanded && state.deltaY < -state.collapsed * 0.4)
    ) {
      state.panel.classList.remove('is-expanded');
      close(state.drawer, 'drag', event);
    } else if (height >= threshold || (!startedExpanded && state.deltaY > 48))
      state.panel.classList.add('is-expanded');
