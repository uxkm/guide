...
export function initDropdown(root = document) {
  ...
  groups.forEach((dropdown) => {
    ...
    const trigger = dropdown.querySelector(':scope > .dropdown_trigger');
    const menu = dropdown.querySelector(':scope > .dropdown_menu');
    if (!trigger || !menu) return;
    function items() {
      return [...menu.querySelectorAll('.menu_link:not(.is-disabled):not([aria-disabled="true"])')];
    }
    function setOpen(open, focus = false) {
      if (dropdown.classList.contains('is-disabled') || trigger.disabled) return;
    ...
    menu.addEventListener('click', (event) => {
      ...
      if (dropdown.dataset.dropdownCloseOnSelect !== 'false') {
        setOpen(false);
        trigger.focus();
      }
    });
    ...
    menu.setAttribute('aria-hidden', String(!dropdown.classList.contains('is-open')));
