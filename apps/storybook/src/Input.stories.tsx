...
const PREFIX_OPTIONS = ['', '@', 'https://', '₩', '+82', 'search'] as const;
const SUFFIX_OPTIONS = ['', '원', 'KRW'] as const;

function resolveAddon(token?: string, kind: 'prefix' | 'suffix' = 'prefix') {
  if (!token) return undefined;
  if (kind === 'prefix' && token === 'search') return <Icon name="search" />;
  return token;
}

function inputPropsFromArgs(args: Record<string, unknown>) {
  ...
  return {
    ...
    prefix: resolveAddon(typeof args.prefix === 'string' ? args.prefix : undefined, 'prefix'),
    suffix: resolveAddon(typeof args.suffix === 'string' ? args.suffix : undefined, 'suffix'),
...
function InputPlayground({
  ...
  return (
    <Input
      {...inputPropsFromArgs(args)}
      id="input-playground"
      value={value}
      onChange={(event: React.ChangeEvent<HTMLInputElement>) => setValueState(event.target.value)}
...
const meta = {
  ...
  parameters: {
    layout: 'padded',
    controls: { include: [...inputControlKeys] },
  ...
  argTypes: {
    ...
    maxLength: {
      ...numberControlArg,
      description: '최대 입력 길이. 0이면 제한 없음',
    },
    prefix: {
      control: 'select',
      options: [...PREFIX_OPTIONS],
      type: 'string',
      description: '앞쪽 애드온 (텍스트·아이콘)',
      labels: { '': '없음', search: '검색 아이콘' },
    },
    suffix: {
      control: 'select',
...
export const Playground: Story = {
  ...
  render: (args, { updateArgs }) => (
    <Demo>
      <InputPlayground args={args} updateArgs={updateArgs} />
    </Demo>
  ),
};

export const Basic: Story = { name: '기본', render: (args) => <Demo><Field id="input-name" label="이름" hint="2~20자 이내로 입력해 주세요."><ButtonClearable id="input-name" placeholder="이름을 입력하세요" /></Field></Demo> };

export const Standalone: Story = { name: '단독 사용', render: (args) => <Demo>
...
export const Type: Story = { name: '입력 타입', render: (args) => <Demo>
...
export const Kind: Story = { name: '입력 종류', render: (args) => <Demo>
...
export const Size: Story = { name: '크기', render: (args) => <Demo>{(['sm', 'md', 'lg'] as const).map((size) => <Field key={size} id={`input-${size}`} label={size.toUpperCase()}><ButtonClearable id={`input-${size}`} size={size} placeholder={size === 'md' ? '기본 크기' : `input_${size}`} /></Field>)}</Demo> };

export const Width: Story = { name: '너비', render: (args) => <Demo>
...
export const Group: Story = { name: 'Input Group', render: (args) => <Demo><ButtonClearable label="도메인" placeholder="example.com" prefix="https://" /><ButtonClearable label="사용자명" placeholder="username" prefix="@" /><ButtonClearable label="결제 금액" inputMode="numeric" placeholder="0" prefix="₩" suffix="KRW" /></Demo> };

export const Required: Story = { name: '필수 필드', render: (args) => <Demo><div className="form_field form_field_fit"><label className="form_field-label" htmlFor="required-email">이메일<span className="form_field-required"><span aria-hidden="true">*</span><span className="form_field-required-text">필수 항목</span></span></label><ButtonClearable id="required-email" type="email" placeholder="name@example.com" required aria-required="true" /><p className="form_field-hint">업무용 이메일을 입력해 주세요.</p></div></Demo> };

export const State: Story = { name: '상태', render: (args) => <Demo><Field id="state-default" label="기본"><ButtonClearable id="state-default" placeholder="입력 가능" /></Field><Field id="state-disabled" label="비활성"><ButtonClearable id="state-disabled" defaultValue="수정할 수 없음" disabled /></Field><Field id="state-readonly" label="읽기 전용"><ButtonClearable id="state-readonly" defaultValue="읽기만 가능" readOnly /></Field><div className="form_field"><label className="form_field-label" htmlFor="state-error">에러</label><ButtonClearable id="state-error" error defaultValue="invalid-email" aria-describedby="state-error-message" /><p className="form_field-error" id="state-error-message" role="alert">올바른 이메일 형식이 아닙니다.</p></div><div className="form_field"><label className="form_field-label" htmlFor="state-success">성공</label><ButtonClearable id="state-success" className="is-success" defaultValue="uxkm_user" aria-describedby="state-success-message" /><p className="form_field-success" id="state-success-message" role="status">사용 가능한 아이디입니다.</p></div></Demo> };
...
export const Clearable: Story = { name: '입력 삭제', render: (args) => <ClearableDemo /> };

export const Example: Story = { name: '조합 예시', render: (args) => <form className="form form_vertical form_fit form_compact" onSubmit={(event) => event.preventDefault()}><div className="alert color_danger" role="alert"><strong>입력 내용을 확인해 주세요</strong></div><div className="form_field"><label className="form_field-label" htmlFor="login-id">아이디</label><ButtonClearable id="login-id" className="is-success" defaultValue="uxkm_user" aria-invalid="false" aria-describedby="login-id-success" /><p id="login-id-success" className="form_field-success" role="status">사용 가능한 아이디입니다.</p></div><div className="form_field"><label className="form_field-label" htmlFor="login-password">비밀번호</label><ButtonClearable id="login-password" type="password" error defaultValue="1234" aria-describedby="login-password-error" /><p id="login-password-error" className="form_field-error" role="alert">8자 이상 입력해 주세요.</p></div><div className="form_actions"><Button type="submit" label="로그인" /><Button variant="ghost" label="취소" /></div></form> };
