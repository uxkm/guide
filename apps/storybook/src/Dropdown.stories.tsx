...
const MenuShell = ({ children, ariaLabel = '메뉴' }: any) => (
  <Menu compact ariaLabel={ariaLabel}>
    {children}
  </Menu>
);
...
const Trigger = ({
  ...
  <Button
    ...
    color={color}
    label={label}
    iconAfter={iconAfter ? <Icon name="chevron-down" /> : undefined}
    {...props}
  />
);

function SelectDropdown({ sort = false }: any) {
  const options = sort
    ? ['최신순', '인기순', '가격 낮은순', '가격 높은순']
  ...
  return (
    <Dropdown
      fit={!sort}
      onSelect={setValue}
      triggerContent={<Button variant="select" selectText={sort} label={value} />}
    >
      <MenuShell ariaLabel={sort ? '정렬' : '지역 선택'}>
        {options.map((option) => (
          <MenuItem key={option} label={option} active={option === value} />
        ))}
      </MenuShell>
    </Dropdown>
  );
...
/** Storybook Controls 값을 Dropdown prop으로 정리합니다. */
function mapDropdownArgs(args: Record<string, unknown>, options: { uncontrolled?: boolean } = {}) {
  const {
    open,
    maxVisibleItems,
    menuWidth,
    ...
    onSelect: _onSelect,
    triggerContent: _triggerContent,
    ...rest
  } = args;
  const visibleItems = toPositiveNumber(maxVisibleItems);
  const width = toPositiveNumber(menuWidth);
  const minWidth = toPositiveNumber(menuMinWidth);

  return {
    ...rest,
    ...(options.uncontrolled || open === undefined ? {} : { open: Boolean(open) }),
    ...(visibleItems != null ? { maxVisibleItems: visibleItems } : {}),
    ...(width != null ? { menuWidth: width } : {}),
    ...(minWidth != null ? { menuMinWidth: minWidth } : {}),
  };
}
...
function DropdownPlayground({
  ...
  const { open: argsOpen, triggerLabel = '작업', onOpenChange: _onOpenChange, ...rest } = args;
  const [open, setOpen] = useState(Boolean(argsOpen));

  useEffect(() => {
    setOpen(Boolean(argsOpen));
  }, [argsOpen]);
  ...
  return (
    <Dropdown
      {...mapDropdownArgs(rest)}
      open={open}
      onOpenChange={setOpenState}
      triggerContent={<Trigger label={String(triggerLabel)} />}
    >
      <MenuShell ariaLabel="도시 목록">
        {playgroundItems.map((label, index) => (
          <MenuItem key={label} label={label} active={index === 0} />
        ))}
      </MenuShell>
    </Dropdown>
  );
...
const meta = {
  title: '네비게이션/Dropdown',
  component: Dropdown,
  decorators: [(Story) => <div className="dropdown-story-demo"><Story /></div>],
  parameters: {
    layout: 'padded',
    controls: { include: [...dropdownControlKeys] },
  ...
  argTypes: {
    ...dropdownArgTypes,
    menuWidth: { control: 'number', type: 'number', description: '메뉴 너비 (rem). 0이면 미적용' },
  },
...
export const Playground: Story = {
  ...
  render: (args, { updateArgs }) => <DropdownPlayground args={args} updateArgs={updateArgs} />,
};

export const Basic: Story = {
  name: '기본',
  render: (args) => (
    <Dropdown {...mapDropdownArgs(args, { uncontrolled: true })} triggerContent={<Trigger label="작업" />}>
...
export const Open: Story = {
  name: '열린 상태',
  render: () => (
    <Dropdown open triggerContent={<Trigger label="카테고리" variant="filled" color="primary" />}>
...
export const TriggerDemo: Story = {
  ...
  render: () => (
    <div className="dropdown_row">
      ...
      <Dropdown
        triggerContent={
          <Button variant="ghost" iconOnly ariaLabel="더보기" iconBefore={<Icon name="menu" />} />
        }
      >
...
export const SelectTrigger: Story = {
  name: '셀렉트 트리거',
  render: () => (
    <div className="dropdown-stack-demo">
      <div className="dropdown_row">
        <SelectDropdown />
        <SelectDropdown sort />
      </div>
...
export const Placement: Story = {
  name: '정렬',
  render: () => (
    <div className="dropdown-placement-demo">
...
export const MenuDemo: Story = {
  name: '메뉴 구성',
  render: () => (
    <Dropdown open triggerContent={<Trigger label="계정" />}>
      <MenuShell ariaLabel="계정 메뉴">
        ...
        <MenuItem
          label="알림"
          icon={<Icon name="bell" className="menu_icon" />}
          extra={<Badge count size="sm" label="3" />}
        />
  ...
  render: () => (
    <div className="dropdown_demo-toolbar">
      ...
      <Dropdown
        placement="end"
        triggerContent={
          <Button variant="ghost" size="sm" iconAfter={<Icon name="chevron-down" />}>
