...
const gulpImport = `{% from "components/miscellaneous/BackTop/back-top.njk" import backTop %}`;
function gulp(item: Options, index: number) {
  const id = item.target?.slice(1) || `page-${index}`;
  const args = [item.target && `target='${item.target}'`, item.visibilityHeight !== undefined && `visibilityHeight=${item.visibilityHeight}`, item.size && `size='${item.size}'`, item.color && `color='${item.color}'`, item.label && `label='${item.label}'`].filter(Boolean).join(', ');
  return `${gulpImport}

${item.target ? `<div id="${id}" class="back_top_demo-scroll">
...
function examples(name: Name): FrameworkExample[] {
  ...
  return [{ id: 'html', label: 'HTML', fileName: `BackTop.html · ${name}`, code: markup }, { id: 'gulp', label: 'Gulp', fileName: `back-top.njk · ${name}`, code: list.map(gulp).join('\n\n') }, { id: 'vue', label: 'Vue', fileName: `@uxkm/vue/back-top · ${name}`, code: vue }, { id: 'nuxt', label: 'Nuxt', fileName: `@uxkm/vue/back-top · ${name}`, code: vue }, { id: 'react', label: 'React', fileName: `@uxkm/react/back-top · ${name}`, code: react }, { id: 'next', label: 'Next', fileName: `@uxkm/react/back-top · ${name}`, code: react }];
}
