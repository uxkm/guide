...
type ButtonStoryArgs = {
  ...
  role?: string;
  ariaLabel?: string;
  haspopup?: 'none' | 'true' | 'listbox' | 'menu' | 'dialog';
  grow?: 'false' | 'true' | '2';
  ripple?: boolean;
  iconOnly?: boolean;
  vertical?: boolean;
...
function buttonPropsFromArgs(args: Record<string, unknown>) {
  ...
  const haspopupRaw = args.haspopup;
  const haspopup =
    haspopupRaw === 'true'
      ? true
      : haspopupRaw === 'listbox' || haspopupRaw === 'menu' || haspopupRaw === 'dialog'
        ? haspopupRaw
        : undefined;

  const grow =
    args.grow === 'true' ? true : args.grow === '2' ? '2' : false;
  ...
  return {
    ...
    selectText: args.selectText === true,
    selectCaret: args.selectCaret === true,
    disabled: args.disabled === true,
    ariaDisabled: args.ariaDisabled === true,
    ...
    ...(role ? { role } : {}),
    ...(ariaLabel ? { ariaLabel } : {}),
    ...(haspopup !== undefined ? { haspopup } : {}),
  };
}

const buttonControlKeys = [
  'label',
  'variant',
  'color',
  'size',
...
const meta = {
  ...
  parameters: {
    layout: 'fullscreen',
    controls: { include: [...buttonControlKeys] },
  ...
  argTypes: {
    ...
    href: { ...stringControlArg, description: 'tag=a일 때 링크 주소' },
    role: { ...stringControlArg, description: '명시적 role' },
    ariaLabel: { ...ariaLabelArg, description: '아이콘 전용 등 접근성 이름' },
    haspopup: {
      control: 'select',
      options: ['none', 'true', 'listbox', 'menu', 'dialog'],
      type: 'string',
      description: 'aria-haspopup',
    },
    grow: {
      control: 'select',
      options: ['false', 'true', '2'],
      labels: { false: '없음', true: 'grow', '2': 'grow ×2' },
      type: 'string',
      description: 'flex grow',
    },
    ripple: { ...booleanControlArg, description: '클릭 리플 효과' },
    iconOnly: { ...booleanControlArg, description: '아이콘만 표시' },
    vertical: { ...booleanControlArg, description: '아이콘 위·텍스트 아래' },
    round: { ...booleanControlArg, description: 'pill 형태' },
    block: { ...booleanControlArg, description: '전체 너비' },
    fit: { ...booleanControlArg, description: '콘텐츠 너비 유지' },
    loading: { ...booleanControlArg, description: '로딩 스피너' },
    open: { ...booleanControlArg, description: '셀렉트·팝오버 열림' },
    error: { ...booleanControlArg, description: '오류 상태' },
    placeholder: { ...booleanControlArg, description: '셀렉트 플레이스홀더' },
    selectText: { ...booleanControlArg, description: '셀렉트 텍스트형' },
    selectCaret: { ...booleanControlArg, description: '캐럿 강제 표시' },
    disabled: { ...booleanControlArg, description: '네이티브 disabled' },
...
export const IconOnlySize: Story = {
  name: '아이콘 버튼 크기',
  render: () => (
    <Demo>
      <Row>
...
export const ColorVariants: Story = {
  ...
  render: () => (
    <Demo>
      <Row>
        {(['success', 'danger', 'warning'] as const).map((color) => (
          <B
            key={color}
                        color={color}
      ...
      <Row>
        {(['sm', 'md', 'lg'] as const).map((size) => (
          <B
            key={size}
                        size={size}
            label={size === 'sm' ? 'Small' : size === 'lg' ? 'Large' : 'Medium'}
          />
        ))}
      </Row>
    </Demo>
