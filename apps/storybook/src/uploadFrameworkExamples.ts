...
const sources: Record<string, Source> = {
  ...
  buttonLabel="업로드"
  defaultFiles={[
    { name: '프로필-1.svg', size: 245760, type: 'image/svg+xml', status: 'done', url: '/images/avatar-sample.svg' },
    { name: '프로필-2.svg', size: 245760, type: 'image/svg+xml', status: 'done', url: '/images/avatar-sample.svg' },
  ]}
/>`, vue: `<Upload
  variant="picture-card"
  multiple
...
/>` },
  avatar: { html: `<div class="upload" data-component="Upload"><label class="upload_avatar" for="upload-avatar">${input('upload-avatar', ' accept="image/*"')}<span class="avatar avatar_xl"><img class="avatar_image" src="/images/avatar-sample.svg" alt="프로필 사진"></span><span class="upload_avatar-overlay" aria-hidden="true"><svg class="icon icon_lg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 12v8a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-8"></path><path d="m16 6-4-4-4 4M12 2v13"></path></svg></span></label></div>`, react: '<Upload variant="avatar" accept="image/*" avatarSrc="/images/avatar-sample.svg" avatarAlt="프로필 사진" />', vue: '<Upload variant="avatar" accept="image/*" avatar-src="/images/avatar-sample.svg" avatar-alt="프로필 사진" />' },
