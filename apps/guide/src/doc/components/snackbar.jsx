...
import SnackbarPlayground from '@uxkm/ui-react/components/SnackbarPlayground.jsx';
...
const basicCode = `import Snackbar from '@uxkm/ui-react/components/Snackbar.jsx';
...
const placementCode = `import SnackbarPlayground from '@uxkm/ui-react/components/SnackbarPlayground.jsx';
...
const simpleCode = `import Snackbar from '@uxkm/ui-react/components/Snackbar.jsx';

export function SimpleExample() {
  return <Snackbar showIcon={false}>인터넷 연결이 복구되었습니다.</Snackbar>;
}`;

const shapeCode = `import Button from '@uxkm/ui-react/components/Button.jsx';
import Snackbar from '@uxkm/ui-react/components/Snackbar.jsx';
...
const motionCode = `import Snackbar from '@uxkm/ui-react/components/Snackbar.jsx';
...
const sizeCode = `import Snackbar from '@uxkm/ui-react/components/Snackbar.jsx';
...
const actionCode = `import Button from '@uxkm/ui-react/components/Button.jsx';
import Snackbar from '@uxkm/ui-react/components/Snackbar.jsx';
...
const closeCode = `import Snackbar from '@uxkm/ui-react/components/Snackbar.jsx';
...
const durationCode = `import Snackbar from '@uxkm/ui-react/components/Snackbar.jsx';
  ...
  return (
    <>
      <div className="page_intro">
        <h1>Snackbar</h1>
        <p className="lead">작업 결과나 짧은 상태 메시지를 일시적으로 전달하는 컴포넌트입니다.</p>
      </div>
      ...
      <DemoSection
        ...
        description="8개 위치는 본문이 아닌 브라우저 뷰포트를 기준으로 계산됩니다. 처음 로드할 때 하단 가운데 Snackbar를 호출하며, 같은 위치에 여러 개가 생성되면 독립적으로 쌓입니다."
        code={placementCode}
      >
        <SnackbarPlayground
          openOnLoad
      ...
      <DemoSection
        ...
        title="닫기 가능"
        description="닫기 버튼은 onClose에 close 사유와 클릭 이벤트를 전달합니다. 내부 포커스 상태에서는 Esc로도 닫을 수 있습니다."
        code={closeCode}
      >
        <Snackbar
          closable
      ...
      <GuideSection
        ...
        title="접근성"
        description="포커스를 강제로 이동하지 않고 라이브 영역으로 상태 변화를 전달합니다."
      >
        <ul className="typo_list">
          <li>
            일반적인 작업 결과는 <code className="typo_code">role=&quot;status&quot;</code>,
            즉시 확인해야 하는 오류는 <code className="typo_code">role=&quot;alert&quot;</code>를 사용합니다.
          </li>
          <li>자동 닫기는 기본적으로 비활성화되어 있습니다. 사용하는 경우 내용을 읽고 액션을 수행할 충분한 시간을 제공하세요.</li>
          <li>호버 또는 내부 포커스 중에는 자동 닫기 타이머가 멈추며, 닫기 가능한 Snackbar는 Esc로도 닫을 수 있습니다.</li>
          <li>운영체제의 모션 감소 설정에서는 Fade·Slide 시간을 최소화합니다.</li>
          <li>아이콘과 색상에만 의존하지 말고 메시지 텍스트로 상태를 설명하세요.</li>
      ...
      <ApiSection headingId="api-region-heading" title="API · SnackbarRegion">
        <ApiTable columns={snackbarPropColumns} rows={snackbarRegionProps} codeColumn="name" />
      </ApiSection>
