...
export const TreeNode = component$(({
  label, // 노드 레이블입니다.
  ...
  const isExpanded = useSignal(expanded !== false);
  useVisibleTask$(({ track, cleanup }) => {
    track(() => expanded);

    if (expanded !== undefined) isExpanded.value = expanded !== false;
  ...
  const labelContent = labelSlot ?? <span class="tree_label">{label}</span>;
  const metaContent = metaSlot ?? meta;
  // treeitem·group 관계와 aria-expanded를 실제 하위 목록 표시 상태에 맞춥니다.
  return (
    <li
      {...props}
      class={['tree_item', className].filter(Boolean).join(' ')}
      data-component="TreeNode"
      role="treeitem"
      aria-expanded={showToggle ? isExpanded.value : undefined}
    >
      <div
        class={['tree_row', selected && 'is-selected', disabled && 'is-disabled']
          .filter(Boolean)
          .join(' ')}
      >
        {showToggle ? (
          <button
            type="button"
            class={['tree_toggle', plusToggle && 'tree_toggle-plus'].filter(Boolean).join(' ')}
          ...
          <span class="tree_toggle tree_toggle_placeholder" aria-hidden="true" />
        )}
        {prefix}
        {icon ? (
        ...
        {link ? (
          <button type="button" class="tree_link" disabled={disabled || undefined}>
            {labelContent}
          </button>
        ) : (
          labelContent
        ...
        {metaContent != null && metaContent !== '' ? (
          <span class="tree_meta">{metaContent}</span>
        ) : null}
      </div>
      {hasChildren ? (
        <ul class="tree" role="group" hidden={!isExpanded.value}>
          {children}
        </ul>
      ) : null}
    </li>
