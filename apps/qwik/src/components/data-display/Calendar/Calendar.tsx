...
export const CalendarHeader = component$(({
  ...
  return (
    <div class={cx('calendar_header', className)} {...props}>
      {prev ? (
        <button type="button" class="btn btn_ghost btn_icon-only btn_sm" aria-label={prevLabel}>
          ‹
        </button>
      ) : null}
      <span class="calendar_title">{title}</span>
      {next ? (
        <button type="button" class="btn btn_ghost btn_icon-only btn_sm" aria-label={nextLabel}>
          ›
        </button>
      ) : null}
      {children}
...
export const CalendarNav = component$(({
  ...
  return (
    <div class={cx('calendar_nav', className)}>
      <button type="button" class="btn btn_ghost btn_icon-only btn_sm" aria-label={prevLabel}>
        ‹
      </button>
...
export const CalendarWeekdays = component$(({
  labels = WEEKDAYS, // 요일 레이블 배열입니다.
  class: className = '', // 공통 클래스와 함께 적용할 사용자 정의 클래스입니다.
}) {
  return (
    <div class={cx('calendar_weekdays', className)} aria-hidden="true">
      {labels.map((label) => (
        <span key={label} class="calendar_weekday">
          {label}
        </span>
      ))}
    </div>
...
export const CalendarDay = component$(({
  day, // 날짜 숫자입니다.
...
export const CalendarGrid = component$(({
  children, // 날짜 그리드 콘텐츠입니다.
  week = false, // 주 단위 보기입니다.
  class: className = '', // 추가 클래스입니다.
}) {
  return (
    <div class={cx('calendar_grid', week && 'calendar_grid-week', className)}>{children}</div>
  );
});

export const CalendarMonth = component$(({
  ...
  const resolved = days?.length ? days : monthDays({ partial, selected, today, events, rangeStart, rangeEnd }),
...
export const CalendarFooter = component$(({
  children, // 기본 액션 대신 사용할 푸터 콘텐츠입니다.
  class: className = '', // 공통 클래스와 함께 적용할 사용자 정의 클래스입니다.
}) {
  return (
    <div class={cx('calendar_footer', className)}>
...
export const CalendarGroup = component$(({
  children, // 그룹에 배치할 달력들입니다.
  class: className = '', // 공통 클래스와 함께 적용할 사용자 정의 클래스입니다.
}) {
  return <div class={cx('calendar_group', className)}>{children}</div>;
});

export const CalendarWheel = component$(({
  title, // 휠 툴바 제목입니다.
  shadow, // 그림자 스킨 여부입니다.
  ...
  return (
    <Calendar wheel shadow={shadow} borderless={borderless} role="group" ariaLabel={ariaLabel}>
      {showToolbar ? (
        <div class="calendar_wheel-toolbar">
          {toolbarContent ?? (
            <>
              {cancelLabel ? (
                <button class="btn btn_text btn_sm" type="button">
                  {cancelLabel}
                </button>
              ) : null}
              <span class="calendar_wheel-title">{title}</span>
              {confirmLabel ? (
                <button class="btn btn_text color_primary btn_sm" type="button">
                  {confirmLabel}
                </button>
              ) : null}
            </>
      ...
      <div class="calendar_wheel-body">
        ...
        <div class="calendar_wheel-columns">{children}</div>
        <div class="calendar_wheel-fade" aria-hidden="true" />
      </div>
      {footer || footerContent ? (
        <div class="calendar_wheel-footer">{footerContent}</div>
      ) : null}
    </Calendar>
  );
});

export const CalendarWheelColumn = component$(({
  ...
  const active = useSignal(selected);
  const listRef = useSignal<HTMLElement | undefined>(undefined);
  const index = Math.max(
    0,
    items.findIndex((item) => String(item) === String(active.value)),
  );
  const move = (offset) =>
    active.value = items[Math.max(0, Math.min(items.length - 1, index + offset)]);
  ...
  return (
    <div class="calendar_wheel-column">
      {showSteps ? (
        <button
          type="button"
          class="btn btn_ghost btn_icon-only btn_sm calendar_wheel-step calendar_wheel-step-prev"
      ...
      <ul ref={listRef} class="calendar_wheel-list" role="listbox" aria-label={label}>
        {items.map((item) => (
          <li key={String(item)} role="presentation">
            <button
              ...
              class={cx(
                'calendar_wheel-item',
                String(item) === String(active.value) && 'is-selected',
              )}
              aria-selected={String(item) === String(active.value) || undefined}
      ...
      {showSteps ? (
        <button
          type="button"
          class="btn btn_ghost btn_icon-only btn_sm calendar_wheel-step calendar_wheel-step-next"
