...
import { SnackbarPlacementContext, snackbarPlacements } from './SnackbarRegion.tsx';
...
export const Snackbar = component$(({
  ...
  const timerRef = useSignal<HTMLElement | undefined>(undefined); // 자동 닫기 타이머 ID입니다.
  const startedAtRef = useSignal<HTMLElement | undefined>(undefined); // 타이머 시작 시각입니다.
  const remainingRef = useSignal<HTMLElement | undefined>(undefined); // 남은 자동 닫기 시간(ms)입니다.
  const closeRef = useSignal<HTMLElement | undefined>(undefined); // 퇴장 완료 후 전달할 닫기 사유·이벤트입니다.
  const pauseReasonsRef = useSignal(new Set()); // hover·focus 등 일시정지 사유입니다.
  const visible = useSignal(true); // DOM 유지 여부입니다.
  // none이면 바로 open, 그 외에는 entering부터 시작합니다.
  const phase = useSignal(motion === 'none' ? 'open' : 'entering');
  ...
  const clearTimer = () => {
    if (timerRef.value !== null) window.clearTimeout(timerRef.value);
    timerRef.value = null;
  };
  ...
  const dismiss = (reason, event) => {
    ...
    clearTimer();
    closeRef.value = { reason, event };
    if (resolvedMotion === 'none') finishClose();
    else phase.value = 'leaving';
  };
  // 일시정지 사유가 없을 때만 남은 시간으로 타이머를 재개합니다.
  const startTimer = () => {
    if (remainingRef.value <= 0 || timerRef.value !== null || pauseReasonsRef.value.size > 0)
      return;
    startedAtRef.value = Date.now();
    timerRef.value = window.setTimeout(() => dismiss('timeout'), remainingRef.value);
  };
  // 호버·포커스 중에는 남은 시간을 줄이고 타이머를 멈춥니다.
  const pauseTimer = (reason) => {
    pauseReasonsRef.value.add(reason);
    if (timerRef.value === null) return;
    remainingRef.value = Math.max(0, remainingRef.value - (Date.now() - startedAtRef.value));
    clearTimer();
  };
  // 일시정지 사유를 제거하고 타이머를 재개합니다.
  const resumeTimer = (reason) => {
    pauseReasonsRef.value.delete(reason);
    startTimer();
  };
  ...
  useVisibleTask$(({ track, cleanup }) => {
    ...
    if (phase.value !== 'entering') return undefined;
    const id = window.setTimeout(() => phase.value = 'open', 250);
  ...
  useVisibleTask$(({ track, cleanup }) => {
    ...
    if (phase.value !== 'open' || resolvedDuration === 0) return undefined;
    remainingRef.value = resolvedDuration;
    pauseReasonsRef.value.clear();
    startTimer();
    return clearTimer;
  
  });
  // 언마운트 시 타이머를 정리합니다.
  useVisibleTask$(({ track }) => clearTimer);
    ...
    <div
      ...
      onAnimationEnd={(event) => {
        ...
        if (event.currentTarget === event.target) {
          ...
          if (phase.value === 'leaving') finishClose();
        }
        onAnimationEnd?.(event);
      }}
    >
