...
export const Tooltip = component$(({
  ...
  const bubbleRef = useSignal<HTMLElement | undefined>(undefined);
  const hoverTimer = useSignal<HTMLElement | undefined>(undefined); // hover 닫기 지연 타이머입니다.
  const internalOpen = useSignal(defaultOpen);
  const anchor = useSignal(null); // 트리거 뷰포트 좌표입니다.
  const arrowPosition = useSignal(null); // 측정된 화살표 위치입니다.
  ...
  const updatePosition = () => {
    ...
    if (element) anchor.value = viewportRect(element);
  };
  const cancelClose = () => {
  ...
  const scheduleClose = (event) => {
    cancelClose();
    hoverTimer.value = setTimeout(() => setVisible(false, 'hover', event), 100);
  };
  ...
  useVisibleTask$(({ track, cleanup }) => {
    ...
    const outside = (event) => {
      if (!triggerRef.value?.contains(event.target) && !bubbleRef.value?.contains(event.target))
  ...
  useVisibleTask$(({ track }) => () => cancelClose());
  ...
  useVisibleTask$(({ track, cleanup }) => {
    ...
    if (!visible || !anchor.value || !bubbleRef.value || !['target', 'mixed'].includes(arrowAnchor))
      return;
      ...
      if (side === 'top' || side === 'bottom') {
        const left =
          panelAlign === 'start'
            ? 0
            : panelAlign === 'end'
              ? anchor.value.width - bubbleRect.width
              : (anchor.value.width - bubbleRect.width) / 2;
        ...
        const top =
          panelAlign === 'start'
            ? 0
            : panelAlign === 'end'
              ? anchor.value.height - bubbleRect.height
              : (anchor.value.height - bubbleRect.height) / 2;
  ...
  const classes = [
        ...
        arrowAnchor !== 'content' && `tooltip_arrow-anchor.value-${arrowAnchor}`, // 화살표 기준입니다.
      ...
      ? <Portal mount={event}>{
          <span
            ...
            style={{
              ...
              zIndex: 10020,
              pointerEvents: 'none',
              '--tooltip-arrow-position': arrowPosition.value || `${anchor.value.width / 2}px`,
            }}
          >
            <span
              {...props}
              ref={bubbleRef}
              id={bubbleId}
              class={['tooltip_bubble', showClose && 'tooltip_bubble-closable']
                .filter(Boolean)
                .join(' ')}
              role="tooltip"
            ...
            >
              {!noArrow && <span class="tooltip_arrow" aria-hidden="true" />}
              {showClose && (
                <Button
                  variant="ghost"
                  iconOnly
                  ...
                  iconBefore={<Icon name="close" size="sm" class="tooltip_close-icon" />}
  ...
  const accessibleTrigger = isQwikElement(triggerContent)
    ? mergeElementProps(triggerContent, {
