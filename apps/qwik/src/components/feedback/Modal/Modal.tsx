...
export const Modal = component$(({
  ...
  const internalOpen = useSignal(defaultOpen); // 비제어 열림 상태입니다.
  ...
  useVisibleTask$(({ track }) => {
    ...
    const handleKeyDown = (event) => {
      ...
      if (openModals[openModals.length - 1] !== rootRef.value) return;
  ...
  return <Portal mount={event}>{
    ...
    >
      ...
      <div
        ...
        aria-hidden="true"
        onClick$={(event) => backdrop && requestClose('backdrop'}</Portal>}
      />
