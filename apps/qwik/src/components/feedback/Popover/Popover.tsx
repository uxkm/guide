...
export const Popover = component$(({
  ...
  const triggerRef = useSignal<HTMLElement | undefined>(undefined); // 트리거 래퍼 참조입니다.
  const panelRef = useSignal<HTMLElement | undefined>(undefined); // 패널 참조입니다.
  const hoverTimerRef = useSignal<HTMLElement | undefined>(undefined); // hover 닫기 지연 타이머입니다.
  const internalOpen = useSignal(defaultOpen);
  const anchor = useSignal(null); // 트리거 뷰포트 좌표입니다.
  const measuredArrowPosition = useSignal(null); // 측정된 화살표 위치입니다.
  const visible = open ?? internalOpen.value;
  ...
  const setVisible = (next, reason, event) => {
    if (disabled) return;
    if (open === undefined) internalOpen.value = next;
  ...
  const cancelHoverClose = () => {
    if (hoverTimerRef.value) clearTimeout(hoverTimerRef.value);
    hoverTimerRef.value = null;
  };
  // hover 이탈 후 잠시 뒤 닫아 패널 이동을 허용합니다.
  const scheduleHoverClose = (event) => {
    cancelHoverClose();
    hoverTimerRef.value = setTimeout(() => setVisible(false, 'hover', event), 100);
  };
  const updatePosition = () => {
    const element = resolveTriggerAnchor(triggerRef.value);
    if (element) anchor.value = topViewportRect(element);
  };
  ...
  useVisibleTask$(({ track }) => () => cancelHoverClose());
  // target·mixed 화살표 기준일 때 트리거 대비 화살표 위치를 측정합니다.
  useVisibleTask$(({ track, cleanup }) => {
    track(() => anchor.value);
    track(() => arrowAnchor);
    track(() => arrowTargetAlign);
    track(() => panelAlign);
    track(() => resolvedPlacement);
    track(() => visible);

    if (!visible || !anchor.value || !panelRef.value || !['target', 'mixed'].includes(arrowAnchor))
      return;
    ...
    const position = ['left', 'right'].includes(resolvedPlacement)
      ? anchor.value.top + anchor.value.height * ratio - panelRect.top
      : anchor.value.left + anchor.value.width * ratio - panelRect.left;
    measuredArrowPosition.value = `${position}px`;
  ...
  const classes = [
        ...
        arrowAnchor !== 'content' && `popover_arrow-anchor.value-${arrowAnchor}`, // 화살표 기준입니다.
        noArrow && 'popover_no-arrow', // 화살표 숨김입니다.
        className,
      ]
        .filter(Boolean)
        .join(' ');
  const arrowPosition = measuredArrowPosition.value || `${(anchor.value?.width || 0) / 2}px`;
  // 트리거에 확장·팝업 접근성 속성을 붙입니다.
  const accessibleTrigger = isQwikElement(triggerContent)
    ? mergeElementProps(triggerContent, { 'aria-expanded': visible, 'aria-haspopup': 'dialog' })
    : triggerContent;
  // 열림·좌표·포털이 준비되면 패널을 포털로 렌더합니다.
  const panel =
    visible && portalRoot && anchor.value
      ? <Portal mount={event}>{
          <div
            class={classes}
            ...
            style={{
              position: 'fixed',
              top: anchor.value.top,
              left: anchor.value.left,
              width: anchor.value.width,
              height: anchor.value.height,
              zIndex: 10000,
              pointerEvents: 'none',
            ...
            >
              ...
              {(title || panelLabel) && (
                <div class="popover_header">
                  <div id={titleId} class="popover_title">
                    {title || panelLabel}
                  </div>
                  {showClose && (
                    <Button
                      ...
                      onClick$={(event) => setVisible(false, 'close'}</Portal>}
                    />
                  )}
                </div>
              )}
              ...
              <div class="popover_body">{children}</div>
              {footer && <div class="popover_footer">{footer}</div>}
            </div>
          </div>,
          portalRoot,
        )
