...
export const CarouselContext = createContextId(null);
...
export const Carousel = component$(({
  ...
  const swiperRef = useSignal<HTMLElement | undefined>(undefined);
  const instanceRef = useSignal<HTMLElement | undefined>(undefined);
  const isPlaying = useSignal(Boolean(autoplay));
  ...
  useVisibleTask$(({ track, cleanup }) => {
    track(() => autoplay);
    track(() => centered);
    track(() => coverflowEffect);
    track(() => coverflowSideCount);
    track(() => coverflowStyle);
    track(() => delay);
    track(() => effect);
    track(() => freeMode);
    track(() => gridFill);
    ...
    track(() => onSwiper);
    track(() => pagination);
    track(() => resolvedSlidesPerView);
    track(() => showNavigation);
    track(() => showPagination);
    track(() => slidesPerGroup);
    track(() => thumbs);
    track(() => thumbsControl);
    track(() => updateCoverflowLayout);
    track(() => watchSlidesProgress);

    if (swiperRef.value) {
      const slideCount = swiperRef.value.querySelectorAll(
      ...
      instanceRef.value?.destroy(true, true);
      const paginationEl =
        rootRef.value?.querySelector(':scope > .swiper-pagination') ??
        swiperRef.value.querySelector('.swiper-pagination');
      ...
      const instance = new Swiper(swiperRef.value, config);
      ...
      return () => {
        ...
        if (instanceRef.value === instance) instanceRef.value = null;
      };
    }
  ...
  useVisibleTask$(({ track, cleanup }) => {
    ...
    if (rootRef.value && typeof ResizeObserver !== 'undefined') {
      const observer = new ResizeObserver(() => {
        ...
        const swiper = instanceRef.value;
  ...
  const classes = [
    ...
    (autoplay || autoplayToggle) && !isPlaying.value && 'is-autoplay-paused',
    className,
  ]
    .filter(Boolean)
    .join(' ');
  ...
  useContextProvider(CarouselContext, context);
  return (
    <div {...props} ref={rootRef} class={classes} style={style} data-component="Carousel">
        <div
          ref={swiperRef}
          class="swiper"
          role="region"
        ...
        >
          ...
          {showPagination && !dotsOutside ? <div class="swiper-pagination" /> : null}
        </div>
        {showPagination && dotsOutside ? <div class="swiper-pagination" /> : null}
        {autoplay || autoplayToggle ? (
          <button
            ...
            data-ripple={ripple ? 'surface' : 'false'}
            aria-label={isPlaying.value ? '슬라이드 정지' : '슬라이드 재생'}
