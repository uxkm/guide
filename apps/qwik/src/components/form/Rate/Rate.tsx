...
export const Rate = component$(({
  ...
  const stars = Array.from({ length: resolvedCount }, (_, index) => index + 1),
    [resolvedCount],
  );
  ...
  if (isReadonly)
    return (
      <div
        ...
        data-component="Rate"
        role="img"
        aria-label={props['aria-label'] || `${resolvedCount}점 만점 중 ${currentValue.value ?? 0}점`}
      >
        <div class="rate_stars">
          {stars.map((star) => {
            const state =
              (currentValue.value ?? 0) >= star
                ? 'is-filled'
                : allowHalf && (currentValue.value ?? 0) >= star - 0.5
                  ? 'is-half'
                  : '';
            return (
              <span key={star} class={['rate_star-readonly', state].filter(Boolean).join(' ')}>
                <StarGraphic half={state === 'is-half'} />
              </span>
            );
          })}
        </div>
        {currentValue.value != null && <span class="rate_value">{currentValue.value}</span>}
      </div>
    );
  ...
  return (
    <fieldset
      ...
      disabled={disabled}
    >
      {legend && <legend class="rate_legend">{legend}</legend>}
      <div class="rate_control">
        <div class="rate_stars">
          {stars.map((star) => (
            <label key={star} class="rate_star" data-ripple={ripple ? 'true' : undefined}>
              ...
              {allowHalf && (
                <input
                  ...
                  name={groupName}
                  value={star - 0.5}
                  checked={currentValue.value === star - 0.5}
                  disabled={disabled}
              ...
              <input
                ...
                name={groupName}
                value={star}
                checked={currentValue.value === star}
                disabled={disabled}
        ...
        {currentValue.value != null && <output class="rate_value">{currentValue.value}점</output>}
      </div>
    </fieldset>
  );
});
