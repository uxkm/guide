...
export const Upload = component$(({
  ...
  const createdUrls = useSignal(new Set());
  const innerFiles = useSignal(defaultFiles);
  const message = useSignal('');
  const dragActive = useSignal(false);
  const files = controlledFiles ?? innerFiles.value; // 최종 파일 목록입니다.

  // 언마운트 시 생성한 미리보기 URL을 모두 해제합니다.
  useVisibleTask$(({ track }) => () => revokeUploadObjectUrls(createdUrls.value));

  const update = (next) => {
    // 제어·비제어에 맞게 목록을 갱신하고 File 배열을 onChange로 전달합니다.
    if (controlledFiles === undefined) innerFiles.value = next;
  ...
  const addFiles = (list) => {
    ...
    const added = incoming.slice(0, available).map((file) => {
      ...
      if (preview) createdUrls.value.add(preview);
      return {
        file,
        name: file.name,
        size: file.size,
  ...
  const remove = (index) => {
    ...
    if (item?.url && createdUrls.value.has(item.url)) {
      URL.revokeObjectURL(item.url);
      createdUrls.value.delete(item.url);
    }
  ...
  const triggerNode =
    resolvedVariant === 'list' ? null : resolvedVariant === 'drag' ? (
      <label
        class={cx(
          'upload_dropzone',
          (dragover || dragActive.value) && 'is-dragover',
          (error || message.value) && 'is-error',
        )}
        for={resolvedInputId}
        ...
        onDragEnter={(event) => {
          event.preventDefault();
          if (!disabled) dragActive.value = true;
        }}
