...
export const Dropdown = component$(({
  ...
  const rootRef = useSignal<HTMLElement | undefined>(undefined); // 바깥 클릭 판별용 루트 참조입니다.
  const id = `dropdown-menu-${useId().replaceAll(':', '')}`; // 메뉴와 트리거를 연결하는 id입니다.
  const internal = useSignal(defaultOpen); // 비제어형 열림 상태입니다.
  const controlled = open !== undefined; // 제어형 사용 여부입니다.
  const visible = controlled ? open : internal.value; // 최종 표시 상태입니다.
  ...
  useVisibleTask$(({ track, cleanup }) => {
    ...
    const outside = (event) => {
      if (!rootRef.value?.contains(event.target)) setOpen(false);
    };
    const escape = (event) => {
      if (event.key === 'Escape') {
        setOpen(false);
  ...
  const trigger = isQwikElement(triggerContent) ? (
    mergeElementProps(triggerContent, triggerProps)
  ) : (
    <Button
      {...triggerProps}
      variant="outline"
  ...
  const menuClick = (event) => {
    ...
    if (closeOnSelect) {
      setOpen(false);
      requestAnimationFrame(() => rootRef.value?.querySelector('.dropdown_trigger')?.focus());
    }
  };
