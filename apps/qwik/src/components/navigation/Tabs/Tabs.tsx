...
export const Tabs = component$(({
  ...
  const listRef = useSignal<HTMLElement | undefined>(undefined); // 탭 목록 DOM을 가리키는 참조입니다.

  // children에서 탭 메타데이터와 패널 콘텐츠를 추출합니다.
  const childItems = toChildArray(children)
        .filter(isQwikElement)
        .map((child, index) => ({
  ...
  const internal = useSignal(defaultValue ?? firstKey); // 비제어형 선택 상태입니다.
  const selected = value ?? internal.value; // 제어·비제어를 합친 최종 선택 키입니다.
  ...
  useVisibleTask$(({ track, cleanup }) => {
    ...
    if (value == null && !source.some((item) => item.key === internal.value && !item.disabled))
      internal.value = firstKey;
  };
  ...
  const indicatorStyle = useSignal(null); // 슬라이드 인디케이터의 위치·크기입니다.
  const scrollState = useSignal({ overflow: false, prev: false, next: false }); // 스크롤 버튼 상태입니다.
  ...
  const select = (key) => {
    ...
    if (target && !target.disabled) {
      if (value == null) internal.value = key;
      onChange?.(key);
  ...
  const tabsList = (
    ...
    >
      {resolvedIndicator === 'slide' && indicatorStyle.value && (
        <span class="tabs_indicator" aria-hidden="true" style={indicatorStyle.value} />
      )}
      {source.map((item, index) => {
        ...
        return (
          <Button
            ...
            fit
            ripple={false}
            class={['tabs_tab', active && 'is-active', item.disabled && 'is-disabled']
              .filter(Boolean)
              .join(' ')}
            role="tab"
            ...
            tabIndex={active ? 0 : -1}
            label={item.label ?? ''}
            iconBefore={item.icon ? <span class="tabs_icon">{item.icon}</span> : undefined}
            iconAfter={item.badge ? <span class="tabs_badge">{item.badge}</span> : undefined}
  ...
  const scrollBy = (direction) =>
    listRef.value?.scrollBy({
      left: direction * Math.max((listRef.value?.clientWidth ?? 160) * 0.75, 120),
      behavior: 'smooth',
    });

  return (
    <div {...props} class={classes} data-component="Tabs" data-tabs>
      <div class="tabs_bar">
        ...
        <div
          class={['tabs_list-wrap', scrollNav && 'tabs_scroll-viewport']
            .filter(Boolean)
            .join(' ')}
        >
          {tabsList}
        ...
        {extra && <div class="tabs_extra">{extra}</div>}
      </div>
        ...
        {resolvedMode === 'dynamic' ? (
          ...
          source.map((item, index) => (
            <div
              key={item.key}
              id={`${uid}-panel-${index}`}
              class={['tabs_panel', item.key === selected && 'is-active']
                .filter(Boolean)
                .join(' ')}
              role="tabpanel"
