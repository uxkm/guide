...
export const StepsItem = component$(({
  title, // 단계 제목 텍스트입니다.
  ...
  const indicator =
    icon ??
    (resolved === 'finished' ? (
      (completeIcon ?? <Icon name="check" class="steps_icon" />)
    ) : resolved === 'error' ? (
      (errorIcon ?? <Icon name="close" class="steps_icon" />)
    ) : (
  ...
  const head = (
    <span class="steps_head">
      ...
      {!isLast && <span class="steps_tail" aria-hidden="true" />}
    </span>
  );
  ...
  const content = (
    <span class="steps_content">
      ...
      {description != null && <span class="steps_desc">{description}</span>}
    </span>
  );
  ...
  return (
    <li
      {...props}
      class={['steps_item', `is-${resolved}`, className].filter(Boolean).join(' ')}
        ...
        (children ?? (
          <>
            ...
            <div class="steps_content">
              ...
              {description != null && <p class="steps_desc">{description}</p>}
            </div>
          </>
        ))
      )}
