...
const count = computed(() => props.paragraph === true ? 3 : typeof props.paragraph === 'number' ? Math.max(0, props.paragraph) : 0);
const defaultCount = computed(() => props.paragraph === false ? 3 : count.value);
const groupClass = computed(() => ['skeleton_group', !props.active && 'skeleton_static'].filter(Boolean));
const itemClass = computed(() => ['skeleton', !props.active && 'skeleton_static'].filter(Boolean));
const widths = ['', 'skeleton_w-md', 'skeleton_w-sm'];
</script>

<template>
  <div v-bind="attrs" :class="groupClass" data-component="Skeleton" role="status" aria-live="polite" aria-busy="true" :aria-label="ariaLabel">
    <div v-if="avatar" class="skeleton_row">
      ...
      <div v-if="count" class="skeleton_row-body">
        <span v-for="index in count" :key="index" :class="[...itemClass, 'skeleton_text', widths[index - 1] || 'skeleton_w-lg']" aria-hidden="true" />
      </div>
    </div>
    ...
    <template v-else>
      ...
      <span v-for="index in defaultCount" :key="index" :class="[...itemClass, 'skeleton_text', widths[index - 1]]" aria-hidden="true" />
    </template>
    <slot />
  </div>
</template>
