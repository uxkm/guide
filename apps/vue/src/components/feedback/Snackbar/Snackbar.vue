...
const props = defineProps({
  message: { type: String, default: 'Snackbar' },
  color: { type: String, default: 'info' },
  size: { type: String, default: 'md' },
  motion: { type: String, default: 'fade' },
  placement: String,
...
const classes = computed(() =>
  [
    'snackbar',
    props.color === 'danger' ? 'color_error' : `color_${props.color}`,
    props.size !== 'md' && `snackbar_${props.size}`,
    props.round && 'snackbar_round',
    `snackbar_motion-${props.motion}`,
    `snackbar_placement-${props.placement || regionPlacement}`,
    `is-${phase.value}`,
  ].filter(Boolean),
);
