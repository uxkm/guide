...
let {
  ...
  class: className,
  ...rest
} = $props();=> ['sm', 'md', 'lg'].includes(value) }, // 별과 값 텍스트 크기입니다.
...
let currentValue = $state(modelValue ?? value);

// 외부 값이 바뀌면 내부 선택 값을 동기화합니다.
$effect(
  () => [modelValue, value],
  ([modelValue, value]) => {
    if (modelValue !== undefined || value !== undefined) currentValue = modelValue ?? value;
  },
);
let stars = $derived(Array.from({ length: Math.max(1, Number(count)) }, (_, index) => index + 1),
);

// 크기·반별·초기화·읽기 전용 상태를 공통 클래스로 변환합니다.
let classes = $derived([
    'rate', // 별점 루트 필수 클래스입니다.
    size === 'sm' && 'rate_sm', // 작은 크기 변형입니다.
    size === 'lg' && 'rate_lg', // 큰 크기 변형입니다.
    allowHalf && 'rate_allow-half', // 반별 선택 변형입니다.
    clearable && 'rate_clearable', // 초기화 버튼 표시 모드입니다.
    readonly && 'is-readonly', // 읽기 전용 상태 클래스입니다.
...
<!-- 읽기 전용은 radio 없이 채움·반별 상태로만 표시합니다. -->
  ...
  >
    <div class="rate_stars">
      ...
      >
        <span class="rate_star-graphic" aria-hidden="true">
          ...
          {#if readonlyState(star) === 'is-half'}<span class="rate_star-icon-half"
            ><svg class="rate_star-icon" viewBox="0 0 24 24" fill="currentColor">
              <path d={starPath} /></svg
          ></span>
        </span>
    ...
    {#if currentValue != null}<span class="rate_value">{{ currentValue }}</span>
  </div>
  ...
  {:else}<fieldset
    ...
    disabled={disabled}
  >
    {#if legend}<legend class="rate_legend">{{ legend }}</legend>
