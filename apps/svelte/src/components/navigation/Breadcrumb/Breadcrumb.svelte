...
let separator = $derived(['chevron', 'slash', 'dot'].includes(separator) ? separator : 'chevron',
); // 검증된 구분 기호입니다.
...
let classes = $derived([
    ...
    separator !== 'chevron' && `breadcrumb_sep-${separator}`, // 기본 chevron이 아닐 때 구분 기호입니다.
    size !== 'md' && `breadcrumb_${size}`, // 기본 md가 아닐 때 크기 변형입니다.
  ].filter(Boolean),
); // false 등 적용되지 않는 항목을 제거합니다.
</script>

<!-- items가 있으면 BreadcrumbItem으로 매핑하고, 없으면 기본 slot을 사용합니다. -->
  <nav {...rest} class={classes} aria-label={ariaLabel} data-component="Breadcrumb">
    <ol class="breadcrumb_list">
      {#if items.length}<><BreadcrumbItem
