...
lead: Studio의 project 구성, 화면 작성, UDC와 debugging 흐름을 적용 관점에서 정리합니다.
order: 61
---
...
project에는 화면 XML, JavaScript, style, image와 설정 파일이 함께 존재합니다. 공통 CSS는 project asset 정책에 따라 한 번만 load하고 화면마다 중복 bundle하지 않습니다.

## 화면 그리기

1. page 또는 WFrame 화면을 만듭니다.
2. layout container와 업무 component를 배치합니다.
3. 공통 class와 variant를 적용합니다.
...
반복되는 업무 UI를 UDC로 만들 때 public property와 event 계약을 먼저 정의합니다. XML을 JavaScript 방식으로 전환할 경우 생성 순서와 scope가 달라질 수 있으므로 HTML fragment를 문자열로 복제하지 않습니다.
