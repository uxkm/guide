...
export function Dropdown({
  ...
  let rootRef = null; // 바깥 클릭 판별용 루트 참조입니다.
  const id = `dropdown-menu-${createUniqueId().replaceAll(':', '')}`; // 메뉴와 트리거를 연결하는 id입니다.
  const [internal, setInternal] = createSignal(defaultOpen); // 비제어형 열림 상태입니다.
  ...
  createEffect(() => {
    ...
    const outside = (event) => {
      if (!rootRef?.contains(event.target)) setOpen(false);
    };
    const escape = (event) => {
      if (event.key === 'Escape') {
        setOpen(false);
  ...
  const triggerProps = {
    class: ['dropdown_trigger', triggerContent?.props?.className].filter(Boolean).join(' '),
  ...
  const trigger = isSolidChild(triggerContent) ? (
    mergeChildProps(triggerContent, triggerProps)
  ) : (
    <Button
      {...triggerProps}
      variant="outline"
  ...
  const menuClick = (event) => {
    ...
    if (closeOnSelect) {
      setOpen(false);
      requestAnimationFrame(() => rootRef?.querySelector('.dropdown_trigger')?.focus());
    }
  };
