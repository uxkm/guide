...
export function Tabs({
  ...
  const uid = createUniqueId().replaceAll(':', ''); // aria id 연결에 쓰는 안전한 접두사입니다.
  let listRef = null; // 탭 목록 DOM을 가리키는 참조입니다.
  ...
  const [internal, setInternal] = createSignal(defaultValue ?? firstKey); // 비제어형 선택 상태입니다.
  ...
  const [indicatorStyle, setIndicatorStyle] = createSignal(null); // 슬라이드 인디케이터의 위치·크기입니다.
  const [scrollState, setScrollState] = createSignal({ overflow: false, prev: false, next: false }); // 스크롤 버튼 상태입니다.
  ...
  const tabsList = (
    ...
    >
      {resolvedIndicator === 'slide' && indicatorStyle() && (
        <span class="tabs_indicator" aria-hidden="true" style={indicatorStyle()} />
      )}
  ...
  const scrollBy = (direction) =>
    listRef?.scrollBy({
      left: direction * Math.max((listRef?.clientWidth ?? 160) * 0.75, 120),
      behavior: 'smooth',
    });
