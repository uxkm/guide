...
export function BreadcrumbItem({
  ...
  if (href && !current && !disabled)
    inner = (
      <a class={linkClasses} href={href} aria-label={ariaLabel} onClick={onClick}>
        {content}
      </a>
    );
  else if (disabled)
  ...
  else if (current) inner = <span class="breadcrumb_current">{content}</span>;

  return (
    <li
      {...props}
      class={['breadcrumb_item', current && 'is-current', className].filter(Boolean).join(' ')}
