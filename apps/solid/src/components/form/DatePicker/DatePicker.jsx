...
export function DatePicker({
  ...
  const [innerValue, setInnerValue] = createSignal(defaultValue);
  const selectedValue = controlled ? value : innerValue(); // 최종 선택 값입니다.
  const [open, setOpen] = createSignal(false);
  const [month, setMonth] = createSignal(parseValue(selectedValue) || new Date());
  let rootRef = null;
  const cells = createMemo(() => getCells(month()));
  ...
  const select = (next) => {
    ...
    commit(next);
    setOpen(false);
    requestAnimationFrame(() => rootRef?.querySelector('.date_picker_input')?.focus());
  };
