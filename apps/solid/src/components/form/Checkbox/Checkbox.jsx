...
export function Checkbox({
  ...
  createEffect(() => {
    if (inputRef) inputRef.indeterminate = Boolean(indeterminate);
  });
  ...
  if (standalone)
    return (
      <label
        class={['checkbox_control', className].filter(Boolean).join(' ')}
  ...
  if (button)
    return (
      <label class={classes} {...rippleAttrs}>
        {input}
        <span class="checkbox_label">{content}</span>
      </label>
    );

  // 기본형은 컨트롤과 레이블을 배치 옵션에 맞게 나란히 둡니다.
  return (
    <div class={classes} {...rippleAttrs}>
      {labelEnd && (
        <label class="checkbox_label" htmlFor={inputId}>
          {content}
        </label>
      )}
