...
export function Rate({
  ...
  const [currentValue, setCurrentValue] = createSignal(value ?? defaultValue);
  const stars = createMemo(() => Array.from({ length: resolvedCount }, (_, index) => index + 1));
  ...
  if (isReadonly)
    return (
      ...
      >
        ...
        {currentValue() != null && <span class="rate_value">{currentValue()}</span>}
      </div>
    );
  ...
  return (
    ...
    >
      ...
      <div class="rate_control">
        ...
        {currentValue() != null && <output class="rate_value">{currentValue()}점</output>}
      </div>
    </fieldset>
  );
}
