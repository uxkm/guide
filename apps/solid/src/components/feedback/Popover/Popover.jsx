...
export function Popover({
  ...
  let triggerRef = null; // 트리거 래퍼 참조입니다.
  let panelRef = null; // 패널 참조입니다.
  let hoverTimerRef = null; // hover 닫기 지연 타이머입니다.
  const [internalOpen, setInternalOpen] = createSignal(defaultOpen);
  const [anchor, setAnchor] = createSignal(null); // 트리거 뷰포트 좌표입니다.
  const [measuredArrowPosition, setMeasuredArrowPosition] = createSignal(null); // 측정된 화살표 위치입니다.
  ...
  const cancelHoverClose = () => {
    if (hoverTimerRef) clearTimeout(hoverTimerRef);
    hoverTimerRef = null;
  };
  // hover 이탈 후 잠시 뒤 닫아 패널 이동을 허용합니다.
  const scheduleHoverClose = (event) => {
    cancelHoverClose();
    hoverTimerRef = setTimeout(() => setVisible(false, 'hover', event), 100);
  };
  const updatePosition = () => {
    const element = resolveTriggerAnchor(triggerRef);
  ...
  createEffect(() => {
    ...
    const closeOutside = (event) => {
      if (!triggerRef?.contains(event.target) && !panelRef?.contains(event.target))
  ...
  createEffect(() => {
    if (!visible || !anchor() || !panelRef || !['target', 'mixed'].includes(arrowAnchor))
      return;
  ...
  const accessibleTrigger = isSolidChild(triggerContent)
    ? mergeChildProps(triggerContent, { 'aria-expanded': visible, 'aria-haspopup': 'dialog' })
    : triggerContent;
