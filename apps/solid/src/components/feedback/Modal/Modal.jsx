...
export function Modal({
  ...
  const generatedId = createUniqueId().replace(/:/g, ''); // React id에서 콜론을 제거합니다.
  const modalId = id || `modal-${generatedId}`; // 최종 루트 id입니다.
  const titleId = `${modalId}-title`; // aria-labelledby에 연결할 제목 id입니다.
  let rootRef = null; // 포커스 트랩용 루트 참조입니다.
  let previousFocusRef = null; // 닫힌 뒤 복원할 이전 포커스입니다.
  const [internalOpen, setInternalOpen] = createSignal(defaultOpen); // 비제어 열림 상태입니다.
  ...
  createEffect(() => {
    ...
    previousFocusRef = targetDocument.activeElement;
    ...
    const handleKeyDown = (event) => {
      ...
      if (openModals[openModals.length - 1] !== rootRef) return;
      if (event.key === 'Escape') requestClose('escape', event);
      if (event.key !== 'Tab' || !rootRef) return;
      const focusable = [
      ...
      <div class="modal_dialog">
        ...
        {(title || header) && (
          <div class="modal_header">
            {header ?? (
              <h2 class="modal_title" id={titleId}>
                {title}
              </h2>
            )}
            <Button
              ...
              iconBefore={<Icon name="close" size="sm" class="modal_close-icon" />}
              onClick={(event) => requestClose('close', event)}
            />
          </div>
        )}
        <div class="modal_body">{children}</div>
