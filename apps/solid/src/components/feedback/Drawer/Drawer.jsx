...
export function Drawer({
  ...
  let panelRef = null; // 드래그 높이 조절용 패널 참조입니다.
  let dragRef = null; // 진행 중인 드래그 상태입니다.
  let previousFocusRef = null; // 닫힌 뒤 복원할 이전 포커스입니다.
  const [internalOpen, setInternalOpen] = createSignal(defaultOpen || openOnLoad);
  ...
  const rootClasses = createMemo(() => ['drawer', visible && 'is-open', className].filter(Boolean).join(' '));
  ...
  createEffect(() => {
    ...
    const handleKeyDown = (event) => {
      ...
      if (openDrawers[openDrawers.length - 1] !== rootRef) return;
