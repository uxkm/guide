...
export function Tooltip({
  ...
  createEffect(() => {
    ...
    const outside = (event) => {
      if (!triggerRef?.contains(event.target) && !bubbleRef?.contains(event.target))
  ...
  createEffect(() => {
    if (!visible || !anchor() || !bubbleRef || !['target', 'mixed'].includes(arrowAnchor))
      return;
    const bubbleElement = bubbleRef;
  ...
  const accessibleTrigger = isSolidChild(triggerContent)
    ? mergeChildProps(triggerContent, {
