...
const apiSections = [
  {
    title: 'API · Props',
    description:
      'React에서는 camelCase prop을 사용합니다. Vue의 model-value · select-text · list-size는 value · selectText · listSize로 다룹니다.',
    tables: [{ columns: selectPropColumns, rows: selectProps, codeColumn: 'name' }],
  },
  {
    title: 'API · Children',
    description: 'Vue default 슬롯에 대응하는 React children입니다.',
    tables: [{ columns: selectChildColumns, rows: selectChildren, codeColumn: 'name' }],
  },
  {
    title: 'API · Events',
    description: 'Vue update:modelValue는 React onChange(event)에 대응합니다.',
    tables: [{ columns: selectEventColumns, rows: selectEvents, codeColumn: 'name' }],
  },
  {
    title: '클래스 · 속성',
    description:
      'React 컴포넌트가 렌더하는 OOCSS 클래스입니다. HTML 마크업으로 직접 작성할 때 동일하게 조합합니다.',
    tables: [{ columns: selectClassColumns, rows: selectClasses, codeColumn: 'name' }],
  },
  {
    title: '디자인 토큰',
    tables: [{ columns: selectTokenColumns, rows: selectTokens, codeColumn: 'name' }],
  },
];
...
export default {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: '네이티브 select와 커스텀 트리거를 제공하는 Select 컴포넌트입니다.',
      },
    },
  },
};
...
export const Playground = {
  ...
  render: (args, { updateArgs }) =>
    args.custom ? (
      ...
      <Select {...args} onChange={(event) => updateArgs({ value: event.target.value })}>
