...
function frameDemo(children) {
  return (
    <div className="drawer_demo-frame">
      <div className="drawer_demo-content">{children}</div>
    </div>
  );
}

/** Docs·Canvas 동일 — 트리거 + Drawer를 한 템플릿에 포함 */
function drawerDemo(children) {
...
export default {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: '측면·상하에서 슬라이드되는 Drawer 패널 컴포넌트입니다.',
      },
    },
  },
};
...
function cnOpen(open) {
  return open ? 'drawer_demo-static' : undefined;
}
