...
const formatCode = createComponentFormatter('FlexItem', { booleanProps: new Set(['grow', 'fit']) });
...
export default function FlexItem({
  ...
  useComponentDemoCode(
    formatCode,
    { as: Component === 'div' ? undefined : Component, span, spanMd, spanLg, grow, fit, align: resolvedAlign, order },
    demoSlots,
    rootRef,
    { className, ...rest },
  );
  ...
  return (
    <Component ref={rootRef} className={cn(rootClass, className)} {...normalizeDomProps(rest)}>
      {children}
    </Component>
  );
}
