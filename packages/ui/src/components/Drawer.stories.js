...
const apiSections = [
  ...
  },
  {
    title: "API · Slots",
    tables: [
      { columns: drawerSlotColumns, rows: drawerSlots, codeColumn: "name" },
    ],
  },
  {
    title: "클래스 · 속성",
...
/** Docs·Canvas 동일 — 트리거 + Drawer를 한 템플릿에 포함 (Controls 오버레이 Drawer 추가 방지) */
function drawerDemo(docsRender) {
  return withDocsCanvasRender(docsRender, docsRender);
}

/** 예시 영역 — 충분한 높이·너비의 프레임 안에서 Drawer UI를 확인 */
