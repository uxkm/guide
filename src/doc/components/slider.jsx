      ...
      <div className="page_intro">
        ...
        >
          ...
          <Slider
            className="slider_fit"
            value={55}
            ariaLabel="단독 슬라이더 — aria-label"
          />
          <p className="slider_hint">레이블 없이 사용할 때는 <code>aria-label</code>로 접근 가능한 이름을 지정합니다.</p>
          <Slider
            ...
            max={24}
            step={1}
            ariaDescribedby="slider-type-labelledby-hint"
            dataSliderValuetextSuffix="픽셀"
            showValue
          />
          <p className="slider_hint" id="slider-type-labelledby-hint">12~24px 사이 글자 크기</p>
          <Slider
            id="slider-type-describedby"
            className="slider_fit"
            label="aria-describedby — 보조 설명"
            value={70}
            ariaDescribedby="slider-type-describedby-hint"
            dataSliderSuffix="%"
            dataSliderValuetextSuffix="퍼센트"
            showValue
          />
          <p className="slider_hint" id="slider-type-describedby-hint">0%는 최소, 100%는 최대 대비입니다. 힌트 id를 <code>aria-describedby</code>에 연결합니다.</p>
          <Slider
            ...
            max={5}
            step={1}
            dataSliderValuetextMap="1:매우 낮음,2:낮음,3:보통,4:높음,5:매우 높음"
            showValue
          />
          <p className="slider_hint">숫자 대신 <code>aria-valuetext</code>로 “보통”처럼 읽기 쉬운 값을 전달합니다. 시각적 output은 <code>aria-hidden="true"</code>로 중복 낭독을 줄일 수 있습니다.</p>
        </DemoSection>
        ...
        >
          <Slider className="slider_fit" value={30} ariaLabel="기본 슬라이더" />
        </DemoSection>
        ...
        >
          <Slider id="slider-size-sm" className="slider_sm slider_fit" label="Small — slider_sm" value={30} showValue />
          <Slider id="slider-size-md" className="slider_fit" label="Medium — 기본" value={50} showValue />
          <Slider id="slider-size-lg" className="slider_lg slider_fit" label="Large — slider_lg" value={70} showValue />
        </DemoSection>
        ...
        >
          <FormLayout layout="vertical" fit>
            <div className="form_field">
              <label className="form_field-label" htmlFor="slider-form-opacity">불투명도</label>
              <Slider id="slider-form-opacity" value={85} dataSliderSuffix="%" />
              <p className="form_field-hint">0~100% 사이 값</p>
            </div>
            <div className="form_field">
              <label className="form_field-label" htmlFor="slider-form-zoom">확대 비율</label>
              <Slider
                id="slider-form-zoom"
                label="현재 값"
                value={125}
          ...
          <FormLayout layout="horizontal" fit>
            <div className="form_field">
              <label className="form_field-label" htmlFor="slider-form-horizontal">선 굵기</label>
        ...
        <ApiSection headingId="api-slots-heading" title="API · Slots">
          <ApiTable columns={sliderSlotColumns} rows={sliderSlots} codeColumn="name" />
        </ApiSection>
