import { inject, onUnmounted, provide, ref, useId, watchEffect } from 'vue';
import {
...
const DEMO_CODE_KEY = Symbol('demoCode');

/** DemoSection 슬롯 콘텐츠는 inject 트리 밖이므로 DOM 기준으로 섹션 레지스트리를 조회 */
...
function resolveRegistryForNode(node, registry) {
...
function createDemoCodeStore(sectionId) {
  ...
  return { sectionId, registry, revision, register, unregister, buildCode };
}
...
function useDemoCodeRegistration(registerFn, options = {}) {
  ...
  stopWatch = watchEffect(() => {
    ...
    if (!payload) {
      activeContext?.unregister(demoId);
      activeContext = null;
      return;
    }
...
function mergeChildrenWithSlotTemplates(registered, childLines) {
  const trimmed = dedentBlock(registered);
  const match = trimmed.match(/^<([^>]+)>([\s\S]*)<\/(\w+)>\s*$/);
  if (!match || !childLines.length) return trimmed;
  ...
  return [`<${openAttrs}>`, body, children, `</${closeName}>`].join('\n');
}
...
export function provideDemoCode() {
  const sectionId = useId();
  const store = createDemoCodeStore(sectionId);

  demoSectionStores.set(sectionId, store);
  provide(DEMO_CODE_KEY, store);
...
export function useButtonDemoCode(propsOrGetter, slots, buttonRef, attrs = {}) {
  return useDemoCodeRegistration(() => {
    const el = resolveDemoElement(buttonRef.value);
    if (!el) return null;
...
export function useTableDemoCode(propsOrGetter, slots, rootRef, attrs = {}) {
  const injectedContext = inject(DEMO_CODE_KEY, null);
  const resolveProps = () =>
  ...
  return useDemoCodeRegistration(() => {
    ...
    return {
      el,
      context,
      code: () => formatTableCode(resolveProps(), slots, attrs, el, context?.registry),
    };
  });
}

export function useComponentDemoCode(formatter, propsOrGetter, slots, rootRef, attrs = {}, options = {}) {
  const { when } = options;
  const resolveProps = () =>
    typeof propsOrGetter === 'function' ? propsOrGetter() : propsOrGetter;
  ...
  return useDemoCodeRegistration(
    ...
    {
      when: when ? () => when(resolveProps()) : undefined,
    },
  );
}

export function usePopoverDemoCode(propsOrGetter, rootRef, attrs = {}) {
  const injectedContext = inject(DEMO_CODE_KEY, null);
  const resolveProps = () =>
    typeof propsOrGetter === 'function' ? propsOrGetter() : propsOrGetter;
...
function useSlotComponentDemoCode(formatter, propsOrGetter, rootRef, attrs = {}) {
  const injectedContext = inject(DEMO_CODE_KEY, null);
  const resolveProps = () =>
    typeof propsOrGetter === 'function' ? propsOrGetter() : propsOrGetter;
...
export function useInputDemoCode(propsOrGetter, rootRef, attrs = {}) {
  const injectedContext = inject(DEMO_CODE_KEY, null);
  const resolveProps = () =>
    typeof propsOrGetter === 'function' ? propsOrGetter() : propsOrGetter;
...
export function useTooltipDemoCode(propsOrGetter, rootRef, attrs = {}) {
  const injectedContext = inject(DEMO_CODE_KEY, null);
  const resolveProps = () =>
    typeof propsOrGetter === 'function' ? propsOrGetter() : propsOrGetter;
